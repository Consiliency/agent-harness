...
**Fable and Opus seats are first-party-subscription only.** The seat must run on a logged-in first-party subscription session. Never substitute a gateway, API key, SDK, direct HTTP call, alternate endpoint, or cloud-provider selector, and never add such a fallback.

**Which backing depends on the harness, and native is the rule.** Inside a NATIVE Claude harness (Claude Code), fill the seat with the driving session's own native sub-agent (Task tool): that session already IS the first-party subscription, so the policy the TUI requirement exists to enforce is satisfied, and there is no second TUI to drive. The runtime defers such a seat with `under_claude_code` precisely so the driver fills it natively -- treat that, and `tui_adapter_required` on a native harness, as *fill this yourself*, NOT as an unavailable seat (ah#396). Leaving it unfilled runs the board below its independence floor while looking merely degraded.

The homebrew self-PTY TUI adapter is for NON-NATIVE harnesses (codex / gemini / opencode), where a native sub-agent would not be a first-party Claude session. There the runtime requires the self-PTY backing, rejects alternate backings before gateway access, launches the exact requested model only after a metadata-only probe proves a logged-in first-party subscription, scrubs tokens, API helpers, custom request headers, alternate endpoints, and cloud-provider selectors, and isolates user/project/local settings. On those hosts a seat whose adapter cannot run fails closed; retry from a host where it can.
...
Under Claude Code, expect a default Claude seat to report `UNAVAILABLE` with `detail="tui_adapter_required"` and no native-fill request. This is an explicit unavailable seat, not authorization to bypass the TUI/auth boundary.
