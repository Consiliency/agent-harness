...
2. Spawn a native sub-agent (the Agent tool, `model: "opus"` or the seat's model) with `instructions.md` as its brief and `artifact.md` as the material; it must end with `AGREE` / `PARTIALLY AGREE` / `DISAGREE` on the LAST line. Write its review to `native-fill/<request_id>/claude.md`.
