...
Under Claude Code the default Claude seat reports `UNAVAILABLE` with `detail="under_claude_code"` AND a native-fill request. Fill it with the emit → fill → invoke protocol (no re-spend, no mailbox):
