...
Under Claude Code, expect the `claude` leg to report `UNAVAILABLE` (deferred to the native Agent) and carry a `needs_native_agent` request — the runtime does not spawn a Claude TUI here. Fill the seat with a native Fable Task Agent (`model: fable`, at `req.lens` / `req.effort`), then reconcile its verdict with ALL the runtime legs (`grok` + `codex` + `gemini`, whichever are up and authed) — the board is 4-vendor, so do not drop `grok`. Leaving the seat unfilled is an explicit, provenance-tagged choice, never a silent short-a-seat board.
