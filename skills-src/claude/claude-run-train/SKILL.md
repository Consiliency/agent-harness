...
- Under Claude Code the train review's claude seat is filled NATIVELY by this session (REVIEWTRUTH
  early slice, EC-REVIEWTRUTH-14): run `--governed --review-only --review-material material.json --emit-native-request` first (it
  ...
  `--governed --review-only --review-material material.json --native-leg claude=<that dir>`. A moved train refuses the fill as
