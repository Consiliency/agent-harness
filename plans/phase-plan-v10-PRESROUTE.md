...
- **Post-landing policy amendment** (agent-harness#1025): Opus 5.5 is now the built-in first rung. The SL-0 receipt still proves agent-harness#998's original frozen bytes; the amended ladder assertion has separate tests-first evidence. Verify the receipt in agent-harness#998's landing checkout, never against current amended tests.
...
- [ ] EC-PRESROUTE-0 — the original tests-first freeze is proven by `content_tdd_receipt.v1` and RED evidence at the historical agent-harness#998 landing; falsified by a changed frozen file at that landing, a missing receipt, or an absent RED log. The agent-harness#1025 amendment intentionally changed the ladder assertion after that landing; its RED-on-unchanged-main and current green evidence supersede byte-equality against today's tests. Keep the historical receipt unchanged.
