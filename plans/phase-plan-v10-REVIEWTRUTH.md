...
`early_prover` has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `outcome`, `receipt`, `role`, `seat`, `started_at`, `usable`, and `vendor`; it must be usable Codex, `role="early_prover"`, `capability="can_probe"`, `binding_prover=false`, and `outcome="CLEAR"`. Its strict canonical receipt bytes independently bind the same phase/wave/attempt/base, start/completion instants, capability, and artifact reference. `bundle_staging` has exactly `bundle`, `early_artifact`, `early_receipt`, and `staged_at`; both early references equal the `early_prover` references exactly, and `early_prover.completed_at < bundle_staging.staged_at`. The referenced bundle is itself canonical `v10.review-evidence-bundle.v1` JSON with exactly `schema`, `base`, and sorted unique `entries`; its `base` equals the receipt base and its direct file entries include the exact final-audit, final-evidence, early-receipt, and early-artifact references. The bundle contains no receipt or bundle self-digest. This is the sole evidence-bearing Option-2 bundle: every reviewer receives only this by-reference bundle and read-only referenced bytes, with no arbitrary working-tree or shared-development-database execution.

`seats` is the ordered four-entry array for Grok, GPT-5.6 Sol, Gemini, and Fable. Each strict entry has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `counts_toward_floor`, `lens`, `material_findings`, `outcome`, `position`, `receipt`, `role`, `seat`, `started_at`, `usable`, `vendor`, and `verdict`. Each retained seat receipt is canonical JSON that repeats and binds those values plus the same phase/wave/attempt/base; Stage A parses it and rejects a copied wrapper claim. Every counting critic and Fable has `consumed_bundle` exactly equal to `bundle_staging.bundle`, starts strictly after `bundle_staging.staged_at`, and has a recomputed receipt and artifact digest. Positions 1 through 3 are critics with `binding_prover=false`; position 4 is Fable, the only `role="binding_prover"`, the only `binding_prover=true`, and starts strictly after all three critic completion instants. The final authorizing wave has no retained contradiction or material finding, and its effective-policy facts are derived rather than asserted: Stage A recomputes the roadmap-specific typed resolver input/output, vendor/lens counts, consensus, and prover fact. GPT-5.6 Sol and Fable must each be `outcome="reviewed"`, `usable=true`, and `verdict="AGREE"`; Fable alone supplies the prover fact.

`effective_policy` has exactly `consensus`, `gate`, `on_shortfall`, `required_lens_coverage`, `required_prover`, `required_vendors`, `resolver_input`, and `resolver_output`. The completion wave uses `gate="pre-merge-CR"` and resolves the shipped default `required_vendors=3`, `required_lens_coverage=3`, and `required_prover=true`; the common policy machinery remains parameterized for all four gates and retains a valid explicit boolean `required_prover=false` override that removes only the prover shortfall. The shipped defaults for plan-ratify, design-ratify, pre-merge-CR, and release-dispatch remain 3/3/true. Stage A re-hashes both policy artifacts, re-runs their strict typed resolution, and derives counted vendors, lenses, Sol AGREE, and the Fable prover fact from the seat receipts.

`restart_chain` is a chronological array of strict objects with exactly `attempt`, `finding`, `invalidated_agree_artifacts`, `prior_bundle`, `prior_receipt`, and `replacement_bundle_sha256`. An empty array means no contradiction occurred. Otherwise Stage A recursively rebuilds every earlier receipt and finding reference, requires contiguous attempts ending at the final `attempt`, requires each material finding or contradiction to invalidate every prior retained `AGREE`, requires a different replacement bundle digest, and requires the last replacement digest to equal the final staged bundle. A finding, contradiction, changed phase byte, or evidence change therefore invalidates the complete prior wave—not one seat—then repeats early prover CLEAR, bundle staging, all critics, and Fable in full. The producer closes every artifact descriptor, makes the evidence tree immutable, and has a fresh coordinator process recompute all file references, canonical bundle bytes, strict seat receipts, chronology, effective policy, restart chain, and inner receipt digest before `update_lifecycle(..., "completed", ...)` is allowed to emit the event.
...
- [ ] PC-REVIEWTRUTH-2 — Broad compatibility accounting is independent from phase-node accounting, separates marker deselections from skips, and uses the two-stage exact-base reanchor protocol above. Stage A runs only after CONFORM and HARDEN are both present in one exact fetched canonical base through two distinct operator-supplied completion-control merge identities `CONFORM_F` and `HARDEN_F`; identical values are rejected. Before any network fetch, the preflight authenticates remote `origin` as canonical `Consiliency/agent-harness`: fetch and push each have exactly one URL, each URL is exactly one of `git@github.com:Consiliency/agent-harness.git`, `ssh://git@github.com/Consiliency/agent-harness.git`, or `https://github.com/Consiliency/agent-harness.git`, both parse to the same case-sensitive owner/repository identity, and no effective `url.*.insteadOf` or `url.*.pushInsteadOf` rule exists at any config origin. A missing/extra origin URL, helper/rewritten/noncanonical URL, different fetch/push identity, or noncanonical owner/repository fails before fetch. The coordinator also supplies `REVIEWTRUTH_RUNNER_ROOT` as a new canonical absolute path outside the exact git toplevel and `REVIEWTRUTH_UV` as a canonical absolute executable path.
  ...
  After origin authentication the preflight fetches exact `refs/remotes/origin/main`, requires the clean full worktree HEAD to equal that fetched commit, and treats each supplied `F` as a completion-control merge rather than an arbitrary ancestor. For phase `P` in `{CONFORM,HARDEN}`, it proves `F` is a non-base, non-identical ancestor of fetched main with exactly ordered parents `[M,FH]`; `FH` has the sole parent `M`; both `M..FH` and `M..F` change only `plans/manifest.json` and preserve every other blob; and the fetched manifest has exactly one phase-qualified row. In that row, the exact unique strict `completed` event carrying `metadata.phase_completion_landing.schema=v10.phase-completion-landing.v1` is absent at `M`, appended in `FH` after an exact lifecycle prefix, byte-semantically unchanged in `F` and fetched main, and consistent with row status/timestamp. Its payload has exactly the producer-frozen top-level keys and the one nested `review_wave` contract above. Stage A requires explicit canonical evidence roots for both phases, matches each root to `review_wave.receipt.evidence_root_id`, resolves every immutable relative locator beneath only that root, recomputes every byte count and SHA-256, parses and canonicalizes the bundle and every early/seat/restart receipt, rebuilds `review_wave.receipt_sha256`, and validates common base, bundle, policy, chronology, role, and complete-restart bindings. It independently derives usable `AGREE` Sol and Fable and Fable-only prover facts under the effective roadmap policy; no SHA regex, aggregate claim, producer boolean, or equality among un-opened metadata values is evidence. Any missing, duplicate, drifted, wrong-phase, stale-plan/roadmap, unreadable or unrehashable locator, noncanonical JSON, arbitrary digest/outcome, wrong bundle, chronology or policy mismatch, stale `AGREE`, incomplete restart, self-referential receipt/`FH`/`F`, or topology/delta mismatch fails. Only topology derives `FH` and `F`; neither may appear in the event or receipt. The preflight writes a private write-once sidecar recording the authenticated origin fetch/push spellings and identity, exact fetched base commit/tree, both derived `{completion_control_merge:F,audited_implementation_landing:M,control_head:FH}` triples, both verified evidence-root identities, and both recomputed review-wave receipt/bundle digests while retaining `conform_landing=CONFORM_F` and `harden_landing=HARDEN_F` only as explicit compatibility aliases.
...
class EvidenceRoot:
    def __init__(self, supplied, label):
    ...
    def identity(self, evidence_root_id):
        return {
...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, final, label):
    ...
    need(early["seat"] == "codex" and early["vendor"] == "openai", f"{label}: early identity")
    need(early["role"] == "early_prover" and early["capability"] == "can_probe", f"{label}: early role")
    need(boolean(early["binding_prover"], f"{label}.early.binding") is False, f"{label}: early binding")
    need(boolean(early["usable"], f"{label}.early.usable") is True and early["outcome"] == "CLEAR", f"{label}: early CLEAR")
    early_started = timestamp(early["started_at"], f"{label}.early.started_at")
    early_completed = timestamp(early["completed_at"], f"{label}.early.completed_at")
    need(early_started < early_completed, f"{label}: early chronology")
    root.read_ref(file_ref(early["artifact"], f"{label}.early.artifact"), f"{label}.early.artifact")
    early_receipt, early_digest = root.json_ref(file_ref(early["receipt"], f"{label}.early.receipt"), f"{label}.early.receipt")
    expected_early = {key: value for key, value in early.items() if key != "receipt"}
    ...
    for required in (receipt["base"]["final_audit"], receipt["base"]["final_evidence"], early["receipt"], early["artifact"]):
    ...
    roster = ((1, "grok", "critic", "read_only", False), (2, "gpt-5.6-sol", "critic", "read_only", False), (3, "gemini", "critic", "read_only", False), (4, "fable", "binding_prover", "binding_prover", True))
    seat_completed = []
    agree_artifacts = []
    finding_refs = []
    for index, (seat, expected) in enumerate(zip(seats, roster)):
        exact_keys(seat, {"artifact", "binding_prover", "capability", "completed_at", "consumed_bundle", "counts_toward_floor", "lens", "material_findings", "outcome", "position", "receipt", "role", "seat", "started_at", "usable", "vendor", "verdict"}, f"{label}.seat[{index}]")
        position, seat_name, role, capability, binding = expected
        need((seat["position"], seat["seat"], seat["role"], seat["capability"], seat["binding_prover"]) == expected, f"{label}.seat[{index}]: role")
        integer(seat["position"], f"{label}.seat[{index}].position", 1)
        boolean(seat["binding_prover"], f"{label}.seat[{index}].binding")
        usable = boolean(seat["usable"], f"{label}.seat[{index}].usable")
        counts = boolean(seat["counts_toward_floor"], f"{label}.seat[{index}].counts")
        text(seat["vendor"], f"{label}.seat[{index}].vendor")
        text(seat["lens"], f"{label}.seat[{index}].lens")
        need(seat["outcome"] in {"reviewed", "unavailable", "errored", "timed_out", "refused", "capped", "empty"}, f"{label}.seat[{index}]: outcome")
        need(seat["verdict"] in {"AGREE", "DISAGREE", "DEFER"}, f"{label}.seat[{index}]: verdict")
        need(counts is (usable and seat["outcome"] == "reviewed"), f"{label}.seat[{index}]: derived count")
        started = timestamp(seat["started_at"], f"{label}.seat[{index}].started_at")
        finished = timestamp(seat["completed_at"], f"{label}.seat[{index}].completed_at")
        need(staged_at < started < finished, f"{label}.seat[{index}]: chronology")
        seat_completed.append(finished)
        need(seat["consumed_bundle"] == bundle_ref, f"{label}.seat[{index}]: bundle")
        root.read_ref(file_ref(seat["artifact"], f"{label}.seat[{index}].artifact"), f"{label}.seat[{index}].artifact")
        root.read_ref(file_ref(seat["consumed_bundle"], f"{label}.seat[{index}].consumed_bundle"), f"{label}.seat[{index}].consumed_bundle")
        need(type(seat["material_findings"]) is list, f"{label}.seat[{index}]: findings")
        seat_findings = []
        for finding_index, finding in enumerate(seat["material_findings"]):
            ref = file_ref(finding, f"{label}.seat[{index}].finding[{finding_index}]")
            validate_finding(ref, root, forbidden, f"{label}.seat[{index}].finding[{finding_index}]")
            seat_findings.append(ref)
        finding_refs.extend(seat_findings)
        seat_receipt, seat_digest = root.json_ref(file_ref(seat["receipt"], f"{label}.seat[{index}].receipt"), f"{label}.seat[{index}].receipt")
        expected_seat = {key: value for key, value in seat.items() if key != "receipt"}
        expected_seat.update({"attempt": attempt, "base": receipt["base"], "phase_alias": receipt["phase_alias"], "wave_id": receipt["wave_id"]})
        need(seat_receipt == expected_seat, f"{label}.seat[{index}]: receipt binding")
        reject_forbidden(seat_receipt, forbidden | {seat_digest}, f"{label}.seat[{index}].receipt")
        if usable and seat["outcome"] == "reviewed" and seat["verdict"] == "AGREE":
            agree_artifacts.append(seat["artifact"])
    need(timestamp(seats[3]["started_at"], f"{label}.fable.started_at") > max(seat_completed[:3]), f"{label}: Fable order")
    need(completed_at > max(seat_completed), f"{label}: receipt completion")
    need(sum(seat["binding_prover"] for seat in seats) == 1 and seats[3]["binding_prover"], f"{label}: Fable-only binding")
    if final:
        need(not finding_refs, f"{label}: final material finding")
        need(seats[1]["outcome"] == "reviewed" and seats[1]["usable"] and seats[1]["verdict"] == "AGREE", f"{label}: Sol result")
        need(seats[3]["outcome"] == "reviewed" and seats[3]["usable"] and seats[3]["verdict"] == "AGREE", f"{label}: Fable result")

    validate_policy(receipt["effective_policy"], seats, root, forbidden, final, f"{label}.policy")
...
def validate_completion(repo, base, phase, supplied_f, root):
    ...
    return {
        ...
        "evidence_root": root.identity(metadata["evidence_root_id"]),
...
def main():
    ...
    conform_root = EvidenceRoot(os.environ["REVIEWTRUTH_CONFORM_COMPLETION_EVIDENCE_ROOT"], "CONFORM root")
    harden_root = EvidenceRoot(os.environ["REVIEWTRUTH_HARDEN_COMPLETION_EVIDENCE_ROOT"], "HARDEN root")
    try:
