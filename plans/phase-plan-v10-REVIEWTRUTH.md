...
The early-prover terminal is the exact raw final nonempty line `PROBE_STATUS: CLEAR`; bare, case-changed, whitespace-padded, quoted, or Markdown-prefixed probe-status terminals fail. A canonical `PROBE_STATUS: BLOCKED` may be retained only as standalone non-authorizing early-prover evidence; it makes the prover unusable and cannot appear in a completed review-wave receipt or restart chain, whose prior entries are complete post-`CLEAR` waves invalidated by a later material finding or contradiction.
