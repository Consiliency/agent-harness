...
The early-prover terminal is the exact raw final nonempty line `PROBE_STATUS: CLEAR`; bare, case-changed, whitespace-padded, quoted, or Markdown-prefixed probe-status terminals fail. A canonical `PROBE_STATUS: BLOCKED` remains parseable for non-authorizing retained attempts but can never make the early prover usable.
...
The native citation-marker family is detected after leading Markdown/whitespace normalization, including `+` list bullets, and case-insensitive key/colon-spacing recognition, but only the exact canonical `EVIDENCE_REF:` spelling may parse. A case-changed or colon-spaced look-alike line fails instead of remaining analytical prose.

Protected rendered-prompt marker families are detected after leading Markdown/whitespace normalization, including `+` list bullets, and case-insensitive key/colon-spacing recognition, then compared to the exact canonical case-sensitive lines. A bullet-prefixed, quoted, differently cased, or colon-spaced conflicting marker therefore fails instead of hiding beside the expected marker.
...
def validate_rendered_prompt(reference, root, *, lens, seat_instance_id, bundle_digest, required_evidence, label):
    ...
    for prefix, expected in marker_groups.items():
        ...
        actual = [line for line in normalized_lines if family.match(line)]
...
def evidence_refs_from_text(value, label):
    ...
    for index, line in enumerate(value.splitlines()):
        ...
        if EVIDENCE_REF_FAMILY.match(normalized) is None:
            continue
        need(normalized.startswith("EVIDENCE_REF:"), f"{label}: malformed evidence ref marker")
