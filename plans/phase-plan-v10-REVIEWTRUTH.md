...
`effective_policy` has exactly `gate`, `on_shortfall`, `required_consensus`, `required_lens_coverage`, `required_prover`, `required_vendors`, `resolver_input`, and `resolver_output`. The completion wave uses `gate="pre-merge-CR"` and resolves the shipped default `required_vendors=3`, `required_lens_coverage=3`, and `required_prover=true`; the common policy machinery remains parameterized for all four gates and retains a valid explicit boolean `required_prover=false` override that removes only the prover shortfall. The shipped defaults for plan-ratify, design-ratify, pre-merge-CR, and release-dispatch remain 3/3/true. Stage A audits those defaults separately, then applies and validates the typed repository override without re-imposing the default tuple after resolution. `on_shortfall=proceed_degraded` may waive only vendor/lens availability floors; `required_consensus` remains mandatory, and an effective `required_prover=true` remains hard. Stage A re-hashes both policy artifacts, re-runs their strict typed resolution using the production `required_consensus` key without a shadow mapping, and derives counted vendors, lenses, Sol AGREE, and the Fable prover fact only from the parsed and independently validated seat artifacts.
...
- **Tasks**:
  ...
  - test: SL1-T1 parameterized-policy arm — In the same immutable review-wave node, retain the shipped-default fixture proving all four gates resolve to 3/3/true, then apply typed overrides independently through the production `required_consensus` key. A literal shadow `consensus` key must fail. A `required_prover=false` fixture with all unrelated values byte-equal must remove only the Fable-prover shortfall and pass where the same fixture under `true` fails. Vendor/lens overrides and `on_shortfall=proceed_degraded` may waive only their availability floors; required consensus still fails, and `required_prover=true` still fails, independently of `on_shortfall`. No verifier may re-impose the shipped tuple after resolution.
...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, final, label):
    ...
    effective_policy = validate_policy(receipt["effective_policy"], validated_seats, root, forbidden, final, f"{label}.policy")
    if final:
