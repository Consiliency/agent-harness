...
All prompt and native-report line framing is LF-only; CR, VT, FF, record separators, U+2028, and U+2029 remain in the raw line and cannot become hidden boundaries. `PARTIALLY AGREE` contains one literal ASCII space and no whitespace normalization; control or Unicode separators between its words fail. The early-prover terminal is the exact raw final nonempty line `PROBE_STATUS: CLEAR`; bare, case-changed, whitespace-padded, quoted, Markdown-prefixed, control-separated, or Unicode-separated probe-status terminals fail. A canonical `PROBE_STATUS: BLOCKED` may be retained only as standalone non-authorizing early-prover evidence; it makes the prover unusable and cannot appear in a completed review-wave receipt or restart chain, whose prior entries are complete post-`CLEAR` waves invalidated by a later material finding or contradiction.
...
- **Tasks**:
  ...
  - test: SL1-T1 leading-markup grammar arm — In that same immutable review-wave bundle node and before parser implementation, retain literal RED mutations for `+ EVIDENCE_REF: {…}`, `+ EVIDENCE_REF : {…}`, `EVIDENCE_REF:   {…}`, `EVIDENCE_REF:\t{…}`, trailing citation whitespace, and Markdown-prefixed canonical or conflicting lens, seat-instance, output-prefix, bundle-digest, and required-evidence prompt markers. For citation and probe-terminal lines, append CR, VT (`U+000B`), FF (`U+000C`), RS (`U+001E`), U+2028, and U+2029 and prove LF-only parsing leaves each byte/code point in-line to fail; inject the same six separators between `PARTIALLY` and `AGREE` and prove no review can count. Also retain bare `CLEAR`, `+ PROBE_STATUS: CLEAR`, case-changed, whitespace-padded, quoted, and Markdown-prefixed probe terminals, plus a canonical standalone `PROBE_STATUS: BLOCKED` that must remain non-authorizing and be rejected from every completed receipt/restart entry. Each must fail as a malformed citation, noncanonical raw prompt marker, or noncanonical/non-CLEAR terminal; none may remain analytical prose or make the early prover usable.
...
The strict dependency verifier is the following exact `58030`-byte UTF-8/LF Python source at SHA-256 `5a2c54c53b5e93da9615142b215c8e17e0f5a01ec62f040ca45406c7cf095760`. The second Stage-A command extracts only these marked bytes from this plan into the private external `$REVIEWTRUTH_RUNNER_ROOT.stage-a-review-wave-verifier.py`, verifies both constants before execution, and never imports REVIEWTRUTH production code.
...
VERDICT = re.compile(r"(PARTIALLY AGREE|DISAGREE|AGREE)", re.IGNORECASE)
