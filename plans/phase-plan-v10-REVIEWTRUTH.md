...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, repo, final, label):
    ...
    if not degraded_early:
        ...
        early_required_entries = [early_execution_ref, early_native_ref, early_prompt_ref]
        ...
        need(type(source_preflight) is dict and source_preflight.get("schema") == "v10.codex-external-tool-preflight.v1", f"{label}: degraded source preflight schema")
        need(source_preflight.get("verdict") == "FAIL" and source_preflight.get("reasons") == reasons, f"{label}: degraded source preflight facts")
        preflight_started = timestamp(preflight["started_at"], f"{label}.early.degraded.preflight.started_at")
        preflight_completed = timestamp(preflight["completed_at"], f"{label}.early.degraded.preflight.completed_at")
        need(preflight_started < preflight_completed, f"{label}: degraded preflight chronology")
        ...
        need(type(policy) is dict and policy.get("schema") == "v10.codex-external-tool-policy.v1", f"{label}: degraded policy schema")
        ...
        need(type(config) is dict and config.get("schema") == "v10.codex-effective-config-receipt.v1", f"{label}: degraded effective config schema")
        ...
        need(type(source_launch) is dict and source_launch.get("schema") == "v10.codex-early-prover-launch-receipt.v1", f"{label}: degraded source launch schema")
        need(source_launch.get("launched") is False and source_launch.get("codex_process_count") == 0, f"{label}: degraded source launch facts")
        launch_started = timestamp(launch["started_at"], f"{label}.early.degraded.launch.started_at")
        launch_completed = timestamp(launch["completed_at"], f"{label}.early.degraded.launch.completed_at")
