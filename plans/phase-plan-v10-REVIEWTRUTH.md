...
`early_prover` has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `outcome`, `receipt`, `role`, `seat`, `started_at`, `usable`, and `vendor`; those facts are derived rather than accepted. Its `artifact` is strict canonical `v10.review-early-prover-artifact.v1` with exactly `attempt`, `base`, `binding_prover`, `capability`, `completed_at`, `effort`, `execution_attestation`, `harness`, `lens`, `model`, `native_result`, `outcome`, `phase_alias`, `position`, `probe_report`, `role`, `schema`, `seat`, `seat_instance_id`, `started_at`, `status`, `vendor`, and `wave_id`. Before launch, composition/auth preflight writes the distinct immutable canonical `v10.review-early-prover-execution-attestation.v1`; after launch, the worktree executor writes the distinct immutable canonical `v10.review-early-prover-native-result.v1`; only then may a separate reducer write the artifact. Stage A derives Codex identity, `role="early_prover"`, `capability="can_probe"`, and `binding_prover=false` from the preflight attestation and derives status, exact raw probe report, exact terminal `PROBE_STATUS: CLEAR`, chronology, and usability from the native result. A self-authored or opaque artifact, absent/swapped/aliased producer record, padded terminal token, embedded elision marker, or disagreement among those three records fails. Its strict canonical receipt mirrors only the derived facts plus the same phase/wave/attempt/base. `bundle_staging` has exactly `bundle`, `early_artifact`, `early_receipt`, and `staged_at`; both early references equal the `early_prover` references exactly, and `early_prover.completed_at < bundle_staging.staged_at`. The referenced bundle is itself canonical `v10.review-evidence-bundle.v1` JSON with exactly `schema`, `base`, and sorted unique `entries`; its `base` equals the receipt base and its direct file entries include the exact final-audit, final-evidence, early receipt, early artifact, early execution-attestation, and early native-result references. The bundle contains no receipt or bundle self-digest. This is the sole evidence-bearing Option-2 bundle: every reviewer receives only this by-reference bundle and read-only referenced bytes, with no arbitrary working-tree or shared-development-database execution.

`seats` is the ordered four-entry array for Grok, GPT-5.6 Sol, Gemini, and Fable. Each strict entry has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `counts_toward_floor`, `lens`, `material_findings`, `outcome`, `position`, `receipt`, `role`, `seat`, `started_at`, `usable`, `vendor`, and `verdict`. `artifact` is canonical `v10.review-seat-artifact.v1` with exactly `attempt`, `base`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `effort`, `execution_attestation`, `grounding`, `harness`, `lens`, `material_findings`, `model`, `native_result`, `outcome`, `phase_alias`, `position`, `review_text`, `role`, `schema`, `seat`, `seat_instance_id`, `started_at`, `status`, `vendor`, and `wave_id`. It is a reducer output, not the trust root. Before each launch, composition/auth preflight writes a distinct immutable canonical `v10.review-seat-execution-attestation.v1` binding the exact roster identity, model/effort/lens, role, capability, binding flag, seat instance, base, bundle, and wave. After each critic exits, `panel_invoker.invoke_board` writes a distinct immutable canonical `v10.review-seat-native-result.v1`; after the subscription-TUI native Fable report binds exactly once, `panel_invoker.bind_native_agent_leg_result` writes the same native-result schema. Only then may the reducer write the seat artifact. Stage A derives identity, role, capability, and Fable-only binding from the execution attestation; derives native status, exact raw review text, start/completion instants, bundle consumption, and a unique native-result identity from the native result; requires the artifact to mirror both; and only then derives terminal verdict, substantive material, grounding, normalized outcome, findings, and floor eligibility. The last nonempty line, after the optional exact `VERDICT:` label and formatting trim, must full-match only `AGREE`, `PARTIALLY AGREE`, or `DISAGREE`; prefixes or suffixes such as `AGREE, but not approval` fail. Any `[elided]`, `<elided>`, standalone `...`, or `elided`/`omitted` marker anywhere in the body fails even when padded past the byte floor. A zero-byte/noncanonical or self-authored artifact, absent/swapped/aliased producer record, empty/sentinel/verdict-only review, malformed terminal verdict, duplicate seat/native-result identity, ungrounded review, producer/identity/capability substitution, or bundle/text/status/chronology substitution is non-review evidence and cannot count. Each retained seat receipt then mirrors only those independently derived values plus the same phase/wave/attempt/base; Stage A rejects disagreement among producer records, artifact, seat entry, and receipt. Every counting critic and Fable consumes exactly `bundle_staging.bundle` after staging. Positions 1 through 3 are critics with `binding_prover=false`; position 4 is Fable, the only `role="binding_prover"`, the only `binding_prover=true`, and starts strictly after all three critic completion instants. The final authorizing wave has no retained contradiction or material finding, and its effective-policy facts are derived from the validated producer chain rather than asserted wrappers: Stage A recomputes the roadmap-specific typed resolver input/output, vendor/lens counts, consensus, and prover fact. GPT-5.6 Sol and Fable must each derive `outcome="reviewed"`, `usable=true`, and `verdict="AGREE"`; Fable alone supplies the prover fact.
...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, final, label):
    ...
    early_artifact_keys = {"attempt", "base", "binding_prover", "capability", "completed_at", "effort", "execution_attestation", "harness", "lens", "model", "native_result", "outcome", "phase_alias", "position", "probe_report", "role", "schema", "seat", "seat_instance_id", "started_at", "status", "vendor", "wave_id"}
    ...
    early_execution_keys = {"attempt", "attested_at", "base", "binding_prover", "capability", "effort", "harness", "lens", "model", "phase_alias", "position", "producer", "role", "schema", "seat", "seat_instance_id", "vendor", "wave_id"}
    ...
    early_native_keys = {"attempt", "base", "completed_at", "execution_attestation", "native_result_id", "phase_alias", "probe_report", "producer", "schema", "seat_instance_id", "started_at", "status", "wave_id"}
    ...
    need(early_artifact_ref["path"] != early_execution_ref["path"] != early_native_ref["path"] and early_artifact_ref["path"] != early_native_ref["path"], f"{label}: early producer artifact alias")
    need(len({early_artifact_digest, early_execution_digest, early_native_digest}) == 3, f"{label}: early producer digest alias")
    ...
    need(early_native["status"] in {"OK", "UNAVAILABLE", "ERROR", "TIMEOUT", "DEGRADED", "EMPTY"}, f"{label}: early native status")
    probe_status, probe_substantive = probe_text_facts(early_native["probe_report"], f"{label}.early.native.probe_report")
    early_usable = early_native["status"] == "OK" and probe_status is not None and probe_substantive
    ...
    for required in (receipt["base"]["final_audit"], receipt["base"]["final_evidence"], early["receipt"], early["artifact"], early_execution_ref, early_native_ref):
    ...
    agree_artifacts = []
    finding_refs = []
    artifact_keys = {"attempt", "base", "binding_prover", "capability", "completed_at", "consumed_bundle", "effort", "execution_attestation", "grounding", "harness", "lens", "material_findings", "model", "native_result", "outcome", "phase_alias", "position", "review_text", "role", "schema", "seat", "seat_instance_id", "started_at", "status", "vendor", "wave_id"}
    for index, (seat, expected) in enumerate(zip(seats, roster)):
        ...
        execution_keys = {"attempt", "attested_at", "base", "binding_prover", "capability", "consumed_bundle", "effort", "harness", "lens", "model", "phase_alias", "position", "producer", "role", "schema", "seat", "seat_instance_id", "vendor", "wave_id"}
        ...
        native_keys = {"attempt", "base", "completed_at", "consumed_bundle", "execution_attestation", "native_result_id", "phase_alias", "producer", "review_text", "schema", "seat_instance_id", "started_at", "status", "wave_id"}
        ...
        need(len({artifact_ref["path"], execution_ref["path"], native_ref["path"]}) == 3, f"{seat_label}: producer artifact alias")
        need(len({artifact_digest, execution_digest, native_digest}) == 3, f"{seat_label}: producer digest alias")
        ...
        need(native["status"] in {"OK", "UNAVAILABLE", "ERROR", "TIMEOUT", "DEGRADED", "EMPTY"}, f"{seat_label}: native status")
        ...
        terminal, substantive = review_text_facts(native["review_text"], f"{seat_label}.native.review_text")
        usable = native["status"] == "OK" and terminal is not None and substantive and grounded
