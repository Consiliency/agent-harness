...
`seats` is the ordered four-entry array for Grok, GPT-5.6 Sol, Gemini, and Fable. Each strict entry has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `counts_toward_floor`, `lens`, `material_findings`, `outcome`, `position`, `receipt`, `role`, `seat`, `started_at`, `usable`, `vendor`, and `verdict`. `artifact` is not opaque prose: it references canonical `v10.review-seat-artifact.v1` JSON produced from the completed runtime seat result, with exactly `attempt`, `base`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `effort`, `grounding`, `harness`, `lens`, `material_findings`, `model`, `outcome`, `phase_alias`, `position`, `review_text`, `role`, `schema`, `seat`, `seat_instance_id`, `started_at`, `status`, `vendor`, and `wave_id`. The artifact embeds the exact immutable raw review text. Stage A requires a unique nonempty seat-instance identity, derives the terminal verdict from the last nonempty review-text line, derives reviewed usability from normalized `status="OK"`, a substantive non-sentinel body, a conforming terminal verdict, and direct grounding in both exact final-audit and final-evidence bundle entries, and derives bundle consumption, identity, lens, role, capability, binding, chronology, outcome, findings, and floor eligibility from the parsed artifact. A zero-byte/noncanonical artifact, empty or sentinel/elided review, verdict-only response, malformed terminal verdict, ungrounded review, identity/capability substitution, or bundle substitution is non-review evidence and cannot count. Each retained seat receipt then mirrors and binds only those independently derived values plus the same phase/wave/attempt/base; Stage A rejects disagreement among artifact, seat entry, and receipt. Every counting critic and Fable consumes exactly `bundle_staging.bundle` after staging. Positions 1 through 3 are critics with `binding_prover=false`; position 4 is Fable, the only `role="binding_prover"`, the only `binding_prover=true`, and starts strictly after all three critic completion instants. The final authorizing wave has no retained contradiction or material finding, and its effective-policy facts are derived from the validated artifacts rather than asserted wrappers: Stage A recomputes the roadmap-specific typed resolver input/output, vendor/lens counts, consensus, and prover fact. GPT-5.6 Sol and Fable must each derive `outcome="reviewed"`, `usable=true`, and `verdict="AGREE"`; Fable alone supplies the prover fact.
...
The strict dependency verifier is the following exact `41244`-byte UTF-8/LF Python source at SHA-256 `9c8cde3888369ef66f1f7d04fe84e3eb6d6befab79cc311fafd2d5ee87fa3961`. The second Stage-A command extracts only these marked bytes from this plan into the private external `$REVIEWTRUTH_RUNNER_ROOT.stage-a-review-wave-verifier.py`, verifies both constants before execution, and never imports REVIEWTRUTH production code.
...
VERDICT = re.compile(r"^(PARTIALLY\s+AGREE|DISAGREE|AGREE)\b", re.IGNORECASE)
...
def review_text_facts(value, label):
    ...
    match = VERDICT.match(terminal)
    verdict = re.sub(r"\s+", " ", match.group(1).upper()) if match else None
    body = "\n".join(lines[:-1]).strip()
    sentinels = {"...", "[elided]", "<elided>", "elided", "omitted", "n/a", "no review"}
    substantive = verdict is not None and len(body.encode("utf-8")) >= 40 and body.casefold() not in sentinels
...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, final, label):
    ...
    agree_artifacts = []
    finding_refs = []
    artifact_keys = {"attempt", "base", "binding_prover", "capability", "completed_at", "consumed_bundle", "effort", "grounding", "harness", "lens", "material_findings", "model", "outcome", "phase_alias", "position", "review_text", "role", "schema", "seat", "seat_instance_id", "started_at", "status", "vendor", "wave_id"}
    for index, (seat, expected) in enumerate(zip(seats, roster)):
        ...
        seat_instance_ids.append(text(artifact["seat_instance_id"], f"{seat_label}.artifact.seat_instance_id"))
        started = timestamp(artifact["started_at"], f"{seat_label}.artifact.started_at")
        finished = timestamp(artifact["completed_at"], f"{seat_label}.artifact.completed_at")
        need(staged_at < started < finished, f"{seat_label}: artifact chronology")
        ...
        grounded = receipt["base"]["final_audit"] in grounding_refs and receipt["base"]["final_evidence"] in grounding_refs
        ...
        need(artifact["status"] in {"OK", "UNAVAILABLE", "ERROR", "TIMEOUT", "DEGRADED", "EMPTY"}, f"{seat_label}: status")
        need(artifact["outcome"] in {"reviewed", "unavailable", "errored", "timed_out", "refused", "capped", "empty"}, f"{seat_label}: outcome")
        terminal, substantive = review_text_facts(artifact["review_text"], f"{seat_label}.artifact.review_text")
        usable = artifact["status"] == "OK" and terminal is not None and substantive and grounded
        need((artifact["outcome"] == "reviewed") is usable, f"{seat_label}: derived outcome")
