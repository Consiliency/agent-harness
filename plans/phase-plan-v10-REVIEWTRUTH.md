...
- **Owned files**: `phase-loop-runtime/src/phase_loop_runtime/runner.py`, `phase-loop-runtime/src/phase_loop_runtime/governed_premerge.py`, `phase-loop-runtime/src/phase_loop_runtime/cli.py`, `phase-loop-runtime/src/phase_loop_runtime/train_runner.py`, `phase-loop-runtime/src/phase_loop_runtime/train_ledger.py`, `phase-loop-runtime/scripts/gate_a_cleanroom.sh`, `skills-src/claude/claude-advisor-board/SKILL.md`, `phase-loop-skills/advisor-board/**`, `phase-loop-runtime/src/phase_loop_runtime/skills_bundle/*-advisor-board/**`, `phase-loop-runtime/src/phase_loop_runtime/skills_bundle/*-advisor-panel/**`
...
- **Tasks**:
  ...
  - impl: SL6-T2 — Write only the owned evidence verifier and pre-final evidence docs, including the non-wrapper `gate-a` runner plus its fresh internal `gate-a-attest` verifier that consume SL-1's exact Gate-A four-module/48-nodeid collection boundary, complete 59-tuple runtime boundary, 39-tuple restricted source baseline, separate expected collection/runtime unions, 102-tuple total freeze, and SL-5's persistent tests/conftest/parser copy contract; run the real four-vendor by-reference board over those exact staged digests, record exact already-existing source/proof digests and operational traces while deferring every final XML/attestation digest to SL6-T3's post-parser finalized record, and update the canonical contract and capability card.
...
- The complete tracked phase-owned write set is the sorted 33-pattern union of SL-0 through SL-6, SHA-256 `199f22d088c71bfdb57033a7b59c1b57b4750020e26a3ea42169e0a551844c1e` over LF-joined repo-relative patterns plus terminal LF. The immutable ten-path SL-1 tests/evidence boundary uses the same framing and SHA-256 `3828b5cd50a38b7665d6520ba437e410ac77a34f15488ce035fb1b6febfdeb6f`. The current repair adds no SL-1 path, but adds exactly one literal node in already-owned `phase-loop-runtime/tests/test_reviewtruth_phase.py`: `phase-loop-runtime/tests/test_reviewtruth_phase.py::test_gate_a_persists_independently_attestable_tests_conftest_and_parser_copy`; its singleton sorted-LF digest is `c96415d6e2c94d3d2f4a42eeaf38a34572a2e88af2e743711f6b97751767071d`. `phase-loop-runtime/scripts/gate_a_cleanroom.sh` is the one newly owned implementation path and participates in the 33-pattern count/digest.
