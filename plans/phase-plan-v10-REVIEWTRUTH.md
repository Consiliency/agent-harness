...
Read-only GitHub metadata rechecked on 2026-07-31 establishes that the human decision requested by `agent-harness#398` is now satisfied by maintainer comment `5139465317` at `https://github.com/Consiliency/agent-harness/issues/398#issuecomment-5139465317`. The exact UTF-8 body is `2479` bytes at SHA-256 `a2dc69639b89743ba351bebf4cd46e81ef8d97901bc5545c00f4777f737238a7`; author `ViperJuice` has repository permission `admin` and author association `MEMBER`; and `.created_at` and `.updated_at` are both `2026-07-31T05:04:41Z`. The comment expressly identifies itself as the maintainer decision rather than the coordinator recommendation and ratifies Option 2 evidence staging: codex/gemini/grok vendor review legs stay read-only over the exact by-reference bundle, the prover stages redacted digest-bound evidence, arbitrary real-tree and shared-development-database execution stays forbidden, and the byte-frozen `PANEL_LEGS` tuple and non-review goldens do not shift. Open `agent-harness#405` is a separate, non-authorizing follow-on pilot with database isolation as a prerequisite, codex first, gemini only after demonstrated value, and grok excluded; it neither changes Option 2 nor supplies ratification, and it cannot widen `PANEL_LEGS`.
...
This static re-anchoring occurs before REVIEWTRUTH's still-incomplete SL-0 durable-record,
effective-rule, and merge-topology gate, so that gate cannot block CONFORM or HARDEN; the human
decision itself is already satisfied, while the complete executable provision/paired-observe chain
remains correctly downstream of completed SL-0.
...
- [ ] PC-REVIEWTRUTH-0 — Both already-issued `agent-harness#398` human decisions are bound in `docs/research/reviewtruth-leg-capability-ratification.md`, merged separately before the tests-only landing, and binding on every later posture-assuming change. General roadmap execution authorization, panel approval, and the separate `agent-harness#405` pilot are non-substitutes.

  The record contains exactly one `REVIEWTRUTH_CAPABILITY_RATIFICATION_JSON` envelope with the strict keys `schema`, `issue`, `disposition_id`, `disposition_summary`, `option_2_posture`, `real_tree_arbitrary_execution`, `ratification_comment_id`, `ratification_comment_sha256`, `ratifier_login`, `follow_up_pilot`, and `prover_directive`. The existing Option-2 fields and `follow_up_pilot` object remain byte-semantically unchanged. `prover_directive` has exactly: `issue="Consiliency/agent-harness#398"`; comment ID `5139609713`; node ID `IC_kwDOTFEWvM8AAAABMlg4cQ`; exact URL; body bytes `2697`; body SHA-256 `284e37117787f653ae91cebc2c04454ddb54ef0cc6434e26dc08b0875cddccfc`; ratifier `ViperJuice`; association `MEMBER`; both timestamps `2026-07-31T05:29:20Z`; `required_prover_all_gates=true`; `required_vendors_all_gates=3`; `required_lens_coverage_all_gates=3`; `plan_design_on_shortfall="proceed_degraded"`; `merge_release_on_shortfall="escalate"`; `missing_prover_action="escalate_nonwaivable"`; `current_prover_seat={"harness":"claude","model":"claude-fable-5","lens":"correctness"}`; `already_bound_verdicts="stand"`; `unbound_verdicts="floor3_plus_usable_fable"`; `runtime_gap_enforcement="procedural"`; `root_panel_binding="grok_sol_gemini_fable_usable_agree"`; `direct_claude_p=false`; `option_2_unchanged=true`; and `follow_up_pilot_authorizing=false`. Unknown/missing/extra keys or values fail.

  SL0-T3 uses only existing tools plus the merged record. It proves both exact server comment identities, issue/comment URLs, byte counts/digests, ratifier, `MEMBER` association, fixed creation/update timestamps, current `admin` permission, and all semantic clauses; requires all four timestamps no later than the record PR merge; proves the strict Option-2, non-authorizing-pilot, and prover-directive objects; and separately retains the effective-rule, canonical-rules-digest, record-only PR/diff, two-parent/PR-head, fetched-main reachability, and no-posture-implementation checks. Its canonical proof emits both comment identities, all three object digests, rules digest/boolean, and topology. Only zero exit completes SL-0. After SL-1 lands, chronology independently repeats those checks and compares the retained object/rules identities; neither a future wrapper nor `agent-harness#405` can authorize backward.
...
- **Scope**: Bind the exact already-issued `agent-harness#398` Option-2 maintainer decision and its binding execution-capability/prover follow-on directive, land one durable disposition on the target, and prove the independent protection and landing conditions before any posture-assuming implementation begins.
...
- **Tasks**:
  - test: SL0-T1 — Define disposition falsifiers for read-only inspection, throwaway-copy probes, execution-capability/prover policy, and narrower variants while preserving the real-tree arbitrary-execution prohibition; freeze the strict two-comment record envelope and every rejection arm of the existing-tool coordinator proof before the record PR lands.
  - impl: SL0-T2 — Inspect effective target rules, obtain the required effective `pull_request` rule plus separate two-parent landing posture, bind exact `agent-harness#398` comments `5139465317` and `5139609713` with their immutable body digests, identities, timestamps, Option-2/prover semantics, parameterized-policy/default posture, and current-v10 procedural self-application into the strict record, bind `agent-harness#405` only as the non-authorizing pilot, and merge the standalone metadata-only disposition record.
  - verify: SL0-T3 — Run the exact coordinator proof block in `## Verification`; complete SL-0 only when it proves both directive comments were created and last edited no later than merge, current effective PR-only protection and its canonical digest, maintainer permission, dedicated server-recorded merged PR, separate two-parent landing, fetched canonical-main reachability, unchanged record, and record-only PR/diff with no posture-assuming implementation.

| Task ID | Type | Depends on | Files in scope | Tests owned | Test command | Work |
|---|---|---|---|---|---|---|
| SL0-T1 | test | (none) | `docs/research/reviewtruth-leg-capability-ratification.md` | disposition and coordinator-proof falsifiers | exact `REVIEWTRUTH_SL0_COORDINATOR_PROOF` block in `## Verification` after SL0-T2 | Write alternatives and falsifiers for read-only artifact inspection, bounded probes against a throwaway staged copy, execution-capability attestation, parameterized prover policy, and any narrower ratified variant; preserve the invariant that no vendor seat receives arbitrary execution against the real tree. Freeze the exact envelope keys and reject either comment's wrong issue/schema/disposition/comment/node/URL/body bytes/body digest/timestamps/login/association/permission, a missing or altered Option-2/prover semantic clause, fixed-policy hardcoding disguised as a default, any posture-object drift, any `agent-harness#405` authority/ratification/`PANEL_LEGS` widening claim, absent or insufficient permission, either comment's creation or last edit after merge, absent/malformed effective rules, no effective `pull_request` rule, branch-rules digest drift, shallow/grafted/replaced history, missing/duplicate/non-main PR, squash/rebase/direct push, non-two-parent landing, PR-head mismatch, record drift, extra PR/diff path, and unreachable record commit. |
| SL0-T2 | impl | SL0-T1 | `docs/research/reviewtruth-leg-capability-ratification.md` | none | exact `REVIEWTRUTH_SL0_COORDINATOR_PROOF` block in `## Verification` after merge | Inspect effective `main` rules with read-only `gh` metadata and obtain an effective `pull_request` rule through the approved admin path; this is the PR-only target-protection requirement and is distinct from the Git proof of an actual two-parent landing. Bind both existing exact maintainer comment IDs/body SHA-256 values/timestamps/ratifier, the disposition ID, strict Option-2 posture, execution-capability/prover directive, parameterized-policy/default distinction, current-v10 procedural self-application, and non-authorizing `agent-harness#405` follow-up object in the record; do not require or manufacture a `REVIEWTRUTH-RATIFY-V1` token. Merge only this durable record in its own PR. The record cannot self-claim its future PR, merge commit, or run-time rules digest; SL0-T3 derives them from fetched Git/GitHub metadata. `agent-harness#400`, blanket roadmap execution approval, plan review, ordinary issue discussion, and `agent-harness#405` do not satisfy this task. |
| SL0-T3 | verify | SL0-T2 | `docs/research/reviewtruth-leg-capability-ratification.md` | none | exact `REVIEWTRUTH_SL0_COORDINATOR_PROOF` block in `## Verification` | Run the command from a full clone with authenticated read-only `gh` access. Its zero exit and canonical JSON stdout prove both stated decisions, exact maintainer ratification with all four comment timestamps no later than merge, effective PR-only branch protection and its canonical response digest, dedicated server-recorded merged PR, separate two-parent record landing, fetched canonical-main reachability, unchanged record, and no posture-assuming implementation because both server PR files and first-parent landing diff equal the record singleton. That proof completes SL-0 and unblocks SL-1 without any SL-1 file, wrapper, parser, provisioned environment, baseline observation, or manual exception. Retain the exact stdout/exit metadata, including both comment bindings and branch-rules digest/boolean, in the coordinator ledger for later independent comparison. |

### SL-1 — Tests-First Falsifier Lane And RED Baseline

- **Scope**: After SL-0 independently completes, use the exact clean ratified base that already contains the separately committed planning lock prerequisite to execute the exact-digest external bootstrap observer in one controlled pytest process before mutation; then land the complete REVIEWTRUTH falsifier and compatibility migration suite, including the literal three-node EC-REVIEWTRUTH-16 capability/policy/durable-self-application RED contract, deterministic marker activation, CI-safe dual-mode post-parser collection, the strict typed degraded-seat append/read/rewrite/reread contract, the exact-node PyPI and source-layout console-script availability dispositions and RED falsifiers, non-colliding native request/attempt identity tests, structured JUnit contract, retrospective two-comment SL-0 chronology verification, and panel-reviewed RED record before any production file changes.
...
- **Owned files**: `phase-loop-runtime/src/phase_loop_runtime/panel_invoker.py`, `phase-loop-runtime/src/phase_loop_runtime/launcher.py`, `phase-loop-runtime/src/phase_loop_runtime/advisor_board/schema.py`, `phase-loop-runtime/src/phase_loop_runtime/advisor_board/composition.py`, `phase-loop-runtime/src/phase_loop_runtime/fab_gate.py`
- **Interfaces provided**: `IF-0-REVIEWTRUTH-1`, `REVIEWTRUTH-prover-capability-attestation`, `AuthPreflightResult.metadata.execution_capability`, `Board.capability_attestations`, `PanelLegOutcome`, `PanelLegResult.outcome`, `PanelLegResult.required`, `PanelLegResult.degraded`, `PanelLegResult.prompt_lens_digest`, `PanelResult.reviewed_seat_count`, `SeatOutcomeRecord.degraded`, `REVIEWTRUTH-seat-outcome-degraded-lossless-reader`, `NativeAgentLegRequest.request_id`, `NativeAgentLegRequest.seat_instance_id`, `NativeAgentLegRequest.attempt_id`, `NativeAgentLegReport`, `NativeAgentLegBindingDisposition`, `bind_native_agent_leg_result()`
...
- SL0-T3's existing-tool proof is the only gate that completes SL-0 and unblocks SL-1. The exact `agent-harness#398` human decision is already satisfied, but it cannot substitute for the separately proved effective PR-only target protection from the canonical branch-rules response, actual two-parent record landing topology from Git/PR metadata, record-only contents, and unedited-before-merge ratification from both fixed comment timestamps; none substitutes for another, and `agent-harness#405` supplies no authority. The post-SL-0 provision/dual-materialize/freeze/source-observe/Gate-A-observe operations then run before any SL-1 mutation and seal both evidence directories plus the independent-constant comparison; no broad-baseline parse is claimed until the parser is authored within SL-1. `reviewtruth_preimplementation_chronology_all` freshly recomputes the comment identity and semantics, strict posture/follow-up objects and digests, rules digest/boolean, and all other SL-0 facts, requires equality to the retained proof, and is the only chronology gate that unblocks SL-2 through SL-5. Neither verifier delegates authority backward to a future lane, and neither has a manual bypass.
...
SL0-T3 runs the following exact `REVIEWTRUTH_SL0_COORDINATOR_PROOF` block from a full clone with authenticated read-only `gh` access. The ratification record must contain exactly one JSON object between literal `<!-- REVIEWTRUTH_CAPABILITY_RATIFICATION_JSON_BEGIN -->` and `<!-- REVIEWTRUTH_CAPABILITY_RATIFICATION_JSON_END -->` lines; no future verifier or wrapper is used. The command treats the fixed server comment identity and exact body as the human-decision input, never asks the immutable comment to contain a plan-invented token, fetches canonical `main`, derives the record landing and PR instead of trusting self-reported commit metadata, fails on every PC-REVIEWTRUTH-0 drift arm, and prints one canonical proof object on success. The current human decision alone is insufficient: a nonzero exit, including the currently absent effective `pull_request` rule or missing record landing, keeps SL-0 incomplete; only a zero exit completes SL-0 and unblocks SL-1.
...
Every criterion inherits the independently executable SL0-T3 coordinator proof as a chronology prerequisite, including the fixed `agent-harness#398` comment identity/body/semantic clauses, the exact Option-2 and non-authorizing `agent-harness#405` objects, both ratification timestamps no later than merge, an effective `pull_request` branch rule and canonical response digest, and separate two-parent record landing evidence. The human decision by itself and the pilot issue each remain insufficient. The final EC-REVIEWTRUTH-15/16 pytest commands are retrospective immutable tests of the same facts after SL-1 exists; neither is the authority that completes SL-0 or starts SL-1. Before any SL-2-through-SL-5 mutation, `reviewtruth_preimplementation_chronology_all` must independently re-run those Git/GitHub checks, require fresh posture/follow-up and rules digest equality to the retained proof, and bind the result to the tests-only landing. No wrapper, future parser, baseline parse, or manual inspection can substitute for either ordered proof.
