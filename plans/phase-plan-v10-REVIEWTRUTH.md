...
### 2026-08-19 Universal native-first routing amendment (authoritative)

`Consiliency/agent-harness#396`, its universal-routing maintainer comment
`https://github.com/Consiliency/agent-harness/issues/396#issuecomment-5308819363`, and its governance
correction `https://github.com/Consiliency/agent-harness/issues/396#issuecomment-5308839625` amend this
plan after `Consiliency/agent-harness#545` merged to canonical `main`. This section supersedes only older wording that
treats native fulfillment as a Claude/Fable exception or permits an own-vendor adapter to be the
normal path. It does not rewrite completed historical evidence, change `PANEL_LEGS`, confer
`binding_prover` capability on a native seat, alter EC-REVIEWTRUTH-16 through
EC-REVIEWTRUTH-19, or weaken any grounding, evidence, repair, quiescence, lifecycle, or release
gate below.

The normative routing matrix is host-relative and universal:

| Current harness | Native vendor family | Required routing | Other vendor families |
|---|---|---|---|
| Codex | Codex/OpenAI | native Codex subagent rendezvous | existing adapters |
| Claude Code | Claude/Anthropic | native Claude subagent rendezvous | existing adapters |
| Antigravity/Gemini | Gemini/Google | native Gemini subagent rendezvous | existing adapters |
| Grok | Grok/xAI | native Grok subagent rendezvous | existing adapters |
| standalone/no native capability | none | no native route | all adapters; topology explicitly records standalone mode |

`HostContext` is an explicit input, not an ambient inference. It carries a closed supported
`host_harness` identity and an explicit `native_agent_capable` fact. The ordinary phase-loop CLI
entrypoint supplies standalone/no-native context unless a supported harness integration provides
the typed native capability. A supported host identity with native capability absent or false is
a configuration failure for a matching vendor-family seat; it may not silently fall back to that
host's TUI, CLI, SDK, API, gateway, or other external adapter. `identify_native_host_seats()`
returns every matching seat instance in board order, including duplicate same-family seats;
`identify_host_leg()` may remain only as a compatibility projection and is not routing authority.
Nonmatching seats continue through their existing adapters without semantic change.

The runtime prepares native work but does not pretend it can invoke a host's private agent API.
For each matching seat it emits one strict `NativeAgentLegRequest` rendezvous record containing at
least `board_run_id`, stable unique `seat_instance_id`, fresh unique `request_id` and `attempt_id`,
`host_harness`, `host_vendor_family`, `seat_vendor_family`, seat/model/effort/lens/mode/verdict
contract, artifact and brief identities, and exact artifact/brief/prompt/lens digests. The host
integration spawns its native subagent and returns one `NativeAgentLegReport` echoing that complete
identity plus the terminal outcome and payload metadata. Binding is exact-once and attempt-local:
missing, malformed, duplicate, late, replayed, superseded, cross-board, cross-seat, wrong-host,
wrong-vendor, wrong-model, wrong-effort, wrong-lens, or digest-mismatched reports are typed
rejections, are never delivered, and cannot increment raw or grounded review counts. A native
result crosses the same capture, process-quiescence, stream-publication, grounding, repair,
finalization, evidence, and lifecycle gates as an adapter result.

This amendment adds plan-local `PC-REVIEWTRUTH-11`. SL-1 owns six literal RED falsifiers in the
already-owned `phase-loop-runtime/tests/test_reviewtruth_phase.py`; SL-2 owns the typed
`HostContext`, all-matching-seat selector, request/report schema, dispatch classification, and
binding semantics in its existing six files; SL-5 owns the host-facing typed rendezvous drivers,
phase-loop CLI standalone projection, skill installation/inventory paths, neutral source skill,
and every generated harness skill mirror. The host integrations must call the typed prepare/bind
surface instead of recursively shelling out a complete external four-seat board. Fable remains the
only binding prover under the existing capability policy; native routing determines transport,
not capability. The older Opus/Fable topology is therefore read host-relatively: under Claude
Code its Claude-family seats are native, while under other hosts their existing subscription-TUI
adapter remains a non-native leg.

No implementation lane may dispatch on this amendment until its exact plan and manifest successor
have an identity-bound native Codex review plus non-native Claude/Fable, Gemini, and Grok reviews,
all against the same bundle digest and all terminal `AGREE`. The native Codex seat is the default
Codex-host route, not an external Codex CLI fallback. Any finding changes the candidate digest and
invalidates every predecessor approval.

The 2026-08-04 maintainer ratification on `agent-harness#442` adds
...
EC-REVIEWTRUTH-16 through EC-REVIEWTRUTH-19 are the sole operational authority for every plan,
bootstrap, test, implementation-review, restart, and closeout board in this document. Any earlier
forward-looking roster, order, chair taxonomy, mandatory-seat list, or round limit is superseded
and non-operational; descriptions of completed predecessor panels remain historical evidence only.

REVIEWTRUTH is explicitly selected for this run. Canonical `.phase-loop/state.json` and `.phase-loop/tui-handoff.md` now agree that REVIEWTRUTH is the current `planned` phase, and the newest canonical ledger event records the GPT-5.6 Sol planning run at maximum effort. The cumulative root-plan candidate owns the EC-REVIEWTRUTH-17 roadmap reseal, semantic repairs to REVIEWTRUTH plus its CONFORM/HARDEN producer contracts, the corresponding three manifest rows, and the already sealed tracked `phase-loop-runtime/uv.lock` bootstrap prerequisite. This coordination repair also binds SCHED before REVIEWTRUTH because both phases own `phase_worktree_executor.py`, `runner.py`, and `test_phase_worktree_executor.py`; REVIEWTRUTH consumes SCHED's landed lifecycle implementation and extends it rather than acting as a concurrent writer. The roadmap and all root-plan bindings are resealed mechanically after that dependency correction. Legacy `.codex/phase-loop/` state is compatibility-only and is not authoritative.

The phase replaces the board's text-derived usability shortcut with a typed per-seat outcome, distinguishes FULL, FLOOR-ONLY, and BELOW-FLOOR delivery, makes lens and artifact grounding load-bearing, attests and persists the bounded Fable binding-prover capability from preflight through policy facts, routes every host-vendor seat through its native subagent by default while retaining adapters only for non-native or explicit standalone legs, persists per-seat and aggregate governed outcomes, fails closed on empty or elided material or a missing effectively-required prover, connects the production repair round, and adds the separately bounded early-prover evidence path required by maintainer comment `5139955591`. It does not implement LEGLIFE timeout enforcement or custom per-repo seats; it only freezes and consumes the `timed_out` outcome that LEGLIFE later produces. It never grants live-tree/shared-data authority, never equates native transport or early Codex/Grok `can_probe` with Fable `binding_prover`, and never alters `PANEL_LEGS` or non-review goldens.
...
This cumulative coordinated repair updates the roadmap's EC-REVIEWTRUTH-17 contract from the superseded 13-node set to the exact 16-node set already required here, corrects the confined-Grok actor and SCHED writer ordering, then reseals all six root plans' current roadmap bindings. REVIEWTRUTH receives the matching semantic plan changes; CONFORM and HARDEN retain their prior producer-contract semantics and receive only the current roadmap/plan seal rollover here. The lock bytes remain unchanged. Historical review and predecessor digests remain labeled as such. This branch binds the repaired producer/consumer dependency-completion contract and corrected roadmap input, and neither authorizes lane dispatch before the required Stage-B freeze and ordered review.
...
`receipt` has exactly `attempt`, `base`, `bundle_staging`, `completed_at`, `early_prover`, `effective_policy`, `evidence_root_id`, `gate`, `phase_alias`, `restart_chain`, `seats`, and `wave_id`. Every time is fixed-width UTC RFC3339 with six fractional digits and `Z`; comparison is chronological over parsed instants, not lexical trust. `base` has exactly `commit`, `tree`, `plan_sha256`, `roadmap_sha256`, `final_audit`, and `final_evidence`; the two artifact values and every other retained file reference have exactly `path`, `bytes`, and `sha256`. A path is a normalized immutable relative locator: nonempty UTF-8, no absolute form, backslash, empty segment, `.`/`..` segment, or symlink at any ancestor. It resolves beneath the explicit coordinator-supplied canonical evidence root for the matching `evidence_root_id`; the resolved regular file must be owned by the coordinator identity, not group/other-writable, and match both integer byte count and recomputed lowercase SHA-256. `final_audit` and `final_evidence` must each have a positive declared and recomputed byte count; zero-byte final grounding fails even though unrelated retained refs may legitimately be empty.

`early_prover` has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `outcome`, `receipt`, `role`, `seat`, `started_at`, `usable`, and `vendor`; those facts are derived rather than accepted. The pre-REVIEWTRUTH Stage-A dependency bootstrap accepts exactly one of two non-overlapping trust paths: launched Codex after a passing Codex preflight, or degraded zero-launch after failed Codex preflight and unavailable Grok confinement. REVIEWTRUTH implementation adds the already-roadmapped third runtime selection branch: failed Codex preflight followed by a positively OS-confined Grok `can_probe` launch. Its strict receipt shape is frozen by SL-1's two-arm Grok test before SL-2 implements it; it is not invented during this planning repair or required as a predecessor completion-event shape.

In the launched path, `artifact.schema="v10.review-early-prover-artifact.v1"`. The artifact has exactly `attempt`, `base`, `binding_prover`, `capability`, `codex_launch_receipt`, `codex_preflight_receipt`, `completed_at`, `effort`, `execution_attestation`, `grounding`, `harness`, `lens`, `model`, `native_result`, `outcome`, `phase_alias`, `position`, `probe_report`, `rendered_prompt`, `role`, `schema`, `seat`, `seat_instance_id`, `started_at`, `status`, `vendor`, and `wave_id`. Its strict canonical `v10.review-early-prover-launched-preflight.v1` record has exactly `agent_launch_count`, `attempt`, `base`, `completed_at`, `effective_config`, `failure_reasons`, `phase_alias`, `policy`, `producer`, `schema`, `source_preflight`, `source_prelaunch_recheck`, `started_at`, `verdict`, and `wave_id`; it binds the wave, has `producer="phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight"`, `verdict="PASS"`, `failure_reasons=[]`, and `agent_launch_count=0`. The retained `v10.codex-external-tool-policy.v1` has exactly `created_at`, `external_feature_denylist`, `network_access`, `producer`, `sandbox`, `schema`, and `workspace_write`; its producer is `phase_loop_runtime.panel_invoker.build_codex_external_tool_policy`, `sandbox="workspace-write"`, `network_access=false`, the denylist is nonempty/sorted/unique, and `workspace_write` has exactly `exclude_slash_tmp=true`, `exclude_tmpdir_env_var=true`, and `writable_roots=[]`. The retained `v10.codex-effective-config-receipt.v1` has exactly `external_feature_denylist`, `mcp_enabled_count`, `network_access`, `plugin_enabled_count`, `policy`, `producer`, `recorded_at`, `sandbox`, `schema`, and `workspace_write`; its producer is `phase_loop_runtime.panel_invoker.read_codex_effective_config`, it references that policy, mirrors its sandbox/network/workspace-write/denylist facts exactly, and records both enabled counts as zero. The distinct full preflight and immediate prelaunch recheck each have schema `v10.codex-external-tool-preflight.v1` and exactly `agent_launch_count`, `completed_at`, `effective_config`, `failure_reasons`, `policy`, `producer`, `schema`, `started_at`, `state_sha256`, and `verdict`; each has producer `phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight`, references those exact policy/config records, has `verdict="PASS"`, `failure_reasons=[]`, `agent_launch_count=0`, and the same lowercase state SHA-256, with the recheck strictly after the first preflight and before launch.

The launched artifact's strict canonical `v10.review-early-prover-launched-launch.v1` record has exactly `attempt`, `base`, `codex_process_count`, `completed_at`, `data_scope`, `launched`, `phase_alias`, `preflight`, `producer`, `reaper`, `schema`, `source_launch`, `started_at`, `workspace_mount`, `worktree`, and `wave_id`; it binds the passing preflight above, has `producer="phase_loop_runtime.phase_worktree_executor.bind_launch_evidence"`, `launched=true`, and `codex_process_count=1`. Its full `v10.codex-early-prover-launch-receipt.v1` source has exactly `codex_process_count`, `completed_at`, `data_scope`, `launched`, `preflight`, `producer`, `reaper`, `sandbox`, `schema`, `started_at`, `workspace_mount`, and `worktree`, has producer `phase_loop_runtime.phase_worktree_executor`, carries the same references/times/count, and has `sandbox="workspace-write"`. The referenced `v10.codex-workspace-mount-receipt.v1` has exactly `attempt`, `base`, `canonical_path`, `checked_at`, `device`, `mode`, `owner_uid`, `phase_alias`, `producer`, `safe`, `schema`, and `wave_id`; it is producer-owned by `phase_loop_runtime.phase_worktree_executor.preflight_workspace_mount`, has a canonical absolute path, nonnegative integer device/owner, private mode, and `safe=true`. The referenced `v10.codex-ephemeral-worktree-receipt.v1` has exactly `attempt`, `base`, `cleanup_owner`, `created_at`, `detached`, `ephemeral`, `head`, `mount`, `path`, `phase_alias`, `producer`, `schema`, `serial_holder`, `tree`, and `wave_id`; it is producer-owned by `phase_loop_runtime.phase_worktree_executor.create_worktree`, binds the safe mount, has a canonical absolute path strictly beneath that mount, binds the receipt base commit/tree, and has `detached=true`, `ephemeral=true`, `serial_holder=true`, and `cleanup_owner="external_coordinator_reaper"`. The referenced `v10.codex-data-scope-receipt.v1` has exactly `attempt`, `base`, `created_at`, `database_policy`, `path`, `phase_alias`, `producer`, `schema`, `shared_database_access`, `wave_id`, and `worktree`; it is producer-owned by `phase_loop_runtime.phase_worktree_executor.create_data_scope`, binds the worktree, has a canonical absolute path strictly beneath it, `database_policy="no_shared_database_access_environment_scrubbed"`, and `shared_database_access=false`. The referenced `v10.codex-external-reaper-receipt.v1` has exactly `attempt`, `base`, `cleanup_status`, `completed_at`, `external`, `leg_cleanup_authority`, `phase_alias`, `producer`, `registered_at`, `schema`, `wave_id`, and `worktree`; it is producer-owned by `phase_loop_runtime.phase_worktree_reaper`, binds the worktree, has `external=true`, `leg_cleanup_authority=false`, and `cleanup_status="removed"`, and proves registration before process start and cleanup after process completion. All operational records are distinct immutable files under the same evidence root; source/projection disagreement, aliasing, extra writable authority, either missing temporary exclusion, nonzero enabled MCP/plugin counts, wrong base/head/tree/path containment, shared database access, non-serial launch, leg-owned cleanup, failed cleanup, or process count other than one fails.

Before launch, composition/auth preflight writes the distinct immutable canonical `v10.review-early-prover-execution-attestation.v1` and binds a distinct immutable rendered-prompt reference; after launch, the worktree executor writes the distinct immutable canonical `v10.review-early-prover-native-result.v1`; only after the operational launch and reaper records exist may a separate reducer write the artifact. The prompt must contain exactly one `REVIEW_LENS: live-probe`, seat-instance marker, output-citation grammar marker, and canonical `REQUIRED_EVIDENCE_REF` marker for each final artifact, with no extra or conflicting marker in those families. Stage A derives Codex identity, `role="early_prover"`, `capability="can_probe"`, and `binding_prover=false` from the execution attestation; derives bounded writable-launch eligibility from the passing external-tool and isolated-worktree producer chain; derives status, exact raw probe report, chronology, and grounding from the native result; and requires the report itself to contain canonical `EVIDENCE_REF` lines exactly equal to both final-artifact references. Citation lines do not count toward the substantive-content floor. Only a substantive non-elided successful report ending with exact `PROBE_STATUS: CLEAR`, backed by that complete producer chain, is usable and may enter a completed receipt. A launched `BLOCKED`, invalid, failed, ungrounded, or operationally unproved result remains non-authorizing and requires repair or a fresh attempt; it is not the degraded no-launch exception.

In the sole no-launch exception, `artifact.schema="v10.review-early-prover-degraded-artifact.v1"` and the artifact has exactly `attempt`, `base`, `binding_prover`, `capability`, `codex_launch_receipt`, `codex_preflight_receipt`, `completed_at`, `grok_fallback_decision`, `outcome`, `phase_alias`, `position`, `role`, `schema`, `seat`, `started_at`, `usable`, `vendor`, and `wave_id`. Its distinct strict canonical `v10.review-early-prover-degraded-preflight.v1` receipt has exactly `agent_launch_count`, `attempt`, `base`, `completed_at`, `effective_config`, `failure_reasons`, `phase_alias`, `policy`, `producer`, `schema`, `source_preflight`, `started_at`, `verdict`, and `wave_id`; `producer="phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight"`, `verdict="FAIL"`, `failure_reasons` is a nonempty sorted unique string array, and `agent_launch_count=0`. The referenced policy uses the exact launched-policy schema and desired closed-surface values. The failed effective-config receipt has exactly `failure_reasons`, `producer`, `recorded_at`, `schema`, and `status`, with producer `phase_loop_runtime.panel_invoker.read_codex_effective_config`, `status="FAIL"`, and reasons exactly equal to the compact preflight. The full `v10.codex-external-tool-preflight.v1` record has exactly `agent_launch_count`, `completed_at`, `effective_config`, `failure_reasons`, `policy`, `producer`, `schema`, `started_at`, `state_sha256`, and `verdict`; it binds those policy/config refs, has the canonical preflight producer, independently carries the same failed verdict/reasons and zero launch count, and supplies the compact interval. Its distinct strict canonical `v10.review-early-prover-degraded-launch.v1` receipt has exactly `attempt`, `base`, `codex_process_count`, `completed_at`, `launched`, `phase_alias`, `preflight`, `producer`, `schema`, `source_launch`, `started_at`, and `wave_id`; it binds the failed wave preflight and the retained full `v10.codex-early-prover-launch-receipt.v1`, has `producer="phase_loop_runtime.phase_worktree_executor"`, `launched=false`, and `codex_process_count=0`. The full launch receipt has exactly `codex_process_count`, `completed_at`, `launched`, `preflight`, `producer`, `schema`, and `started_at`, binds the compact preflight, has the canonical executor producer, independently carries the same zero-launch facts, and supplies the compact interval. Its distinct strict canonical Grok fallback decision has exactly `attempt`, `base`, `codex_launch_receipt`, `codex_preflight_receipt`, `decided_at`, `grok_process_count`, `launched`, `os_confinement_available`, `phase_alias`, `producer`, `reason`, `schema`, and `wave_id`; it binds both compact Codex records, has `producer="phase_loop_runtime.panel_invoker.invoke_review_wave"`, `os_confinement_available=false`, `launched=false`, `grok_process_count=0`, and `reason="os_confinement_unavailable"`. Stage A derives `outcome="DEGRADED_NO_LAUNCH"`, `usable=false`, Codex `can_probe` identity, and chronology only from those compact envelopes plus their full operational sources and rejects a passing/empty-reason preflight, any process count, a launched flag, available confinement, producer/ref/base mismatch, source/projection disagreement, or a reducer-authored substitute. This branch records degraded evidence only; it never supplies `binding_prover`, waives no effective vendor/lens/consensus/prover rule, and is legal because addendum `5139955591` expressly permits the wave to proceed when Codex fails preflight and confined Grok is unavailable.

Both Stage-A bootstrap paths use one strict canonical early receipt that mirrors only independently derived facts plus the same phase/wave/attempt/base. `bundle_staging` has exactly `bundle`, `early_artifact`, `early_receipt`, and `staged_at`; both early references equal the `early_prover` references exactly, and the selected path's final producer action completes before `bundle_staging.staged_at`. The referenced bundle is itself canonical `v10.review-evidence-bundle.v1` JSON with exactly `schema`, `base`, and sorted unique `entries`; its `base` equals the receipt base and its direct file entries include exact final-audit, final-evidence, early receipt, early artifact, and all producer records required by the selected path. A launched Codex path includes its policy, effective-config, compact passing preflight, full passing preflight and recheck, compact and full one-process launch, workspace-mount, ephemeral-worktree, isolated-data-scope, external-reaper, execution-attestation, rendered-prompt, and native-result records. A degraded path includes its policy, effective-config, full and compact failed-preflight records, full and compact zero-launch records, and no-confinement Grok decision and contains no early rendered prompt or native result. The launched bundle may contain other independently retained audit evidence, but every listed launched record is mandatory; the degraded bundle is exactly its listed producer set. The bundle contains no receipt or bundle self-digest. Every critic and Fable starts only after this exact Option-2 bundle is staged and consumes its exact digest; on the degraded branch that staged audit is the addendum's sole exception to probe evidence. No reviewer receives arbitrary working-tree or shared-development-database execution, and Fable remains the only possible binding prover on either bootstrap path or the later confined-Grok runtime path.

`seats` is the ordered four-entry array for Grok, GPT-5.6 Sol, Gemini, and Fable. Each strict entry has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `counts_toward_floor`, `lens`, `material_findings`, `outcome`, `position`, `receipt`, `role`, `seat`, `started_at`, `usable`, `vendor`, and `verdict`. `artifact` is canonical `v10.review-seat-artifact.v1` with exactly `attempt`, `base`, `binding_prover`, `capability`, `completed_at`, `consumed_bundle`, `effort`, `execution_attestation`, `grounding`, `harness`, `lens`, `material_findings`, `model`, `native_result`, `outcome`, `phase_alias`, `position`, `rendered_prompt`, `review_text`, `role`, `schema`, `seat`, `seat_instance_id`, `started_at`, `status`, `vendor`, and `wave_id`. It is a reducer output, not the trust root. Before each launch, composition/auth preflight writes a distinct immutable canonical `v10.review-seat-execution-attestation.v1` binding the exact roster identity, model/effort/lens, role, capability, binding flag, seat instance, base, bundle, wave, and distinct immutable rendered prompt. That prompt must contain exactly one marker each for the attested lens, seat instance, exact bundle digest, output citation grammar, and both canonical required final-artifact references, with no extra or conflicting marker in those families. After each critic exits, `panel_invoker.invoke_board` writes a distinct immutable canonical `v10.review-seat-native-result.v1`; after the host-relative native-or-subscription-TUI Fable report binds exactly once, `panel_invoker.bind_native_agent_leg_result` writes the same native-result schema. Each native result binds that exact prompt. Only then may the reducer write the seat artifact. Stage A derives identity, role, capability, Fable-only binding, and lens coverage from the execution attestation plus actual prompt bytes; derives canonical status, exact raw review text, start/completion instants, bundle consumption, and a unique native-result identity from the native result; maps the canonical statuses `OK`, `UNAVAILABLE`, `ERROR`, `TIMEOUT`, `REFUSED`, `CAPPED`, and `EMPTY` totally and one-to-one onto `reviewed`, `unavailable`, `errored`, `timed_out`, `refused`, `capped`, and `empty`; derives grounding only from canonical native `EVIDENCE_REF` lines that exactly name both staged final artifacts; and requires the reducer artifact to mirror every derived fact. Citation lines do not count toward the substantive-content floor. A reducer grounding list cannot create grounding, a static lens without its rendered-prompt marker cannot count, and one native status cannot be relabeled as another outcome. The last nonempty line, after the optional exact `VERDICT:` label and formatting trim, must full-match only `AGREE`, `PARTIALLY AGREE`, or `DISAGREE`; prefixes or suffixes such as `AGREE, but not approval` fail. Any `[elided]`, `<elided>`, standalone `...`, or `elided`/`omitted` marker anywhere in the body fails even when padded past the byte floor. A zero-byte/noncanonical or self-authored artifact, absent/swapped/aliased prompt or producer record, missing/extra/wrong prompt marker or native citation, citation-only or otherwise empty/sentinel/verdict-only review, malformed terminal verdict, duplicate seat/native-result identity, ungrounded review, producer/identity/capability/outcome substitution, or bundle/text/status/chronology substitution is non-review evidence and cannot count. Each retained seat receipt then mirrors only those independently derived values plus the same phase/wave/attempt/base; Stage A rejects disagreement among prompt, producer records, artifact, seat entry, and receipt. Every counting critic and Fable consumes exactly `bundle_staging.bundle` after staging. Positions 1 through 3 are critics with `binding_prover=false`; position 4 is Fable, the only `role="binding_prover"`, the only `binding_prover=true`, and starts strictly after all three critic completion instants. The final authorizing wave has no retained contradiction or material finding, and its effective-policy facts are derived from the validated producer chain rather than asserted wrappers: Stage A recomputes the roadmap-specific typed resolver input/output, vendor/lens counts, consensus, and prover fact. GPT-5.6 Sol and Fable must each derive `outcome="reviewed"`, `usable=true`, and `verdict="AGREE"`; Fable alone supplies the prover fact.

All prompt and native-report text uses one strict framing rule: UTF-8, NUL-free, terminal LF required, and CR, VT, FF, FS, GS, RS, NEL, U+2028, and U+2029 forbidden anywhere. CRLF and unterminated text therefore fail before marker or vote parsing. `AGREE`, `PARTIALLY AGREE`, `DISAGREE`, and the optional `VERDICT:` label are case-sensitive ASCII; `PARTIALLY AGREE` contains one literal ASCII space and no whitespace normalization. Substantive review/probe bodies require at least 40 raw UTF-8 bytes, at least 40 visible UTF-8 bytes after Unicode control/format/separator/whitespace characters are excluded, and at least eight alphanumeric code points; invisible padding cannot satisfy the floor. The launched early-prover terminal is the exact raw final nonempty line `PROBE_STATUS: CLEAR`; bare, case-changed, whitespace-padded, quoted, Markdown-prefixed, control-separated, or Unicode-separated probe-status terminals fail. A canonical `PROBE_STATUS: BLOCKED` may be retained only as standalone non-authorizing launched-prover evidence; it makes the prover unusable and cannot appear in a completed review-wave receipt or restart chain. The separate typed `DEGRADED_NO_LAUNCH` branch has no probe report or terminal and is authorizable only through the failed-preflight/zero-process/no-confined-fallback producer chain above. Prior restart entries are complete waves with either a fully operationally proved `CLEAR` path or that exact degraded path, later invalidated by a material finding or contradiction.

Restart chronology distinguishes two trust paths. A material seat finding is bound to its native producer interval inside the prior wave. A separate contradiction not present in that wave must be reported strictly after the prior receipt completes and strictly before the replacement early-evidence path starts; backdated or post-start contradiction records fail.
...
The native citation-marker family is detected after leading Markdown/whitespace normalization, including `+` list bullets, and case-insensitive key/colon-spacing recognition, but only an unprefixed raw line equal to `EVIDENCE_REF: ` plus the compact canonical JSON file reference may parse. A Markdown-prefixed, case-changed, colon-spaced, tab-separated, or multiply spaced look-alike line fails instead of remaining analytical prose.

Protected rendered-prompt marker families are detected after leading Markdown/whitespace normalization, including `+` list bullets, and case-insensitive key/colon-spacing recognition, then their raw lines are compared to the exact canonical case-sensitive lines. A bullet-prefixed, quoted, differently cased, or colon-spaced marker therefore fails instead of hiding beside or substituting for the expected marker.

`effective_policy` has exactly `gate`, `on_shortfall`, `required_consensus`, `required_lens_coverage`, `required_prover`, `required_vendors`, `resolver_input`, and `resolver_output`. The completion wave uses `gate="pre-merge-CR"` and resolves the shipped default `required_vendors=3`, `required_lens_coverage=3`, and `required_prover=true`; the common policy machinery remains parameterized for all four gates and retains a valid explicit boolean `required_prover=false` override that removes only the prover shortfall. The shipped defaults for plan-ratify, design-ratify, pre-merge-CR, and release-dispatch remain 3/3/true. Stage A audits those defaults separately, requires the resolver input to reference retained raw repository-manifest bytes exactly equal to `git show <audited-M>:.consiliency/manifest.json`, derives the effective gate override only from that parsed manifest, and never accepts a producer-authored override object. The separate synthetic `required_prover=false` probe proves typed parameterization and unrelated-field preservation but cannot change the effective review-wave policy. `on_shortfall=proceed_degraded` may waive only vendor/lens availability floors; `required_consensus` remains mandatory, and an effective `required_prover=true` remains hard. Stage A re-hashes both policy artifacts and the retained manifest, re-runs strict typed resolution using the production `required_consensus` key without a shadow mapping, and derives counted vendors, lenses, Sol AGREE, and the Fable prover fact only from parsed authoritative inputs and independently validated seat artifacts.

`restart_chain` is a chronological array of strict objects with exactly `attempt`, `finding`, `invalidated_agree_artifacts`, `prior_bundle`, `prior_receipt`, and `replacement_bundle_sha256`. An empty array means no contradiction occurred. Otherwise Stage A recursively rebuilds every earlier receipt and finding reference, requires contiguous attempts ending at the final `attempt`, requires each material finding or contradiction to invalidate every prior retained `AGREE`, requires a different replacement bundle digest, and requires the last replacement digest to equal the final staged bundle. A finding, contradiction, changed phase byte, or evidence change therefore invalidates the complete prior wave—not one seat—then repeats the launched-CLEAR or typed degraded-no-launch early path, bundle staging, all critics, and Fable in full. The producer closes every descriptor, makes the evidence tree immutable, and has a fresh coordinator process recompute all file references, the selected early-path producer chain, canonical seat execution attestations and native results, bundle and reducer artifacts, producer-derived review facts, mirrored receipts, chronology, effective policy, restart chain, and inner receipt digest before `update_lifecycle(..., "completed", ...)` is allowed to emit the event. No artifact reducer may create or rewrite a producer record: launched policy/config/PASS/recheck/launch/mount/worktree/data-scope/reaper/execution-attestation/native-result records remain distinct write-once producer outputs, degraded policy/preflight/launch/fallback records remain distinct write-once coordinator outputs, and artifact reduction starts only from already-closed retained descriptors.
...
The identity-correct replacement Codex evidence SHA-256 `4c87d175a9991d13671881d6abc13ae3502c66e1434f401902a6b70142ce26db` then reviewed the same predecessor plan/head/base and returned `BLOCKED` with report SHA-256 `4a98ced48e515e8e9755467351195a4c8eaf1ac6fc55757b0f5bc33978b8f5c0`, again before critic or Fable launch and with complete cleanup. It confirmed the third-citation, reducer-only, citation-only, suffix/elision, and all 42 status/outcome attacks now fail, but proved that Markdown-bullet-prefixed conflicting lens, seat, output-prefix, bundle-digest, and required-evidence markers bypassed exact-prefix collection. This repair recognizes each protected marker family after leading-markup normalization and case/colon-spacing normalization, then requires exact canonical lines; the unchanged named tests retain those cloaked-marker variants as literal RED. This evidence authorizes nothing and requires another full ordered wave.

The following canonical-label Codex evidence SHA-256 `d05fdefbbbe2127bb48975bb9578661133165ff40a0f72599f522e619451855b` reviewed predecessor plan SHA-256 `05e0c67ab96c4afd735824b65dae45650bb840ae2b60f99fe33db406ea422c71` at source head `01fa37614e5d106dca80e1b6088718c78c2f059f`, exact base bundle SHA-256 `87ddb2cb2a32abdb42fdfe03bd1ae33ad092765e7aed7a83d22468a78c3cc730`, and unchanged roadmap. It returned `BLOCKED` with report SHA-256 `325f209227cc7d89f167894c3801d0dd41afaa157d839609f08abb382edd3440` before critic or Fable launch and with complete cleanup. The prior citation-set and prompt-cloaking attacks failed, but `EVIDENCE_REF :`, lowercase `evidence_ref:`, and Markdown-prefixed variants were ignored as prose while two canonical citations still grounded the review. This repair detects the normalized citation-marker family, rejects any noncanonical spelling, and retains all look-alike variants as literal RED. This evidence authorizes nothing and requires another full ordered wave.

The next exact-digest initial-wave Codex evidence SHA-256 `48b6c70a20a506f20210dd20b6e59b3627906acea4fcb930ee0a1751cb119c7e` returned `CLEAR` for predecessor plan SHA-256 `1c1e1ca39ab26c6ce3df9527766f8032e9c598ae8c98275d6e70c91d62a9ec64` at source head `989a1ce44bddb117c143e3195a7d4d57a15c1d52`, exact base bundle SHA-256 `ec906d5aac749d420289c074d58ee8c11ca45285a529b6c016d942df4264b095`, and unchanged roadmap. The following local-three panel SHA-256 `2f481fbe097f1f7241b542dd53e071ee08efcb791923a16d136209581b4edc16` reviewed evidence-bearing bundle SHA-256 `48b3092a633af502756530f7d8c3d0df4c0ed42eef5ac4b789e3417aa4f84634`: Grok and Gemini returned usable `AGREE`, GPT-5.6 Sol returned usable `DISAGREE`, and Fable remained deferred. Sol proved that a valid Markdown `+` list bullet was absent from the shared leading-markup grammar, allowing plus-prefixed malformed native citations and conflicting protected prompt markers to remain analytical prose. This repair adds `+` to the normalizer and retains plus-prefixed citation and protected-prompt variants as literal RED. Neither artifact authorizes dispatch; the changed plan and producer-plan digests require a full new ordered wave before Fable can bind.

The following isolated Codex attempt produced evidence SHA-256 `6ce4e5a907609c374a3bf7ce779d092412faf956817ef706d3a7fdce796accdd` and report SHA-256 `be08d75bbdf54df86c6cdd487ed69ef2ee2a676f1dc9f55596c46cb4bab0fa67` for predecessor plan SHA-256 `fc2eadf24c62fcd3b274d8f251b9bf41f992d1a14b0a1a979156e8796e7e4d1d` at source head `3ed756f5a3815f18a48eb8dbe6ff7211687818b6`. It returned `BLOCKED` before critic or Fable launch and proved that Stage A accepted bare `CLEAR` and `+ PROBE_STATUS: CLEAR` instead of requiring the exact raw canonical terminal. It also correctly identified a coordinator invocation defect: the `--artifact` input was the superseded early-evidence JSON at SHA-256 `48b6c70a20a506f20210dd20b6e59b3627906acea4fcb930ee0a1751cb119c7e`, not the freshly prepared base review bundle. This repair full-matches the raw terminal line and retains all terminal variants as literal RED; the next attempt must use the exact current `v10-reviewtruth-plan-review-base.md` as its input artifact. The attempt authorizes nothing and requires another full ordered wave.

The corrected initial-wave invocation then used exact current base bundle SHA-256 `5a3e2304113d33e2769e653b2a8df00a9f2631a36c2689ca8bc4b69b9b3ff888` and produced evidence SHA-256 `32980c033f8f7767041c44e34efe23d1da5d4089ab2f6e743c280362c87a9678` with report SHA-256 `5337a66f7dafa6baa4f628dba535e50ddddd544cac1b601814eef61e3a3b2c65` for predecessor plan SHA-256 `faf437a032d0652090c88df79ec9cc358b7583e21783a89734001b6ee71a225f` at source head `e8a21eb36a3175d22a704ccd67898e3218753c6a`. It returned `BLOCKED` before critic or Fable launch. The probe proved that raw Markdown-prefixed canonical prompt markers were normalized into acceptance and that the newly documented standalone `BLOCKED` retention was inconsistent with completed-receipt validation. This repair detects families through normalization but compares raw prompt/citation lines, derives `usable=true` only for exact canonical `CLEAR`, and clarifies that `BLOCKED` is retained outside completed review-wave receipts while restart entries represent complete post-`CLEAR` waves. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `834d267cf3c1f63d43d0c45ab657dee715dd96cecfeb70fe037354d5615f0e65` and produced evidence SHA-256 `241e11e24f1c7271efbdf2fe2fe800f0e67c18a09c7ff24982db5f1328617489` with report SHA-256 `101a371a273038048ca920e2d368e37ed8a501bac374a0a1fedcf3255e99c25a` for predecessor plan SHA-256 `aed0a648106cafd28fe45b625b1ff3bca787d6501525e5f302dcf59611e06b35` at source head `4906eb73b18c5cbcc6a10aee759e824f74986736`. It returned `BLOCKED` before critic or Fable launch and proved the parameterization defect named by the maintainer: the typed resolver accepted `repository_override.required_prover=false`, but Stage A then unconditionally re-imposed effective 3/3/true and unconditionally required the prover. This repair audits shipped defaults before resolution, removes the post-resolution tuple hardcode, gates the hard prover requirement on the effective boolean, and makes `on_shortfall` affect only vendor/lens availability while consensus and an effective prover requirement remain hard. Literal RED fixtures cover each independent dial. The changed plan requires another full ordered wave.

The following exact-base Codex attempt used bundle SHA-256 `13a8a10eb6427979ab0db5b1b3842bc1bcfe2a2cc21fd0eef7f56a59e6601f28` and produced evidence SHA-256 `d04c94196ca236d294b1a931e5024433a86f3d3de4819d3bebe088a07df0f738` with report SHA-256 `0b50df7562a8e5cd662f876d9e83b0fa1e848383d8cbb6ac71c2ae1dc0513a3b` for predecessor plan SHA-256 `69cd453dcf17fb083211843d5934eab40b00cf1e318157fd76cedd4788cc0daa` at source head `08b509b117c8f326f85c8137e234fa97b94ae70a`. It returned `BLOCKED` before critic or Fable launch. The parameterized-policy controls held, but the probe proved that `.strip()` accepted multiple spaces or a tab after the exact citation prefix. This repair requires the complete raw citation line to be exactly `EVIDENCE_REF: ` plus compact canonical JSON and retains post-colon/trailing whitespace mutations as literal RED. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `121f6d544a32a2d47bd5afcfecb6b0073023fcd176c36851698e43092eb4b5bf` and produced evidence SHA-256 `cc10f9fae4fb589117d06ac1bf56ab21c2a7d8d89f02de6330a154e14c618595` with report SHA-256 `ff08d3c84b0ed45725ecfaef1a57cf7a5d67cf6e94ad4db7b6aa126cdd6b55af` for predecessor plan SHA-256 `81e33a631ab208cc6db0e7ac6cea226928b1b2258cc67b9888823971c8aa4ee1` at source head `2fe0dff774e27ad16f28cae2b7f74007fdbc230d`. It returned `BLOCKED` before critic or Fable launch and proved that Python `splitlines()` discarded VT, FF, RS, U+2028, and U+2029 after otherwise canonical citation and probe-terminal bytes. This repair uses LF-only framing and ASCII-horizontal marker trimming throughout prompt, citation, review, and probe parsing; every control/Unicode separator remains in-line and fails. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `11d2a98df6616b2e2a2a06143bc2192e02111f86c737692966e4c4437f9662da` and produced evidence SHA-256 `b27b94fd841fa640dea85fe71a6858ab7bcd1c7827c3b5418cb63935bf79840f` with report SHA-256 `940d4759d96c702c4ceaccfb2d40fee7b4b9461cbf3a23b2a62f0b2aa6d7bee4` for predecessor plan SHA-256 `c03d7fac7773da1d4baad44b92d55a02042b29a0534d27c9d8c34cd8714f9dd7` at source head `0b3784efcbc79dfe8bff77b90372a00aa97e39fc`. It returned `BLOCKED` before critic or Fable launch. Citation, prover-terminal, ordering, and parameterized-policy probes held, but the probe proved `PARTIALLY\s+AGREE` and subsequent whitespace normalization still accepted the six control/Unicode separators inside a review verdict. This repair requires one literal ASCII space, removes verdict whitespace normalization, and retains all six verdict-separator mutations as literal RED. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `b60692707f65eca79c9fdf6eb32129ad44336f74b8a687982b1153fed8893f4b` and produced evidence SHA-256 `8148f2614cb537b7c8113d5c3d66cb8966cb3918cf50dee8f8cbc293feb42063` with report SHA-256 `0995f91f5939f0c28fa4cb35c52178b408178fb8f22b83b7ed5ff7ee824bc835` for predecessor plan SHA-256 `9c5532ea6e277053ff0e871bc7ac2bb270d94b0450ef034433729d13a3945982` at source head `ebcf5a617962ed1cfb04e215372fa65928d6f9f6`. It returned `BLOCKED` before critic or Fable launch. Marker, policy, and specific terminal-separator probes held, but the attempt proved that framing was not globally enforced: separators in substantive text, CRLF, missing terminal LF, and lowercase review verdicts could still produce usable results. This repair centralizes strict LF framing for every prompt/review/probe string, forbids the complete separator set anywhere, requires terminal LF, and makes verdict labels/tokens case-sensitive ASCII. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `86c70e1dd893676119c26ac6ddfec0674590cc7bffb8f4da4ee6a1bb2c26f2c9` and produced evidence SHA-256 `a338117d38fa8c0a51bc59d7346f30c2c427db8a1e831b1e79d315845fd99d39` with report SHA-256 `4457085818c637ff241a2d23bc9cddedebdcfae7a4173862ffb9613da89a532c` for predecessor plan SHA-256 `9ae467e59dbc9b92334e5e71a05521ee38a06faf32feb2ffe5cd9225a14fd657` at source head `0b3d8416154adbeca6be2463afa36d2835de86f6`. It returned `BLOCKED` before critic or Fable launch. Strict framing, marker, verdict, policy, and ordering probes held, but the attempt proved that two distinct correctly hashed zero-byte `final_audit` and `final_evidence` refs could satisfy exact grounding with no evidence content. This repair requires positive declared and recomputed bytes for exactly those two base artifacts and retains both the attack and an unrelated-empty-file scope control as literal RED. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `fcda530ea002e016c463501d87a05edd37e8e38eca99b992460df5d16e3d008e` and produced evidence SHA-256 `07b1688bfa471eea0bd921cb432b8b6ea3846d37dcb583ad397fddba26d3a674` with report SHA-256 `0ec3eadfef87f8b620a1b177b88dbf8dcd65563e1229ec645ed6d9eebcc47cf9` for predecessor plan SHA-256 `c29bf1b9ffd08583203614a4807dca13258b2ceca7fc42985af6119dea057224` at source head `5ab4146c6acd97eb2b2539cbbc25dedfb977d4a8`. It returned `BLOCKED` before critic or Fable launch. Nonempty grounding, framing, marker, policy, and ordering probes held, but fourteen U+200B zero-width spaces met the 40-byte body floor and promoted otherwise citation-only early/critic reports. This repair retains the raw-byte floor and adds a 40-visible-byte plus eight-alphanumeric floor after Unicode control/format/separator/whitespace exclusion, with multilingual positive and invisible-format negative fixtures. The changed plan requires another full ordered wave.

The next exact-base Codex attempt used bundle SHA-256 `2d025de4899d22058d152a9e1d01629b1788f280dfd44198456eecbd4a8496a6` and produced evidence SHA-256 `5a761ede7fbc8988f4961e6528f8e458fa43f8a225eed8b7240450088b44e418` with report SHA-256 `1d5b44e87a035397614dec9d3e9a93dc7be3764a16fa848d25a0b2d7ffb0c661` for predecessor plan SHA-256 `0fe1fecdaac19c667d2f82c6b5cf37b65dc0a83f933e26eaa48b287b587751d6` at source head `0801db87eee4621b67b4c64fcc381720edfc91f5`. It returned `BLOCKED` before critic or Fable launch. Visible substance, grounding, framing, policy, and ordering probes held, but the attempt proved external restart contradictions were accepted only when backdated before prior completion. This repair binds material in-wave findings to their native producer interval and requires external contradictions strictly between prior receipt completion and replacement CLEAR-prover start, with boundary/backdating mutations retained as literal RED. The changed plan requires another full ordered wave.

The corrected chronology attempt then produced CLEAR early-prover evidence SHA-256 `d6a5d0b0721b3899972fd12d3c2d1c25efce59bb6bb62175e9d31fa8aa641499` with report SHA-256 `6787fca77d0ec7d90df958911dae230c8c637e63fa736507df20ce9f32fecef2` for predecessor plan SHA-256 `c7bc20c909a3a8c0d4c1c97a1e752fbf9611d88d0f9beea388a28183471e4a29` at source head `933518c78efc65e2bdfc95e50245cd8bf27f9374`, exact base bundle SHA-256 `24a07afd898e17e2628a8a3c6d916e52901021a46f05e36465ccc0b3ac18eca6`, and unchanged roadmap. The following local-three panel SHA-256 `c6ed5b820a5942e6d32a64e3a53e1812719b460189af6637a0be609d64ce8b3f` reviewed evidence-bearing bundle SHA-256 `739ee5fd6719c383494390d90062b61f8a61dcd107f9acc32293581d2aa3dd43` (`690315` bytes) under instructions SHA-256 `306108775b88e999995e42b2abe40cae4093c69969b0531b3bdce4083179e469`: Grok and Gemini returned usable `AGREE`, GPT-5.6 Sol returned usable `DISAGREE`, and Fable remained deferred. Sol proved that Stage A required a shadow `consensus` key while the typed `RatificationPolicy` and production resolver use `required_consensus`; production-shaped evidence was rejected while synthetic shadow evidence could pass without proving the real override. This repair uses `required_consensus` end to end in defaults, repository overrides, resolver outputs, effective-policy evidence, and evaluation, and retains the shadow-key substitution as literal RED while proving every unrelated field survives an override. Neither artifact authorizes dispatch; the changed plan requires another full ordered wave.

The production-key repair then produced CLEAR early-prover evidence SHA-256 `61e4abb6f652fd06c3a02b9a90aa35ef5cb6ef813cf9aba7e2d4b2f9b42df7ff` with report SHA-256 `180e3e83da89fbc2117f2ee7b25bdbb48e0e1506a7fca662c4304603c16652cc` for predecessor plan SHA-256 `a1a788a52ed591af61eaeeedb3dc91be159b95f4ef0f5df5aa4d7eba9493c8da` at source head `3fe688379103de1c3156965931a506213a60997b`, exact base bundle SHA-256 `33759a27b0b3b033c0aee24f85ce9ecdd03ab8415ea36014e7a45700b943e74c`, and unchanged roadmap. The following local-three panel SHA-256 `1ad9f8eb543a9d8a0c2d4d264f99e2d7d1919f02060e2da69ff69b760e918a22` reviewed evidence-bearing bundle SHA-256 `0f74ae8176389e31d33a1bf04f1049c59114ad7f05c3304605601ee7e0e815d6` (`692787` bytes) under instructions SHA-256 `4da565a0f99336e1c019156451ef20e65e1de379a7833fcaca492f80f73e453c`: Grok and Gemini returned usable `AGREE`, GPT-5.6 Sol returned usable `DISAGREE`, and Fable remained deferred. Sol proved an integration blocker: CONFORM and HARDEN still specified the rejected shadow `consensus` key in the exact producer receipt contract, so no conforming dependency receipt could pass REVIEWTRUTH Stage A. This repair changes both producer plans to emit only production `required_consensus`; all three changed plan digests are unreviewed and require fresh ordered waves. Neither artifact authorizes dispatch.

The integrated producer-key candidate then produced early-prover evidence SHA-256 `ce59c1d27871cb6f28ecfd911a5ce05bfcdc86022fbc07aaf99c4724262ce0a1` with report SHA-256 `d6eaac9b364ebd2bf7fe915173d697c93e92b4581c8c1981a3861fc030adfc40` for predecessor plan SHA-256 `eda02ce50c74e6408306f20b0765b6e1b5399c40bc16e63a8025a7625efbf541` at source head `d479595ec180a66298cfdf4c766dc155388c8d4f`, exact base bundle SHA-256 `6dbe1899abb32869f5f918a377528299138ba2b91ec5a91eb30cb8e279ec8877`, and unchanged roadmap. It returned `BLOCKED` before critic or Fable launch and proved that the opening single-plan scope statement contradicted the later cumulative candidate ownership of the roadmap reseal, CONFORM/HARDEN producer repairs, and tracked lock prerequisite. This repair states the cumulative root-candidate scope and the narrower current integration delta separately, without changing the already sealed roadmap or lock bytes. The evidence authorizes nothing; the changed plan and manifest digest require another full ordered wave.

The scope-coherent candidate then produced CLEAR early-prover evidence SHA-256 `beb8d9d91d237c8e823a9fb02c032489d64490d545e6ab825d1df8ceb935a371` with report SHA-256 `8f8f9d3f987171a05fcbf3494aaca82d40e6c066eb4b7b8581fccd6084265ead` for predecessor plan SHA-256 `2e26757699cb1c9af8af19904e8d54f610394c87ea02d85944f12f68355da1e7` at source head `700df6c25e167839b5aec65c8d252c1aba35a0d3`, exact base bundle SHA-256 `6c840524a3ccbf80fef94a7412c1db3d7883585dc3492257070f326367acb23d`, and unchanged roadmap. The following local-three panel SHA-256 `8998484b2ac7ce0ff0c114779222c6b0ac74f173cc194a6d2d0b27fc9862f333` reviewed evidence-bearing bundle SHA-256 `4dc6b19a7666c7e17b73df3ef60a5ff71a97d1253095f12a1face43326167e07` (`696161` bytes) under instructions SHA-256 `00ba7252d322a047fd75078cd8b9f31dcc37ef59e496436e6cf41faed719f6c6`: Grok and Gemini returned usable `AGREE`, GPT-5.6 Sol returned usable `DISAGREE`, and Fable remained deferred. Sol proved that Stage A trusted a producer-authored `repository_override` object instead of deriving it from exact audited `M:.consiliency/manifest.json`; an internally consistent fabricated `required_prover=false` could therefore waive Fable. This repair replaces that object with a retained raw manifest reference, byte-compares it to the audited Git object, derives the effective override only from those parsed bytes, and retains the false override solely as a separate non-authorizing parameterization probe. Neither artifact authorizes dispatch; the changed plans and manifest digests require fresh ordered waves.

The manifest-authority repair then produced early-prover evidence SHA-256 `bc4e9afe8ecfd56ff911c0ad6ddf497138088936108b6a75a88cccfee453cf2f` with report SHA-256 `fc75f99d4574d8aa624dbb9f9ac651b2c2cec30b3b0ca5dbdccf7a0b759c927e` for predecessor plan SHA-256 `f7de9dcc0a823c96066dcc8346c4f94f453e04edb05cf145c00e4633c799d5cb` at source head `32508faeb9cb03abeb3eec48755a44a658dc8a01`, exact base bundle SHA-256 `28a2235a29e9f919c9df6ae4a2913ff24fb72fd132a8d9c9f3135c4032bea83d`, and unchanged roadmap. It returned `BLOCKED` before critic or Fable launch, with successful stable preflight, one contained Codex launch, zero dirty paths after cleanup, and complete worktree/auth-home cleanup. It confirmed the retained manifest is independently bound to audited `M`, but proved that the completion schema unconditionally required launched `CLEAR` evidence even though maintainer addendum `5139955591` expressly permits a typed degraded-evidence wave when Codex fails preflight and no OS-confined Grok fallback exists. This repair adds that non-overlapping zero-process producer branch without admitting launched `BLOCKED` evidence or weakening Fable-only binding. The evidence authorizes nothing; all three semantically changed producer/consumer plans require fresh ordered waves.

The resulting dual-path repair then produced early-prover evidence SHA-256 `f17ff22b0b13ed73fc719a9a435253e69b8e291669116945c4d3bba732c1fd5d` with report SHA-256 `dd6280edf33532a58745d5152558869b9aa460ac942deed1ec47f89563d4fddf` for predecessor plan SHA-256 `5c8b2165955bc8692cc5f6296e58007cd4f3a0740bcb280aec07df3df0487ae6` at source head `d872d6bc9aee3870b107e064d16c9b3de4d4218d`, exact base bundle SHA-256 `4c9cc4b1693abaa906dd51bcd1651434e56ec9b8f191f10ea211384c823e0fde`, and unchanged roadmap. It returned `BLOCKED` before critic or Fable launch after successful stable external-tool preflight, one CLI-enforced workspace-write Codex process, zero dirty paths, and complete worktree/auth-home cleanup. It proved that the launched Stage-A branch still opened only the generic execution attestation, prompt, and native result, so a nominal `CLEAR` could enter the bundle without independently proving the required policy/effective-config PASS, stable prelaunch recheck, one-process launch, safe mount, exact detached worktree/base, isolated data scope, or external reaper. This repair adds those separately produced records to the launched artifact and mandatory bundle set and derives launched eligibility from their bytes before accepting `CLEAR`. The evidence authorizes nothing; all three producer/consumer plans require another fresh ordered wave.
...
- [ ] PC-REVIEWTRUTH-9 — EC-REVIEWTRUTH-18 is an exact six-node literal TDD contract in the already-owned `phase-loop-runtime/tests/test_reviewtruth_phase.py`: `test_review_wave_board_uses_opus_prover_then_separate_fable_president`, `test_review_wave_opus_unavailable_uses_codex_prover_fallback`, `test_review_wave_critic_dissent_never_skips_opus_or_fable`, `test_review_wave_opus_prover_attests_can_probe_never_binding_prover`, `test_review_wave_opus_prover_preflight_rejects_extra_authority_before_launch`, and `test_review_wave_opus_prover_receipts_bind_model_tools_roots_environment_and_artifact_digest`. Their exact full repo-relative nodeids (`phase-loop-runtime/tests/test_reviewtruth_phase.py::<name>`) have count `6` and sorted-LF SHA-256 `161d57ceca75fc0131822802282ff52985e9ee081e536c0db78a5f3e9f37292b`; their exact corresponding anchors are `REVIEWTRUTH_RED::board_opus_fable_topology`, `REVIEWTRUTH_RED::opus_codex_fallback`, `REVIEWTRUTH_RED::critic_dissent_non_gating`, `REVIEWTRUTH_RED::opus_can_probe_only`, `REVIEWTRUTH_RED::opus_preflight_fail_closed`, and `REVIEWTRUTH_RED::opus_receipt_binding`, with sorted-LF SHA-256 `583efd5c10ee547677b045775a22a6116232ee61d4ac4fc04d1b40011395838f`. SL-1 authors all six before production mutation and adds them mechanically to every affected REVIEWTRUTH node/anchor aggregate. The capability node consumes PC-REVIEWTRUTH-7's immutable EC-REVIEWTRUTH-16 Fable-only binding-prover assertion unchanged and proves the Opus/Codex board role attests only `can_probe`. The two safety nodes independently reject extra Bash/shell/web/MCP authority, unsanitized environment, unbounded roots, ineffective or mismatched preflight, and substituted model/tool/root/environment/artifact receipt fields. Together the six nodes preserve EC-REVIEWTRUTH-16 and EC-REVIEWTRUTH-17 by reference while freezing the additive four-leg-board plus separate-president topology, typed Opus-to-Codex fallback, sequential Claude-model use, critic-dissent non-gating behavior, and the new Opus path's confinement.
- [ ] PC-REVIEWTRUTH-10 — EC-REVIEWTRUTH-19 is an exact four-node literal TDD contract in the already-owned `phase-loop-runtime/tests/test_reviewtruth_phase.py`: `test_review_wave_fable_president_binds_per_finding_dispositions_to_digest`, `test_review_wave_deferred_finding_requires_verbatim_issue_before_dispatch`, `test_review_wave_nondeferrable_or_president_owned_finding_blocks`, and `test_review_wave_president_forces_ruling_at_three_round_cap`. Their exact full repo-relative nodeids (`phase-loop-runtime/tests/test_reviewtruth_phase.py::<name>`) have count `4` and sorted-LF SHA-256 `01857917639d696d6163e219c16bf4f3ee1470300c6e24a68e2efd565f6d8eb9`; their exact corresponding anchors are `REVIEWTRUTH_RED::president_digest_bound_dispositions`, `REVIEWTRUTH_RED::deferred_issue_before_dispatch`, `REVIEWTRUTH_RED::nondeferrable_and_self_finding_block`, and `REVIEWTRUTH_RED::president_round_cap`, with sorted-LF SHA-256 `8dc5f02d01ef37da5f5bed243c73ce416c69703df72fff62193cff00085ed2e2`. SL-1 authors all four before production mutation and adds them mechanically to every affected REVIEWTRUTH node/anchor aggregate. The first node independently mutates and rejects the finding text, artifact digest, disposition, and rationale so rationale bytes are as load-bearing as the other three fields. The tests freeze exact-text, artifact-digest, disposition, and rationale binding; issue-before-dispatch enforcement for every deferral; non-deferrable and president-originated blocking; critic-dissent non-gating; and a forced terminal classification by round three with round four rejected.

  The frozen Grok nodeid is deliberately two-arm: the positive arm requires typed failed Codex
  preflight, zero Codex launches, positive OS confinement, exactly one confined Grok `can_probe`
  launch, and its digest-bound evidence; the negative arm requires the same typed Codex failure,
  unavailable confinement, and a zero-Codex/zero-Grok degraded receipt. Both arms assert the
  `REVIEWTRUTH_RED::degraded_unconfined_grok` injection anchor before implementation.
...
- [ ] IF-0-REVIEWTRUTH-3 — the owner-qualified roadmap gate for PC-REVIEWTRUTH-8. It preserves legacy panel and launcher paths while adding the gate-ordered, serial, isolated, digest-bound early-prover wave whose Codex/Grok result is evidence-only and whose Fable result alone can bind an effective prover requirement. Codex launch is further gated by the exact canonical `CodexExternalToolPolicy` and effective preflight/config/launch-receipt contract: the launch tuple explicitly disables all frozen external/dynamic surfaces, requires `sandbox_workspace_write.exclude_tmpdir_env_var=true` and `sandbox_workspace_write.exclude_slash_tmp=true`, and replaces every layered MCP entry with an inert disabled same-transport definition; preflight proves the same executable/config tuple has no enabled or unknown relevant surface, no `/tmp` authority, and no `TMPDIR` authority beyond a canonical path already inside the per-leg worktree/data scope. Unknown, enabled, locked, changed, uninspectable, missing-exclusion, false-exclusion, or external-`TMPDIR` state emits a digest-bound zero-Codex receipt; it may transition only to a separately attested OS-confined Grok fallback, otherwise both process counts remain zero. Before any leg launch, the external coordinator must also supply and preflight one explicit canonical writable non-symlink `workspace_mount`, pass it unchanged through `lane_worktree_root(..., workspace_mount=...)` and `create_phase_worktree(..., workspace_mount=...)`, create the isolated per-leg worktree and its parent itself, and retain sole external reaper authority; a failed mount, containment, ownership, permission, free-space/inode, ephemerality, or pass-through check produces zero leg launches. Policy/config/preflight/launch/mount/worktree/reaper receipt digests are required inputs to every head/worktree/data/output/evidence digest, and shell sandbox coverage alone is never sufficient. The final wave emits the acyclic strict `v10.review-wave-receipt.v1` wrapper above and no independent SHA-shaped review claims. This fixed safety boundary does not alter the parameterized gate policy: a valid per-repo `required_prover=false` override remains valid and changes only whether a prover is required, never the confinement or mount contract for a launch that is attempted.
- [ ] IF-0-REVIEWTRUTH-4 — the owner-qualified topology gate for PC-REVIEWTRUTH-9. It composes one four-leg board with the Claude role pinned to exact row `claude / claude-opus-5 / correctness / can_probe / write_capable`, selects the Codex fallback only from a typed Opus preflight/availability failure, and runs Opus only through the canonical host-relative Claude native-fill/subscription-TUI adapter, never direct `claude -p`. Under Claude Code, every Claude-family seat, including Opus and Fable, uses the native Anthropic subagent rendezvous; under every non-Claude host and explicit standalone mode, the authenticated Claude subscription-TUI adapter remains the non-native transport. An own-vendor subscription-TUI fallback under Claude Code is forbidden, and the pre-amendment “never native Task Agent” rule is superseded only for the Claude Code host. `write_capable` describes its confined workspace authority, not ratification capability: Opus and Codex can attest only `can_probe`, never EC-REVIEWTRUTH-16's `binding_prover`. The coordinator launches Opus inside IF-0-REVIEWTRUTH-3's per-leg ephemeral worktree/data scope, explicit mount, and serial write lease after a fail-closed preflight binds the exact model and authenticated host-relative transport, scrubbed subscription environment, allowed tools exactly `Read,Write`, no Bash/shell/web/MCP authority, and add-directory roots limited to the staged review directory and per-leg worktree. Digest-bound preflight, launch, and native-result receipts become board evidence; every receipt binds the exact model, tool set, roots, environment posture, artifact digest, and preflight result; failure launches no Opus process and types the Codex fallback. No new launcher branch is introduced. It waits for all four terminal leg outcomes and then invokes a distinct president capability row `claude / claude-fable-5 / correctness / binding_prover` through the same host-relative rule. The two Claude models are sequential roles in one wave, not concurrent board seats; critic verdicts are inputs and never dispatch gates. The usable grounded artifact-bound president result supplies EC-REVIEWTRUTH-16's binding-prover fact. EC-REVIEWTRUTH-16 and EC-REVIEWTRUTH-17 remain the sole capability and ordering contracts.
- [ ] IF-0-REVIEWTRUTH-5 — the owner-qualified authority gate for PC-REVIEWTRUTH-10. It records one exact-text-and-artifact-digest-bound `BLOCKING | DEFERRED` disposition plus rationale for every distinct board finding, blocks dispatch on any open `BLOCKING`, requires a verbatim-text issue binding before any `DEFERRED` dispatch, rejects deferral for evidence/attestation/frozen-test contradictions and president-originated findings, and enforces a maximum of three president-driven rounds with complete forced classification on round three. The president may request fix-and-re-review or unchanged-candidate cross-pollination between rounds, but a fourth round is invalid.
- [ ] PC-REVIEWTRUTH-11 — native-first routing is universal and explicit. `HostContext` identifies one supported host harness and whether its native-agent capability is actually available; `identify_native_host_seats()` selects every matching vendor-family seat instance in order. Codex-host/OpenAI, Claude-host/Anthropic, Gemini-host/Google, and Grok-host/xAI matches emit typed native requests and accept only exact-once identity-bound native reports. Matching seats never use an external adapter or silent fallback. Nonmatching seats preserve their adapters. Standalone/no-capability mode is explicit and all-adapter. The same request/report, outcome, grounding, lifecycle, quiescence, evidence, and finalization contracts apply to native and adapter results, while capability policy remains independently Fable-only for `binding_prover`.
...
- **Scope**: Only after the dependency-complete Stage-B plan is exact-base frozen and reviewed, land the complete REVIEWTRUTH falsifier and compatibility suite before production mutation, including the literal three-node EC-REVIEWTRUTH-16 policy contract, exact 16-node EC-REVIEWTRUTH-17 gate-order contract, six-node EC-REVIEWTRUTH-18 topology/capability/safety contract, four-node EC-REVIEWTRUTH-19 authority contract, and six-node plan-local PC-REVIEWTRUTH-11 universal native-first contract.
...
- **Tasks**:
  ...
  - test: SL1-T1 EC-REVIEWTRUTH-18/-19 arm — Before any production edit, author PC-REVIEWTRUTH-9's exact six topology/capability/safety nodes and PC-REVIEWTRUTH-10's exact four authority nodes in `phase-loop-runtime/tests/test_reviewtruth_phase.py`, add them mechanically to every Stage-B aggregate node/anchor set, and run each under forced activation. Retain full-nodeid-set digests `161d57ceca75fc0131822802282ff52985e9ee081e536c0db78a5f3e9f37292b` and `01857917639d696d6163e219c16bf4f3ee1470300c6e24a68e2efd565f6d8eb9`, anchor-set digests `583efd5c10ee547677b045775a22a6116232ee61d4ac4fc04d1b40011395838f` and `8dc5f02d01ef37da5f5bed243c73ce416c69703df72fff62193cff00085ed2e2`, and prove only the ten mapped RED anchors before implementation.
  - test: SL1-T1 PC-REVIEWTRUTH-11 arm — Before any production edit, author the six exact universal native-first nodes named in `## Acceptance Criteria` in `phase-loop-runtime/tests/test_reviewtruth_phase.py`, add them mechanically to every Stage-B aggregate node/anchor set, and prove each fails only at its distinct `REVIEWTRUTH_RED::native_first_*` anchor. The matrix node covers Codex, Claude Code, Antigravity/Gemini, and Grok hosts and proves no matching adapter subprocess occurs. The duplicate-seat node proves every same-family seat is native. The standalone node proves explicit all-adapter topology while preserving non-native adapter behavior. The identity node rejects every missing/duplicate/late/replayed/cross-seat/cross-host/digest-substituted report. The host-integration node rejects missing or falsely inferred host capability. The skill-contract node proves every supported harness source/bundled skill uses the typed rendezvous and never recursively invokes a complete external board.
  ...
  - test: SL1-T1 dependency-completion provenance arm — In the already-owned `phase-loop-runtime/tests/test_reviewtruth_phase.py`, add the exact future falsifiers `test_dependency_completion_preflight_rejects_noncanonical_or_rewritten_origin`, `test_dependency_completion_preflight_rejects_identical_or_arbitrary_ancestor_f`, `test_dependency_completion_preflight_rejects_wrong_order_or_non_manifest_delta`, `test_dependency_completion_preflight_rejects_missing_duplicate_drifted_or_wrong_phase_event`, `test_dependency_completion_preflight_rebuilds_review_wave_receipt_and_artifact_digests`, `test_dependency_completion_preflight_rejects_review_wave_chronology_bundle_or_policy_mismatch`, `test_dependency_completion_preflight_rejects_stale_agree_or_incomplete_restart_chain`, and `test_dependency_completion_stage_b_rejects_origin_f_or_m_sidecar_drift`. Their shared positive fixtures build distinct legal `M -> FH -> F` histories and immutable evidence roots for both launched-CLEAR and degraded-no-launch paths, then prove canonical origin authentication, exact ordered parents, manifest-only deltas, strict event addition/preservation, canonical receipt/bundle/producer/artifact bytes, direct re-hashing of every relative locator, preflight-derived identity/capability/binding, launched policy/config/PASS/recheck/one-process launch/safe-mount/exact-worktree/isolated-data-scope/external-reaper plus native-result-derived status/text/chronology/bundle, or degraded failed-preflight/zero-process/no-confined-Grok derivation, rendered-prompt lens/seat/bundle binding where launched, exact native `EVIDENCE_REF` grounding in both final artifacts where launched, total canonical status-to-outcome derivation, reducer mirroring, exact terminal verdict, non-elided substance, floor facts, early evidence → staging → critics → Fable chronology, one common bundle, effective 3/3/true pre-merge policy with Sol/Fable usable `AGREE`, sole Fable binding, and Stage-B sidecar rebinding. The unchanged named review-wave bundle test must first retain RED fixtures for the formerly accepted `[elided]` plus 41-byte padding ending in `AGREE`, the synthetic all-AGREE four-seat artifact/receipt set with no producer records, a generic Fable `AGREE` whose reducer alone lists grounding refs, a citation-only native body ending in `AGREE`, a substantive native body citing both final artifacts plus a third valid bundled artifact, a three-lens wave whose rendered prompts omit the lens markers, a prompt with the expected marker beside plain and Markdown/whitespace/case/colon-spaced cloaked conflicting markers, and every cross-class native status/outcome substitution. Separate mutations cover absent/malformed/duplicate/extra/wrong-digest and case/spacing/Markdown-look-alike native `EVIDENCE_REF` lines, prompt omission/substitution/aliasing and protected-family cloaking, dropped, duplicated, extra, or wrong lens/bundle/seat markers, `AGREE, but not approval`, an opaque early artifact, absent/swapped/aliased preflight or native-result refs, forged producer/identity/text/status/bundle/chronology values, duplicate native-result IDs, every launched-path missing/aliased/failed/drifted policy/config/preflight/recheck/launch/mount/worktree/data-scope/reaper record, enabled MCP/plugin surface, writable-root or temporary-exclusion drift, wrong process count/base/tree/path containment, shared database access, non-serial holder, leg-owned cleanup, or failed reaping, a regex-valid false digest, byte-count drift, locator escape/symlink, zero-byte or noncanonical artifact, empty/sentinel/verdict-only review text, forged wrapper usability/outcome/verdict/vendor/lens/capability, duplicate seat-instance, grounding-entry or consumed-bundle substitution, self-referential JSON, a critic before staging, Fable before critic completion, stale `AGREE`, missing invalidation, unchanged restart bundle, incomplete full-wave restart, a degraded branch with passing or empty-reason preflight, any Codex/Grok process count or launched flag, available Grok confinement, wrong producer or cross-attempt/base refs, absent policy/effective-config bytes, or a degraded wrapper without the complete producer chain, all without contacting the network; production/parser code is not written in this repair.
  - test: SL1-T1 leading-markup grammar arm — In that same immutable review-wave bundle node and before parser implementation, retain literal RED mutations for `+ EVIDENCE_REF: {…}`, `+ EVIDENCE_REF : {…}`, `EVIDENCE_REF:   {…}`, `EVIDENCE_REF:\t{…}`, trailing citation whitespace, and Markdown-prefixed canonical or conflicting lens, seat-instance, output-prefix, bundle-digest, and required-evidence prompt markers. For prompt, review, and probe text, retain CRLF, missing-terminal-LF, NUL, CR, VT (`U+000B`), FF (`U+000C`), FS (`U+001C`), GS (`U+001D`), RS (`U+001E`), NEL (`U+0085`), U+2028, and U+2029 mutations at marker, terminal, and substantive-body positions and require framing rejection before vote parsing. Also retain bare/lowercase `agree`, lowercase `verdict: AGREE`, separator-substituted `PARTIALLY AGREE`, bare `CLEAR`, `+ PROBE_STATUS: CLEAR`, case-changed, whitespace-padded, quoted, and Markdown-prefixed probe terminals, plus a canonical standalone launched `PROBE_STATUS: BLOCKED` that must remain non-authorizing and be rejected from every completed receipt/restart entry. Each must fail as malformed framing, a noncanonical raw marker, or a noncanonical/non-CLEAR launched terminal; none may remain analytical prose or make a launched review/prover usable. The separate positive degraded fixture contains no probe text or terminal and passes only through its typed zero-launch producer chain.
  - test: SL1-T1 parameterized-policy arm — In the same immutable review-wave node, retain the shipped-default fixture proving all four gates resolve to 3/3/true, then apply typed overrides independently through the production `required_consensus` key. The positive receipt carries retained raw `.consiliency/manifest.json` bytes exactly equal to audited `M`; Stage A derives the effective override only from those bytes. A producer-authored override object, retained-manifest byte/digest/path substitution, audited-commit substitution, or literal shadow `consensus` key must fail. A separate synthetic `required_prover=false` probe with all unrelated values byte-equal must remove only the Fable-prover shortfall in its probe output but must never alter the effective review-wave policy; the same manifest-derived effective policy under `true` still fails without Fable. Vendor/lens overrides and `on_shortfall=proceed_degraded` may waive only their manifest-authorized availability floors; required consensus still fails, and a manifest-effective `required_prover=true` still fails, independently of `on_shortfall`. No verifier may trust a producer override or re-impose the shipped tuple after resolution.
  - test: SL1-T1 nonempty-final-grounding arm — In the same immutable review-wave node, replace both distinct final-artifact refs with correctly hashed zero-byte regular files and cite them canonically from otherwise substantive early and critic reports. Validation must fail at the base final-artifact invariant before exact-set grounding or usability can be derived. Keep a separate zero-byte unrelated retained-file control to prove the restriction is scoped to `final_audit` and `final_evidence` rather than all file references.
  - test: SL1-T1 visible-substance arm — In the same immutable review-wave node, use U+200B, U+200C, U+200D, U+2060, U+FEFF, bidi/format controls, and Unicode separators as invisible padding around exact citations and canonical terminals. Invisible-only bodies and bodies that meet 40 raw bytes but not 40 visible UTF-8 bytes plus eight alphanumeric code points must remain unusable for both early-prover and critic paths; a multilingual visible-body control above both floors must pass.
  - test: SL1-T1 restart-finding chronology arm — Retain one material seat finding inside its native start/completion interval and one external contradiction strictly between prior receipt completion and replacement early-evidence start as positive controls. Exercise both a replacement launched-CLEAR path and a replacement degraded no-launch path. Backdate the external contradiction before/equal to prior completion, move it to/after replacement early-evidence start, move an in-wave material finding outside its producer interval, and relabel a non-contradicting external finding; every mutation must fail at its distinct chronology/binding anchor.
  ...
  - verify: SL1-T3 — Retrospectively and independently re-prove all three exact SL-0 comment identities/body bytes/body digests and six timestamps, Option-2/prover/ordering semantics, parameterized-policy/default distinction, non-authorizing `agent-harness#405`, Stage-B exact-base/profile facts, immutable Stage-A snapshot manifest regeneration, separate reviewed Stage-B plan binding, chronology, paired observations, and all predecessor plus EC-REVIEWTRUTH-17/-18/-19 RED invariants. This verifier gates SL-2 through SL-5 and cannot be used to skip Stage B.

| Task ID | Type | Depends on | Files in scope | Tests owned | Test command | Work |
|---|---|---|---|---|---|---|
| SL1-T1 | test | SL-0 | all eleven SL-1 owned paths except `docs/research/reviewtruth-red-baseline.md`; the coordinator-provided absolute external runner root is operational evidence, never a landing path | all predecessor EC-REVIEWTRUTH-1-through-16 nodes, the exact PC-REVIEWTRUTH-7 triple, PC-REVIEWTRUTH-8's 16-node set, PC-REVIEWTRUTH-9's six-node set, PC-REVIEWTRUTH-10's four-node set, PC-REVIEWTRUTH-11's six-node set, and the eight exact dependency-completion provenance falsifiers above; all predecessor compatibility, chronology, collection, native-identity, golden, evidence, and JUnit contracts | the Stage-B exact operational chain, then the literal EC-REVIEWTRUTH-17/-18/-19 and PC-REVIEWTRUTH-11 RED commands in `## Verification`, then existing focused/default/activated/golden commands | Verify the exact dependency-complete Stage-B base/tree, both recomputed dependency review-wave receipts, and sealed paired profile before mutation. Author the eight dependency provenance tests, all 26 EC-REVIEWTRUTH-17/-18/-19 tests, and all six PC-REVIEWTRUTH-11 tests in the already-owned test files; prove their literal RED anchors before any production implementation. Stage B, not this document's historical constants, supplies every aggregate collection/count/digest literal. Preserve all predecessor falsifiers and mechanically recompute every aggregate node/anchor set. |
...
| SL1-T3 | verify | SL1-T2 | all eleven SL-1 owned paths | retrospective SL-0 authority, pre-implementation chronology, bootstrap trust, activation, collection, EC-REVIEWTRUTH-17/-18/-19 and PC-REVIEWTRUTH-11 RED, JUnit, native-identity, and immutable-boundary verification | `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests -q -k reviewtruth_preimplementation_chronology_all` | Independently re-prove all three maintainer comment identities/body bytes/body digests/semantic clauses, the strict Option-2 and parameterized prover/ordering contracts, non-authorizing `agent-harness#405`, Stage-B exact base/profile, both dependency review-wave roots/receipts, dedicated record and tests-only landings, and that only the eleven frozen tests/evidence paths changed. Require all 26 EC-REVIEWTRUTH-17/-18/-19 nodes plus all six PC-REVIEWTRUTH-11 nodes to remain RED at their exact anchors and no production implementation to exist. |

### SL-2 — Typed Seat Outcome, Lens Prompt, And Native-Fill Binding

- **Scope**: Publish IF-0-REVIEWTRUTH-1, IF-0-REVIEWTRUTH-3, IF-0-REVIEWTRUTH-4, and plan-local PC-REVIEWTRUTH-11 in the single-writer panel runtime: preserve the existing review read-only and non-review danger-full-access paths, add the explicit bounded write-capable review-wave roles, topology records, capability/sandbox schema, gate-aware acyclic evidence wave, coordinator-owned ephemeral worktree/data-scope/reaper seam, and universal host-vendor native-first selection while retaining the durable outcome/native-fill contracts.
...
- **Interfaces provided**: `IF-0-REVIEWTRUTH-1`, `IF-0-REVIEWTRUTH-3`, `IF-0-REVIEWTRUTH-4`, `PC-REVIEWTRUTH-11`, `HostContext`, `identify_native_host_seats()`, `NativeLegRoutingDisposition`, `ExecutionCapabilityAttestation`, `ReviewWavePolicy`, `ReviewWaveEvidence`, `invoke_review_wave()`, `CodexExternalToolPolicy`, `CodexEffectiveConfigReceipt` or equivalent strict effective-config subrecord, `CodexExternalToolPreflightReceipt`, `CodexEarlyProverLaunchReceipt`, `run_codex_external_tool_preflight()`, `EarlyProverWorktreeHandle`, `create_early_prover_worktree()`, `reap_early_prover_worktrees()`, `REVIEWTRUTH-prover-capability-attestation`, `AuthPreflightResult.metadata.execution_capability`, `Board.capability_attestations`, `PanelLegOutcome`, `PanelLegResult.outcome`, `PanelLegResult.required`, `PanelLegResult.degraded`, `PanelLegResult.prompt_lens_digest`, `PanelResult.reviewed_seat_count`, `SeatOutcomeRecord.degraded`, `REVIEWTRUTH-seat-outcome-degraded-lossless-reader`, `NativeAgentLegRequest.board_run_id`, `NativeAgentLegRequest.request_id`, `NativeAgentLegRequest.seat_instance_id`, `NativeAgentLegRequest.attempt_id`, `NativeAgentLegReport`, `NativeAgentLegBindingDisposition`, `bind_native_agent_leg_result()`
...
- **Tasks**:
  - test: SL2-T1 — Consume all landed outcome/native-fill tests plus the exact EC-REVIEWTRUTH-17, EC-REVIEWTRUTH-18, and PC-REVIEWTRUTH-11 sets; require all 28 relevant nodes RED at their frozen anchors before editing any of the six files.
  - impl: SL2-T2 — In `launcher.py`, add only an explicit early-prover Codex branch containing the exact PC-REVIEWTRUTH-8 argv and fail-closed `CodexExternalToolPolicy`/effective config/preflight/launch receipts, with both exclusion overrides true and `TMPDIR` canonical inside the already-authorized per-leg worktree/data scope; preserve existing review `read-only` and non-review `danger-full-access` branches. In `advisor_board/schema.py` and `composition.py`, add defaulted capability/sandbox/role fields without constructor breakage and never grant early evidence authority to bind. In `panel_invoker.py`, add `invoke_review_wave()` without changing `PANEL_LEGS` or legacy invocation/goldens; require a passing effective receipt before Codex, then after typed Codex preflight failure select Grok only through a separate positive OS-confinement receipt, otherwise stage a typed zero-Codex/zero-Grok degraded record. Enforce the acyclic receipt/bundle/seat chronology and complete restart rule, and preserve Gemini ineligibility. In `phase_worktree_executor.py`, consume SCHED's landed lifecycle implementation and extend it; require an external-coordinator-selected explicit `workspace_mount`; add one fail-closed preflight for canonicalization, non-symlink ancestry, coordinator ownership, private writable/searchable permissions, free bytes/inodes, source/Git-common-dir disjointness, and exact `lane_worktree_root` containment; pass that mount explicitly through `create_phase_worktree`; and extend the existing create/teardown utilities with per-leg lease/data-scope/mount/external-tool receipts, fresh ephemerality, and an external crash reaper. The coordinator creates the parent/worktree before the confined leg starts. Preserve `fab_gate.py`'s strict durable-reader/native-fill work.
  - impl: SL2-T2 topology arm — Implement IF-0-REVIEWTRUTH-4 in the existing board/schema/composition seams: Opus primary through the canonical host-relative Claude native-fill/subscription-TUI adapter, typed Codex fallback after zero-launch Opus failure, all-four-terminal barrier, and the sequential Fable-president transition. Claude Code uses the native Anthropic subagent rendezvous for every Claude-family seat; non-Claude hosts and explicit standalone mode use the authenticated subscription-TUI adapter, with no own-vendor TUI fallback under Claude Code. Preflight and bind Opus's exact model/auth and transport disposition, scrubbed subscription environment, `Read,Write`-only tools, zero Bash/shell/web/MCP authority, limited add-directory roots, per-leg worktree/data scope, explicit mount, and serial lease without adding a launcher branch. Run the Opus/Codex board role as the sole current write holder, and derive binding-prover usability only from the later grounded artifact-bound Fable result.
  - impl: SL2-T2 native-first arm — Extend `HostContext` additively with explicit native capability and closed host identity, add the ordered all-match `identify_native_host_seats()` selector, and classify every seat as native, adapter, or explicit standalone-adapter before launch. Generalize the existing request/report and exact-once binding surface across Codex/OpenAI, Claude/Anthropic, Gemini/Google, and Grok/xAI without weakening the existing identity/digest tuple. Reject a matching host seat when native capability is missing or false; never infer capability from PATH, auth, model availability, or environment; never launch that seat through an external adapter. Preserve nonmatching adapters and the bare phase-loop CLI entrypoint's explicit standalone all-adapter mode. Native transport does not alter role or capability policy.
  - verify: SL2-T3 — Prove every exact PC-REVIEWTRUTH-8 and PC-REVIEWTRUTH-9 node plus the four SL-2-owned PC-REVIEWTRUTH-11 runtime nodes GREEN, while the two SL-5-owned host-integration/skill-contract nodes remain pending until SL5-T3; inspect the emitted Codex argv, full frozen feature/MCP/plugin classification, zero-enabled effective state, both exclusions true in policy/effective config/preflight/launch receipts, contained `TMPDIR`, explicit mount preflight/pass-through, positive custom-`tmp_path` and safely available real-`/mnt/workspace` controls, rejected unusable mounts with zero leg launch, positive worktree/nested-`TMPDIR` writes, rejected `/tmp`/external-`TMPDIR`/live-tree/shared-data writes, worktree/data-scope/reaper receipts, acyclic review-wave digests, zero-launch degraded Codex/Grok records, Gemini ineligibility, all-four-terminal and sequential-Claude topology, Fable-president binding-prover derivation, universal runtime native-first routing, explicit standalone behavior, and contradiction-triggered complete re-review. Prove `can_probe` never satisfies `binding_prover`.

| Task ID | Type | Depends on | Files in scope | Tests owned | Test command | Work |
|---|---|---|---|---|---|---|
| SL2-T1 | test | SL-0, SL-1 | all six SL-2 owned files | consume landed EC-2 through EC-18 tests without editing, including the exact 16 PC-REVIEWTRUTH-8 nodes, six PC-REVIEWTRUTH-9 nodes, and six PC-REVIEWTRUTH-11 nodes | the literal EC-REVIEWTRUTH-17, EC-REVIEWTRUTH-18, and PC-REVIEWTRUTH-11 RED commands followed by existing forced-activation focused and golden commands | Before any SL-2 edit, prove all 28 relevant nodes fail only at their exact mapped anchors and retain their exact node/anchor digests. Confirm legacy review/non-review launcher paths, `PANEL_LEGS`, non-native adapters, explicit standalone topology, and goldens remain GREEN. |
| SL2-T2 | impl | SL2-T1 | all six SL-2 owned files | none | the literal EC-REVIEWTRUTH-17 and EC-REVIEWTRUTH-18 GREEN commands plus the four-node PC-REVIEWTRUTH-11 runtime GREEN command followed by existing focused and golden commands | Implement the exact file-by-file and topology contracts above. Reuse existing worktree lifecycle seams; the coordinator owns mount selection/preflight, parent/worktree creation, launch authorization, serial leases, digest binding, and the external reaper. Compose Opus primary or typed Codex fallback, wait for all four terminal outcomes, then invoke Fable sequentially and derive binding-prover usability from that president result. Classify all host-vendor matching seats through the typed native rendezvous and preserve only nonmatching or explicit standalone adapter legs. Leave the two SL-5-owned host/skill PC-REVIEWTRUTH-11 nodes for SL5-T3. |
| SL2-T3 | verify | SL2-T2 | all six SL-2 owned files | none | the EC-REVIEWTRUTH-17 and EC-REVIEWTRUTH-18 GREEN commands plus the four-node PC-REVIEWTRUTH-11 runtime GREEN command and existing marker-driven SL2 commands | Require all SL-2-focused commands GREEN. Prove the external-tool and mount closure, isolated serial holder and reaper, policy/config/preflight/receipt-bound evidence, gate ordering, Opus-primary/typed-Codex-fallback selection, all-four-terminal barrier, sequential Fable president, Fable-only binding, contradiction re-review, universal runtime native-first routing, explicit standalone behavior, and legacy non-native path/golden compatibility. The complete six-node PC-REVIEWTRUTH-11 set becomes GREEN only at SL5-T3. |
...
- **Interfaces provided**: owner-qualified `IF-0-REVIEWTRUTH-2` implementing the maintainer-named additive `IF-0-POLICY-1` contract, the typed policy-fact portion of `IF-0-REVIEWTRUTH-5`, `RatificationPolicy.required_prover`, `BoardFacts.prover_usable`, typed per-finding president dispositions, parameterized `gate_posture.resolve_ratification_policy`, `BoardDeliveryState`, `classify_board_delivery()`, `BoardFacts.delivery_state`, `BoardFacts.reviewed_seat_count`
...
- **Tasks**:
  ...
  - impl: SL4-T2 president-policy arm — Define strict exact-text-and-artifact-digest-bound `BLOCKING | DEFERRED` facts and non-deferrable classifications for IF-0-REVIEWTRUTH-5 without performing dispatch or issue I/O in the pure policy layer.
...
- **Scope**: Wire the frozen review truth and early-prover evidence-wave contract through gate-specific and train consumers, bounded production repair, canonical lifecycle/summary persistence, universal host-native rendezvous drivers, durable typed train approval/resume evidence, and the single immutable evidence executable used by both implementation-head GitHub CI and the post-landing reducer, then cross one distinct SL-2-through-SL-5 implementation PR/landing boundary.
- **Owned files**: `phase-loop-runtime/src/phase_loop_runtime/runner.py`, `phase-loop-runtime/src/phase_loop_runtime/governed_premerge.py`, `phase-loop-runtime/src/phase_loop_runtime/cli.py`, `phase-loop-runtime/src/phase_loop_runtime/train_runner.py`, `phase-loop-runtime/src/phase_loop_runtime/train_ledger.py`, `phase-loop-runtime/src/phase_loop_runtime/skill_install.py`, `phase-loop-runtime/src/phase_loop_runtime/skill_inventory.py`, `phase-loop-runtime/src/phase_loop_runtime/skill_paths.py`, `phase-loop-runtime/scripts/gate_a_cleanroom.sh`, `phase-loop-runtime/scripts/verify_reviewtruth_evidence.py`, `.github/workflows/test.yml`, `phase-loop-skills/README.md`, `phase-loop-skills/advisor-board/**`, `skills-src/*/*-advisor-board/**`, `skills-src/*/*-advisor-panel/**`, `phase-loop-runtime/src/phase_loop_runtime/skills_bundle/*-advisor-board/**`, `phase-loop-runtime/src/phase_loop_runtime/skills_bundle/*-advisor-panel/**`
- **Interfaces provided**: `IF-0-REVIEWTRUTH-5`, `REVIEWTRUTH-production-gates`, `REVIEWTRUTH-native-driver-contract`, `REVIEWTRUTH-seat-outcome-ledger`, `REVIEWTRUTH-panel-verdict-ledger`, `REVIEWTRUTH-train-approval-evidence`, `REVIEWTRUTH-production-apply-fix`, `REVIEWTRUTH-evidence-executable`, `REVIEWTRUTH-gate-a-suite-attestation`, `REVIEWTRUTH-implementation-landing`
- **Interfaces consumed**: `REVIEWTRUTH-capability-ratification`, `REVIEWTRUTH-tests-first-baseline`, `REVIEWTRUTH-preimplementation-chronology`, `REVIEWTRUTH-post-parser-collection-contract`, `REVIEWTRUTH-seat-outcome-degraded-lossless-reader`, `REVIEWTRUTH-native-fill-identity-contract`, `REVIEWTRUTH-early-prover-wave-contract`, `REVIEWTRUTH-golden-delta-baseline`, `REVIEWTRUTH-tdd-activation`, `REVIEWTRUTH-junit-contract`, `REVIEWTRUTH-broad-compatibility-gate`, `IF-0-REVIEWTRUTH-1`, `IF-0-REVIEWTRUTH-3`, `IF-0-REVIEWTRUTH-4`, `PC-REVIEWTRUTH-11`, `HostContext`, `identify_native_host_seats()`, `NativeLegRoutingDisposition`, `ReviewWaveEvidence`, `CodexExternalToolPolicy`, `CodexEffectiveConfigReceipt` or equivalent strict effective-config subrecord, `CodexExternalToolPreflightReceipt`, `CodexEarlyProverLaunchReceipt`, `NativeAgentLegRequest.board_run_id`, `NativeAgentLegRequest.request_id`, `NativeAgentLegRequest.seat_instance_id`, `NativeAgentLegRequest.attempt_id`, `NativeAgentLegReport`, `NativeAgentLegBindingDisposition`, `bind_native_agent_leg_result()`, `ReviewGrounding`, `GovernedBoardEvidence.grounded_reviewed_seats`, `review_material_issue()`, `BoardDeliveryState`, `classify_board_delivery()`
...
- **Tasks**:
  ...
  - impl: SL6-T2 — Write only the owned pre-final evidence docs and contract/card updates while consuming the immutable evidence executable. Bind all three exact maintainer directives, both frozen Stage-A snapshot digests and their regenerated manifest identities, the separately reviewed Stage-B plan digest, the Stage-B exact base/profile and frozen Codex executable/version/help/feature/MCP/plugin facts, all eleven tests-only path digests, external-tool/confinement/worktree/data-scope/reaper receipts, gate-order records, board bundle digests, contradiction invalidations, every per-finding president disposition, and the distinct usable grounded artifact-bound Fable binding result. Run closeout under EC-REVIEWTRUTH-17 through EC-REVIEWTRUTH-19; no direct `claude -p`.
  - verify: SL6-T3 — Preserve the acyclic final XML/record/Gate-A order and additionally fail on any missing early-prover initial wave, wrong gate order, counted critic without the required evidence digest, unknown/enabled/locked/changed/uninspectable Codex external surface, either false/missing temporary exclusion, `/tmp` or external-`TMPDIR` authority, any Codex launch after failed Codex preflight, unconfined Grok launch, Gemini write selection, Codex-as-binding substitution, stale `AGREE`, missing re-review, or any policy/config/preflight/launch-receipt/worktree/data-scope/reaper/digest gap.

Any pre-amendment panel roster, order, chair taxonomy, or round-limit wording retained in the SL-6
task table is superseded by EC-REVIEWTRUTH-16 through EC-REVIEWTRUTH-19 and is not operational.
...
- The coordinated run retains its bootstrap no-degraded-promotion interlock under EC-REVIEWTRUTH-16 through EC-REVIEWTRUTH-19. Closeout binds the exact finalized evidence record digest, the `final-record` attestation, and the terminal Gate A suite attestation, and writes its result only to the canonical ledger. The runtime implementation may represent FLOOR-ONLY and follow an explicit downstream policy, but this phase cannot use its own new degraded semantics to waive the review that authorizes it.
...
- Documentation impact: SL-6 updates the normative board contract and capability card for universal host-vendor native-first routing, explicit standalone all-adapter topology, exact-once native request/report identity, and the independently retained Fable-only binding-prover boundary. `phase-loop-skills/README.md` is an SL-5 implementation-owned installation/usage surface because Grok becomes an explicit supported harness target. Record `no_doc_delta` for the repository root `README.md`, `CHANGELOG.md`, packaging dependency declarations, env examples, migrations, and release notes because REVIEWTRUTH changes no public release/package version. The newly tracked `phase-loop-runtime/uv.lock` is instead this plan repair's frozen bootstrap prerequisite and is complete before SL-0; it is not an SL-6 documentation delta or a later REVIEWTRUTH implementation change.
...
VERDICT = re.compile(r"(PARTIALLY AGREE|DISAGREE|AGREE)")
PROBE_TERMINAL = re.compile(r"PROBE_STATUS: (CLEAR|BLOCKED)")
EVIDENCE_REF_FAMILY = re.compile(r"EVIDENCE_REF\s*:", re.IGNORECASE)
ELISION_MARKER = re.compile(
    r"(?:\[\s*elided\s*\]|<\s*elided\s*>|\b(?:elided|omitted)\b|(?:^|\s)\.\.\.(?:\s|$))",
    re.IGNORECASE,
)
LEADING_MARKUP = re.compile(r"^(?:[-+*> \t`#]+|\d+[.)][ \t]*)+")
CANONICAL_JSON = "utf8-sorted-keys-compact-lf.v1"
GATES = ("plan-ratify", "design-ratify", "pre-merge-CR", "release-dispatch")
POLICY_KEYS = {"on_shortfall", "required_consensus", "required_lens_coverage", "required_prover", "required_vendors"}
STATUS_OUTCOME = {
    "OK": "reviewed",
...
FORBIDDEN_LINE_SEPARATORS = ("\x00", "\r", "\v", "\f", "\x1c", "\x1d", "\x1e", "\x85", "\u2028", "\u2029")
...
def canonical_absolute_path(value, label):
    path = text(value, label)
    need(path.startswith("/") and os.path.normpath(path) == path, f"{label}: canonical absolute path")
    return path


def strict_descendant(value, parent, label):
    path = canonical_absolute_path(value, label)
    need(path != parent and os.path.commonpath((path, parent)) == parent, f"{label}: containment")
    return path
...
def strict_lf_lines(value, label):
    need(type(value) is str, f"{label}: text")
    need(value.endswith("\n"), f"{label}: terminal LF")
    need(not any(separator in value for separator in FORBIDDEN_LINE_SEPARATORS), f"{label}: forbidden line separator")
    return value.split("\n")


def has_substantive_body(value):
    visible = "".join(
        character for character in value
        if not character.isspace() and unicodedata.category(character)[0] not in {"C", "Z"}
    )
    return len(value.encode("utf-8")) >= 40 and len(visible.encode("utf-8")) >= 40 and sum(character.isalnum() for character in visible) >= 8
...
def validate_rendered_prompt(reference, root, *, lens, seat_instance_id, bundle_digest, required_evidence, label):
    ...
    try:
        ...
        fail(f"{label}: utf8")
    lines = strict_lf_lines(prompt, label)
    normalized_lines = [
        LEADING_MARKUP.sub("", line.strip(" \t")).strip(" \t") for line in lines
    ]
    marker_groups = {
    ...
    for prefix, expected in marker_groups.items():
        family = re.compile(
            rf"{re.escape(prefix.removesuffix(':'))}\s*:", re.IGNORECASE
        )
        actual = [
            raw for raw, normalized in zip(lines, normalized_lines)
            if family.match(normalized)
        ]
...
def validate_base(base, expected, root, label):
    ...
    for artifact_name in ("final_audit", "final_evidence"):
        artifact_ref = file_ref(base[artifact_name], f"{label}.{artifact_name}")
        artifact_bytes, _ = root.read_ref(artifact_ref, f"{label}.{artifact_name}")
        need(artifact_ref["bytes"] > 0 and len(artifact_bytes) > 0, f"{label}.{artifact_name}: empty final artifact")
...
def validate_finding(reference, root, forbidden, label):
    ...
    return reported_at, contradiction, material


def evidence_refs_from_text(value, label):
    references = []
    for index, line in enumerate(strict_lf_lines(value, label)):
        normalized = LEADING_MARKUP.sub("", line.strip(" \t")).strip(" \t")
        if EVIDENCE_REF_FAMILY.match(normalized) is None:
            continue
        need(line == line.strip(" \t") and line.startswith("EVIDENCE_REF: "), f"{label}: malformed evidence ref marker")
        encoded = line[len("EVIDENCE_REF: "):]
...
def review_text_facts(value, label):
    lines = [line for line in strict_lf_lines(value, label) if line.strip(" \t")]
    if not lines:
        return None, False, []
    terminal = LEADING_MARKUP.sub("", lines[-1].strip(" \t")).strip(" \t").strip("*`").strip(" \t")
    if terminal.startswith("VERDICT:"):
...
def probe_text_facts(value, label):
    ...
    if not lines:
        return None, False, []
    match = PROBE_TERMINAL.fullmatch(lines[-1])
...
def validate_policy(policy, seats, root, forbidden, final, repo, expected_base, bundle_entries, label):
    exact_keys(policy, {"gate", "on_shortfall", "required_consensus", "required_lens_coverage", "required_prover", "required_vendors", "resolver_input", "resolver_output"}, label)
    ...
    exact_keys(resolver_input, {"schema", "defaults", "gate", "repository_manifest", "required_prover_false_probe"}, f"{label}.resolver_input")
    ...
    manifest_ref = file_ref(resolver_input["repository_manifest"], f"{label}.repository_manifest")
    need(manifest_ref["path"] == "policy/repository-manifest.source", f"{label}: repository manifest path")
    need(manifest_ref in bundle_entries, f"{label}: repository manifest not staged")
    manifest_bytes, manifest_digest = root.read_ref(manifest_ref, f"{label}.repository_manifest")
    audited_manifest_bytes = git(repo, "show", f'{expected_base["commit"]}:.consiliency/manifest.json')
    need(manifest_bytes == audited_manifest_bytes, f"{label}: repository manifest is not audited M")
    repository_manifest = load_json_bytes(manifest_bytes, f"{label}.repository_manifest")
    need(type(repository_manifest) is dict, f"{label}: repository manifest object")
    ...
    for gate in GATES:
        ...
        need((current["required_consensus"], current["on_shortfall"]) == expected_modes[gate], f"{label}.{gate}: retained policy")
    raw_overrides = repository_manifest.get("ratification_policy_overrides")
    if raw_overrides is None:
        override = {}
    else:
        need(type(raw_overrides) is dict, f"{label}: repository override registry")
        gate_override = raw_overrides.get("pre-merge-CR")
        if gate_override is None:
            override = {}
        else:
            need(type(gate_override) is dict, f"{label}: repository gate override")
            need(set(gate_override) <= POLICY_KEYS, f"{label}: repository override keys")
            override = dict(gate_override)
    for key, value in override.items():
        ...
        elif key == "required_consensus":
            need(value in {"majority", "unanimous"}, f"{label}.override.required_consensus")
        else:
    ...
    reject_forbidden(repository_manifest, forbidden | {manifest_digest}, f"{label}.repository_manifest")
    need(policy["gate"] == "pre-merge-CR", f"{label}: gate")
    for key in ("on_shortfall", "required_consensus", "required_lens_coverage", "required_prover", "required_vendors"):
    ...
    consensus = agreeing == reviewing and reviewing > 0 if effective["required_consensus"] == "unanimous" else agreeing * 2 > reviewing
    fable = seats[3]
    prover = fable["binding_prover"] and fable["usable"] and fable["outcome"] == "reviewed" and fable["verdict"] == "AGREE"
    if final:
        if effective["on_shortfall"] == "escalate":
...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, repo, final, label):
    ...
    early_required_entries = []
    degraded_early = early_artifact["schema"] == "v10.review-early-prover-degraded-artifact.v1"
    if not degraded_early:
        early_artifact_keys = {"attempt", "base", "binding_prover", "capability", "codex_launch_receipt", "codex_preflight_receipt", "completed_at", "effort", "execution_attestation", "grounding", "harness", "lens", "model", "native_result", "outcome", "phase_alias", "position", "probe_report", "rendered_prompt", "role", "schema", "seat", "seat_instance_id", "started_at", "status", "vendor", "wave_id"}
        ...
        launched_preflight_ref = file_ref(early_artifact["codex_preflight_receipt"], f"{label}.early.launched.preflight")
        launched_preflight, launched_preflight_digest = root.json_ref(launched_preflight_ref, f"{label}.early.launched.preflight")
        launched_preflight_keys = {"agent_launch_count", "attempt", "base", "completed_at", "effective_config", "failure_reasons", "phase_alias", "policy", "producer", "schema", "source_preflight", "source_prelaunch_recheck", "started_at", "verdict", "wave_id"}
        exact_keys(launched_preflight, launched_preflight_keys, f"{label}.early.launched.preflight")
        need(launched_preflight["schema"] == "v10.review-early-prover-launched-preflight.v1", f"{label}: launched preflight schema")
        need(launched_preflight["producer"] == "phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight", f"{label}: launched preflight producer")
        need((launched_preflight["attempt"], launched_preflight["base"], launched_preflight["phase_alias"], launched_preflight["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: launched preflight wave binding")
        need(launched_preflight["verdict"] == "PASS" and launched_preflight["failure_reasons"] == [], f"{label}: launched preflight PASS")
        need(integer(launched_preflight["agent_launch_count"], f"{label}.early.launched.preflight.agent_launch_count", 0) == 0, f"{label}: launched preflight before process")

        launched_policy_ref = file_ref(launched_preflight["policy"], f"{label}.early.launched.policy")
        launched_policy, launched_policy_digest = root.json_ref(launched_policy_ref, f"{label}.early.launched.policy")
        exact_keys(launched_policy, {"created_at", "external_feature_denylist", "network_access", "producer", "sandbox", "schema", "workspace_write"}, f"{label}.early.launched.policy")
        need(launched_policy["schema"] == "v10.codex-external-tool-policy.v1" and launched_policy["producer"] == "phase_loop_runtime.panel_invoker.build_codex_external_tool_policy", f"{label}: launched policy producer")
        need(launched_policy["sandbox"] == "workspace-write" and boolean(launched_policy["network_access"], f"{label}.early.launched.policy.network_access") is False, f"{label}: launched policy sandbox")
        launched_workspace_write = launched_policy["workspace_write"]
        exact_keys(launched_workspace_write, {"exclude_slash_tmp", "exclude_tmpdir_env_var", "writable_roots"}, f"{label}.early.launched.policy.workspace_write")
        need(boolean(launched_workspace_write["exclude_slash_tmp"], f"{label}.early.launched.policy.exclude_slash_tmp") is True, f"{label}: launched policy slash tmp")
        need(boolean(launched_workspace_write["exclude_tmpdir_env_var"], f"{label}.early.launched.policy.exclude_tmpdir_env_var") is True, f"{label}: launched policy TMPDIR")
        need(launched_workspace_write["writable_roots"] == [], f"{label}: launched policy writable roots")
        launched_denylist = launched_policy["external_feature_denylist"]
        need(type(launched_denylist) is list and launched_denylist == sorted(set(launched_denylist)) and len(launched_denylist) > 0, f"{label}: launched policy denylist")
        for index, item in enumerate(launched_denylist):
            text(item, f"{label}.early.launched.policy.denylist[{index}]")
        launched_policy_created = timestamp(launched_policy["created_at"], f"{label}.early.launched.policy.created_at")

        launched_config_ref = file_ref(launched_preflight["effective_config"], f"{label}.early.launched.effective_config")
        launched_config, launched_config_digest = root.json_ref(launched_config_ref, f"{label}.early.launched.effective_config")
        exact_keys(launched_config, {"external_feature_denylist", "mcp_enabled_count", "network_access", "plugin_enabled_count", "policy", "producer", "recorded_at", "sandbox", "schema", "workspace_write"}, f"{label}.early.launched.effective_config")
        need(launched_config["schema"] == "v10.codex-effective-config-receipt.v1" and launched_config["producer"] == "phase_loop_runtime.panel_invoker.read_codex_effective_config" and launched_config["policy"] == launched_policy_ref, f"{label}: launched config binding")
        need(launched_config["sandbox"] == launched_policy["sandbox"] and launched_config["network_access"] is launched_policy["network_access"], f"{label}: launched config sandbox")
        need(launched_config["workspace_write"] == launched_workspace_write and launched_config["external_feature_denylist"] == launched_denylist, f"{label}: launched config policy mirror")
        need(integer(launched_config["mcp_enabled_count"], f"{label}.early.launched.config.mcp_enabled_count", 0) == 0, f"{label}: launched config MCP disabled")
        need(integer(launched_config["plugin_enabled_count"], f"{label}.early.launched.config.plugin_enabled_count", 0) == 0, f"{label}: launched config plugins disabled")
        launched_config_recorded = timestamp(launched_config["recorded_at"], f"{label}.early.launched.config.recorded_at")
        need(launched_policy_created <= launched_config_recorded, f"{label}: launched policy/config chronology")

        def launched_source_preflight(reference, source_label):
            source, source_digest = root.json_ref(reference, source_label)
            source_keys = {"agent_launch_count", "completed_at", "effective_config", "failure_reasons", "policy", "producer", "schema", "started_at", "state_sha256", "verdict"}
            exact_keys(source, source_keys, source_label)
            need(source["schema"] == "v10.codex-external-tool-preflight.v1" and source["producer"] == "phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight", f"{source_label}: producer")
            need(source["policy"] == launched_policy_ref and source["effective_config"] == launched_config_ref, f"{source_label}: policy/config binding")
            need(source["verdict"] == "PASS" and source["failure_reasons"] == [], f"{source_label}: PASS")
            need(integer(source["agent_launch_count"], f"{source_label}.agent_launch_count", 0) == 0, f"{source_label}: zero launch")
            need(type(source["state_sha256"]) is str and SHA64.fullmatch(source["state_sha256"]), f"{source_label}: state digest")
            started = timestamp(source["started_at"], f"{source_label}.started_at")
            completed = timestamp(source["completed_at"], f"{source_label}.completed_at")
            need(started < completed, f"{source_label}: chronology")
            return source, source_digest, started, completed

        source_preflight_ref = file_ref(launched_preflight["source_preflight"], f"{label}.early.launched.source_preflight")
        source_recheck_ref = file_ref(launched_preflight["source_prelaunch_recheck"], f"{label}.early.launched.source_prelaunch_recheck")
        source_preflight, source_preflight_digest, source_preflight_started, source_preflight_completed = launched_source_preflight(source_preflight_ref, f"{label}.early.launched.source_preflight")
        source_recheck, source_recheck_digest, source_recheck_started, source_recheck_completed = launched_source_preflight(source_recheck_ref, f"{label}.early.launched.source_prelaunch_recheck")
        need(source_preflight["state_sha256"] == source_recheck["state_sha256"], f"{label}: launched preflight stable state")
        need(launched_config_recorded <= source_preflight_started < source_preflight_completed <= source_recheck_started < source_recheck_completed, f"{label}: launched preflight chronology")
        need(launched_preflight["started_at"] == source_preflight["started_at"] and launched_preflight["completed_at"] == source_recheck["completed_at"], f"{label}: launched compact preflight chronology")

        launched_launch_ref = file_ref(early_artifact["codex_launch_receipt"], f"{label}.early.launched.launch")
        launched_launch, launched_launch_digest = root.json_ref(launched_launch_ref, f"{label}.early.launched.launch")
        launched_launch_keys = {"attempt", "base", "codex_process_count", "completed_at", "data_scope", "launched", "phase_alias", "preflight", "producer", "reaper", "schema", "source_launch", "started_at", "workspace_mount", "worktree", "wave_id"}
        exact_keys(launched_launch, launched_launch_keys, f"{label}.early.launched.launch")
        need(launched_launch["schema"] == "v10.review-early-prover-launched-launch.v1", f"{label}: launched launch schema")
        need(launched_launch["producer"] == "phase_loop_runtime.phase_worktree_executor.bind_launch_evidence", f"{label}: launched launch producer")
        need((launched_launch["attempt"], launched_launch["base"], launched_launch["phase_alias"], launched_launch["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: launched launch wave binding")
        need(launched_launch["preflight"] == launched_preflight_ref, f"{label}: launched launch preflight binding")
        need(boolean(launched_launch["launched"], f"{label}.early.launched.launch.launched") is True, f"{label}: launched process absent")
        need(integer(launched_launch["codex_process_count"], f"{label}.early.launched.launch.codex_process_count", 1) == 1, f"{label}: launched process count")

        mount_ref = file_ref(launched_launch["workspace_mount"], f"{label}.early.launched.workspace_mount")
        mount, mount_digest = root.json_ref(mount_ref, f"{label}.early.launched.workspace_mount")
        mount_keys = {"attempt", "base", "canonical_path", "checked_at", "device", "mode", "owner_uid", "phase_alias", "producer", "safe", "schema", "wave_id"}
        exact_keys(mount, mount_keys, f"{label}.early.launched.workspace_mount")
        need(mount["schema"] == "v10.codex-workspace-mount-receipt.v1" and mount["producer"] == "phase_loop_runtime.phase_worktree_executor.preflight_workspace_mount", f"{label}: launched mount producer")
        need((mount["attempt"], mount["base"], mount["phase_alias"], mount["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: launched mount wave binding")
        need(boolean(mount["safe"], f"{label}.early.launched.mount.safe") is True, f"{label}: launched mount unsafe")
        mount_path = canonical_absolute_path(mount["canonical_path"], f"{label}.early.launched.mount.path")
        integer(mount["device"], f"{label}.early.launched.mount.device")
        integer(mount["owner_uid"], f"{label}.early.launched.mount.owner_uid")
        need(type(mount["mode"]) is str and re.fullmatch(r"0[0-7]{3}", mount["mode"]) and int(mount["mode"], 8) & 0o022 == 0, f"{label}: launched mount private mode")
        mount_checked = timestamp(mount["checked_at"], f"{label}.early.launched.mount.checked_at")

        worktree_ref = file_ref(launched_launch["worktree"], f"{label}.early.launched.worktree")
        worktree, worktree_digest = root.json_ref(worktree_ref, f"{label}.early.launched.worktree")
        worktree_keys = {"attempt", "base", "cleanup_owner", "created_at", "detached", "ephemeral", "head", "mount", "path", "phase_alias", "producer", "schema", "serial_holder", "tree", "wave_id"}
        exact_keys(worktree, worktree_keys, f"{label}.early.launched.worktree")
        need(worktree["schema"] == "v10.codex-ephemeral-worktree-receipt.v1" and worktree["producer"] == "phase_loop_runtime.phase_worktree_executor.create_worktree", f"{label}: launched worktree producer")
        need((worktree["attempt"], worktree["base"], worktree["phase_alias"], worktree["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: launched worktree wave binding")
        need(worktree["mount"] == mount_ref and worktree["head"] == receipt["base"]["commit"] and worktree["tree"] == receipt["base"]["tree"], f"{label}: launched worktree base binding")
        need(boolean(worktree["detached"], f"{label}.early.launched.worktree.detached") is True and boolean(worktree["ephemeral"], f"{label}.early.launched.worktree.ephemeral") is True and boolean(worktree["serial_holder"], f"{label}.early.launched.worktree.serial_holder") is True, f"{label}: launched worktree isolation")
        need(worktree["cleanup_owner"] == "external_coordinator_reaper", f"{label}: launched worktree cleanup owner")
        worktree_path = strict_descendant(worktree["path"], mount_path, f"{label}.early.launched.worktree.path")
        worktree_created = timestamp(worktree["created_at"], f"{label}.early.launched.worktree.created_at")
        need(mount_checked <= worktree_created, f"{label}: launched mount/worktree chronology")

        data_ref = file_ref(launched_launch["data_scope"], f"{label}.early.launched.data_scope")
        data_scope, data_digest = root.json_ref(data_ref, f"{label}.early.launched.data_scope")
        data_keys = {"attempt", "base", "created_at", "database_policy", "path", "phase_alias", "producer", "schema", "shared_database_access", "wave_id", "worktree"}
        exact_keys(data_scope, data_keys, f"{label}.early.launched.data_scope")
        need(data_scope["schema"] == "v10.codex-data-scope-receipt.v1" and data_scope["producer"] == "phase_loop_runtime.phase_worktree_executor.create_data_scope", f"{label}: launched data-scope producer")
        need((data_scope["attempt"], data_scope["base"], data_scope["phase_alias"], data_scope["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: launched data-scope wave binding")
        need(data_scope["worktree"] == worktree_ref and data_scope["database_policy"] == "no_shared_database_access_environment_scrubbed", f"{label}: launched data-scope binding")
        need(boolean(data_scope["shared_database_access"], f"{label}.early.launched.data_scope.shared_database_access") is False, f"{label}: launched shared database access")
        strict_descendant(data_scope["path"], worktree_path, f"{label}.early.launched.data_scope.path")
        data_created = timestamp(data_scope["created_at"], f"{label}.early.launched.data_scope.created_at")
        need(worktree_created <= data_created, f"{label}: launched worktree/data chronology")

        reaper_ref = file_ref(launched_launch["reaper"], f"{label}.early.launched.reaper")
        reaper, reaper_digest = root.json_ref(reaper_ref, f"{label}.early.launched.reaper")
        reaper_keys = {"attempt", "base", "cleanup_status", "completed_at", "external", "leg_cleanup_authority", "phase_alias", "producer", "registered_at", "schema", "wave_id", "worktree"}
        exact_keys(reaper, reaper_keys, f"{label}.early.launched.reaper")
        need(reaper["schema"] == "v10.codex-external-reaper-receipt.v1" and reaper["producer"] == "phase_loop_runtime.phase_worktree_reaper", f"{label}: launched reaper producer")
        need((reaper["attempt"], reaper["base"], reaper["phase_alias"], reaper["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: launched reaper wave binding")
        need(reaper["worktree"] == worktree_ref and reaper["cleanup_status"] == "removed", f"{label}: launched reaper binding")
        need(boolean(reaper["external"], f"{label}.early.launched.reaper.external") is True and boolean(reaper["leg_cleanup_authority"], f"{label}.early.launched.reaper.leg_cleanup_authority") is False, f"{label}: launched external reaper authority")
        reaper_registered = timestamp(reaper["registered_at"], f"{label}.early.launched.reaper.registered_at")
        reaper_completed = timestamp(reaper["completed_at"], f"{label}.early.launched.reaper.completed_at")

        source_launch_ref = file_ref(launched_launch["source_launch"], f"{label}.early.launched.source_launch")
        source_launch, source_launch_digest = root.json_ref(source_launch_ref, f"{label}.early.launched.source_launch")
        source_launch_keys = {"codex_process_count", "completed_at", "data_scope", "launched", "preflight", "producer", "reaper", "sandbox", "schema", "started_at", "workspace_mount", "worktree"}
        exact_keys(source_launch, source_launch_keys, f"{label}.early.launched.source_launch")
        need(source_launch["schema"] == "v10.codex-early-prover-launch-receipt.v1" and source_launch["producer"] == "phase_loop_runtime.phase_worktree_executor" and source_launch["sandbox"] == "workspace-write", f"{label}: launched source launch producer")
        need(source_launch["preflight"] == launched_preflight_ref and source_launch["workspace_mount"] == mount_ref and source_launch["worktree"] == worktree_ref and source_launch["data_scope"] == data_ref and source_launch["reaper"] == reaper_ref, f"{label}: launched source launch bindings")
        need(boolean(source_launch["launched"], f"{label}.early.launched.source_launch.launched") is True and integer(source_launch["codex_process_count"], f"{label}.early.launched.source_launch.codex_process_count", 1) == 1, f"{label}: launched source process")
        need(source_launch["started_at"] == launched_launch["started_at"] and source_launch["completed_at"] == launched_launch["completed_at"], f"{label}: launched source times")
        launch_started = timestamp(source_launch["started_at"], f"{label}.early.launched.source_launch.started_at")
        launch_completed = timestamp(source_launch["completed_at"], f"{label}.early.launched.source_launch.completed_at")
        need(source_recheck_completed <= launch_started < launch_completed, f"{label}: launched preflight/process chronology")
        need(data_created <= reaper_registered < launch_started and launch_completed < reaper_completed, f"{label}: launched isolation/reaper chronology")
        ...
        launched_refs = [early_artifact_ref, launched_preflight_ref, launched_policy_ref, launched_config_ref, source_preflight_ref, source_recheck_ref, launched_launch_ref, source_launch_ref, mount_ref, worktree_ref, data_ref, reaper_ref, early_execution_ref, early_native_ref, early_prompt_ref]
        launched_digests = [early_artifact_digest, launched_preflight_digest, launched_policy_digest, launched_config_digest, source_preflight_digest, source_recheck_digest, launched_launch_digest, source_launch_digest, mount_digest, worktree_digest, data_digest, reaper_digest, early_execution_digest, early_native_digest, early_prompt_digest]
        need(len({reference["path"] for reference in launched_refs}) == len(launched_refs), f"{label}: early producer artifact alias")
        need(len(set(launched_digests)) == len(launched_digests), f"{label}: early producer digest alias")
        ...
        need(early_started == launch_started and early_completed == launch_completed, f"{label}: early native/launch chronology")
        ...
        early_usable = early_native["status"] == "OK" and probe_status == "CLEAR" and probe_substantive and native_early_grounding == required_early_evidence
        need(early_usable, f"{label}: early CLEAR")
        ...
        need(early_artifact["outcome"] == "CLEAR", f"{label}: early artifact derived outcome")
        derived_early = {
        ...
        early_ready_at = reaper_completed
        early_required_entries = [launched_preflight_ref, launched_policy_ref, launched_config_ref, source_preflight_ref, source_recheck_ref, launched_launch_ref, source_launch_ref, mount_ref, worktree_ref, data_ref, reaper_ref, early_execution_ref, early_native_ref, early_prompt_ref]
        for value, value_digest, name in (
            (launched_preflight, launched_preflight_digest, "launched_preflight"),
            (launched_policy, launched_policy_digest, "policy"),
            (launched_config, launched_config_digest, "effective_config"),
            (source_preflight, source_preflight_digest, "source_preflight"),
            (source_recheck, source_recheck_digest, "source_prelaunch_recheck"),
            (launched_launch, launched_launch_digest, "launched_launch"),
            (source_launch, source_launch_digest, "source_launch"),
            (mount, mount_digest, "workspace_mount"),
            (worktree, worktree_digest, "worktree"),
            (data_scope, data_digest, "data_scope"),
            (reaper, reaper_digest, "reaper"),
            (early_execution, early_execution_digest, "execution_attestation"),
            (early_native, early_native_digest, "native_result"),
        ):
            reject_forbidden(value, forbidden | {value_digest}, f"{label}.early.launched.{name}")
    else:
        degraded_keys = {"attempt", "base", "binding_prover", "capability", "codex_launch_receipt", "codex_preflight_receipt", "completed_at", "grok_fallback_decision", "outcome", "phase_alias", "position", "role", "schema", "seat", "started_at", "usable", "vendor", "wave_id"}
        exact_keys(early_artifact, degraded_keys, f"{label}.early.degraded_artifact")
        degraded_identity = (early_artifact["position"], early_artifact["seat"], early_artifact["vendor"], early_artifact["role"], early_artifact["capability"], early_artifact["binding_prover"], early_artifact["usable"], early_artifact["outcome"])
        need(degraded_identity == (0, "codex", "openai", "early_prover", "can_probe", False, False, "DEGRADED_NO_LAUNCH"), f"{label}: degraded early identity")
        integer(early_artifact["position"], f"{label}.early.degraded.position", 0)
        boolean(early_artifact["binding_prover"], f"{label}.early.degraded.binding")
        boolean(early_artifact["usable"], f"{label}.early.degraded.usable")

        preflight_ref = file_ref(early_artifact["codex_preflight_receipt"], f"{label}.early.degraded.preflight")
        preflight, preflight_digest = root.json_ref(preflight_ref, f"{label}.early.degraded.preflight")
        preflight_keys = {"agent_launch_count", "attempt", "base", "completed_at", "effective_config", "failure_reasons", "phase_alias", "policy", "producer", "schema", "source_preflight", "started_at", "verdict", "wave_id"}
        exact_keys(preflight, preflight_keys, f"{label}.early.degraded.preflight")
        need(preflight["schema"] == "v10.review-early-prover-degraded-preflight.v1", f"{label}: degraded preflight schema")
        need(preflight["producer"] == "phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight", f"{label}: degraded preflight producer")
        need((preflight["attempt"], preflight["base"], preflight["phase_alias"], preflight["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: degraded preflight wave binding")
        need(preflight["verdict"] == "FAIL", f"{label}: degraded preflight verdict")
        need(integer(preflight["agent_launch_count"], f"{label}.early.degraded.agent_launch_count", 0) == 0, f"{label}: degraded preflight zero launch")
        reasons = preflight["failure_reasons"]
        need(type(reasons) is list and reasons == sorted(set(reasons)) and len(reasons) > 0, f"{label}: degraded preflight reasons")
        for index, reason in enumerate(reasons):
            text(reason, f"{label}.early.degraded.failure_reasons[{index}]")
        source_preflight_ref = file_ref(preflight["source_preflight"], f"{label}.early.degraded.source_preflight")
        source_preflight, source_preflight_digest = root.json_ref(source_preflight_ref, f"{label}.early.degraded.source_preflight")
        policy_ref = file_ref(preflight["policy"], f"{label}.early.degraded.policy")
        policy, policy_digest = root.json_ref(policy_ref, f"{label}.early.degraded.policy")
        exact_keys(policy, {"created_at", "external_feature_denylist", "network_access", "producer", "sandbox", "schema", "workspace_write"}, f"{label}.early.degraded.policy")
        need(policy["schema"] == "v10.codex-external-tool-policy.v1" and policy["producer"] == "phase_loop_runtime.panel_invoker.build_codex_external_tool_policy", f"{label}: degraded policy producer")
        need(policy["sandbox"] == "workspace-write" and boolean(policy["network_access"], f"{label}.early.degraded.policy.network_access") is False, f"{label}: degraded policy sandbox")
        degraded_workspace_write = policy["workspace_write"]
        exact_keys(degraded_workspace_write, {"exclude_slash_tmp", "exclude_tmpdir_env_var", "writable_roots"}, f"{label}.early.degraded.policy.workspace_write")
        need(boolean(degraded_workspace_write["exclude_slash_tmp"], f"{label}.early.degraded.policy.exclude_slash_tmp") is True, f"{label}: degraded policy slash tmp")
        need(boolean(degraded_workspace_write["exclude_tmpdir_env_var"], f"{label}.early.degraded.policy.exclude_tmpdir_env_var") is True, f"{label}: degraded policy TMPDIR")
        need(degraded_workspace_write["writable_roots"] == [], f"{label}: degraded policy writable roots")
        degraded_denylist = policy["external_feature_denylist"]
        need(type(degraded_denylist) is list and degraded_denylist == sorted(set(degraded_denylist)) and len(degraded_denylist) > 0, f"{label}: degraded policy denylist")
        for index, item in enumerate(degraded_denylist):
            text(item, f"{label}.early.degraded.policy.denylist[{index}]")
        policy_created = timestamp(policy["created_at"], f"{label}.early.degraded.policy.created_at")

        config_ref = file_ref(preflight["effective_config"], f"{label}.early.degraded.effective_config")
        config, config_digest = root.json_ref(config_ref, f"{label}.early.degraded.effective_config")
        exact_keys(config, {"failure_reasons", "producer", "recorded_at", "schema", "status"}, f"{label}.early.degraded.effective_config")
        need(config["schema"] == "v10.codex-effective-config-receipt.v1" and config["producer"] == "phase_loop_runtime.panel_invoker.read_codex_effective_config", f"{label}: degraded config producer")
        need(config["status"] == "FAIL" and config["failure_reasons"] == reasons, f"{label}: degraded config failure binding")
        config_recorded = timestamp(config["recorded_at"], f"{label}.early.degraded.config.recorded_at")
        need(policy_created <= config_recorded, f"{label}: degraded policy/config chronology")

        source_preflight_keys = {"agent_launch_count", "completed_at", "effective_config", "failure_reasons", "policy", "producer", "schema", "started_at", "state_sha256", "verdict"}
        exact_keys(source_preflight, source_preflight_keys, f"{label}.early.degraded.source_preflight")
        need(source_preflight["schema"] == "v10.codex-external-tool-preflight.v1" and source_preflight["producer"] == "phase_loop_runtime.panel_invoker.run_codex_external_tool_preflight", f"{label}: degraded source preflight producer")
        need(source_preflight["policy"] == policy_ref and source_preflight["effective_config"] == config_ref, f"{label}: degraded source preflight policy/config binding")
        need(source_preflight["verdict"] == "FAIL" and source_preflight["failure_reasons"] == reasons, f"{label}: degraded source preflight facts")
        need(integer(source_preflight["agent_launch_count"], f"{label}.early.degraded.source_preflight.agent_launch_count", 0) == 0, f"{label}: degraded source preflight zero launch")
        need(type(source_preflight["state_sha256"]) is str and SHA64.fullmatch(source_preflight["state_sha256"]), f"{label}: degraded source preflight state digest")
        preflight_started = timestamp(source_preflight["started_at"], f"{label}.early.degraded.source_preflight.started_at")
        preflight_completed = timestamp(source_preflight["completed_at"], f"{label}.early.degraded.source_preflight.completed_at")
        need(config_recorded <= preflight_started < preflight_completed, f"{label}: degraded preflight chronology")
        need(preflight["started_at"] == source_preflight["started_at"] and preflight["completed_at"] == source_preflight["completed_at"], f"{label}: degraded compact preflight chronology")

        launch_ref = file_ref(early_artifact["codex_launch_receipt"], f"{label}.early.degraded.launch")
        launch, launch_digest = root.json_ref(launch_ref, f"{label}.early.degraded.launch")
        launch_keys = {"attempt", "base", "codex_process_count", "completed_at", "launched", "phase_alias", "preflight", "producer", "schema", "source_launch", "started_at", "wave_id"}
        exact_keys(launch, launch_keys, f"{label}.early.degraded.launch")
        need(launch["schema"] == "v10.review-early-prover-degraded-launch.v1", f"{label}: degraded launch schema")
        need(launch["producer"] == "phase_loop_runtime.phase_worktree_executor", f"{label}: degraded launch producer")
        need((launch["attempt"], launch["base"], launch["phase_alias"], launch["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: degraded launch wave binding")
        need(launch["preflight"] == preflight_ref, f"{label}: degraded launch preflight binding")
        need(boolean(launch["launched"], f"{label}.early.degraded.launched") is False, f"{label}: degraded Codex not launched")
        need(integer(launch["codex_process_count"], f"{label}.early.degraded.codex_process_count", 0) == 0, f"{label}: degraded Codex process count")
        source_launch_ref = file_ref(launch["source_launch"], f"{label}.early.degraded.source_launch")
        source_launch, source_launch_digest = root.json_ref(source_launch_ref, f"{label}.early.degraded.source_launch")
        source_launch_keys = {"codex_process_count", "completed_at", "launched", "preflight", "producer", "schema", "started_at"}
        exact_keys(source_launch, source_launch_keys, f"{label}.early.degraded.source_launch")
        need(source_launch["schema"] == "v10.codex-early-prover-launch-receipt.v1" and source_launch["producer"] == "phase_loop_runtime.phase_worktree_executor", f"{label}: degraded source launch producer")
        need(source_launch["preflight"] == preflight_ref and source_launch["launched"] is False and integer(source_launch["codex_process_count"], f"{label}.early.degraded.source_launch.codex_process_count", 0) == 0, f"{label}: degraded source launch facts")
        need(launch["started_at"] == source_launch["started_at"] and launch["completed_at"] == source_launch["completed_at"], f"{label}: degraded compact launch chronology")
        launch_started = timestamp(source_launch["started_at"], f"{label}.early.degraded.source_launch.started_at")
        launch_completed = timestamp(source_launch["completed_at"], f"{label}.early.degraded.source_launch.completed_at")
        need(preflight_completed <= launch_started < launch_completed, f"{label}: degraded launch chronology")

        grok_ref = file_ref(early_artifact["grok_fallback_decision"], f"{label}.early.degraded.grok")
        grok, grok_digest = root.json_ref(grok_ref, f"{label}.early.degraded.grok")
        grok_keys = {"attempt", "base", "codex_launch_receipt", "codex_preflight_receipt", "decided_at", "grok_process_count", "launched", "os_confinement_available", "phase_alias", "producer", "reason", "schema", "wave_id"}
        exact_keys(grok, grok_keys, f"{label}.early.degraded.grok")
        need(grok["schema"] == "v10.grok-early-prover-fallback-decision.v1", f"{label}: degraded Grok schema")
        need(grok["producer"] == "phase_loop_runtime.panel_invoker.invoke_review_wave", f"{label}: degraded Grok producer")
        need((grok["attempt"], grok["base"], grok["phase_alias"], grok["wave_id"]) == (attempt, receipt["base"], receipt["phase_alias"], receipt["wave_id"]), f"{label}: degraded Grok wave binding")
        need(grok["codex_preflight_receipt"] == preflight_ref and grok["codex_launch_receipt"] == launch_ref, f"{label}: degraded Grok Codex binding")
        need(boolean(grok["os_confinement_available"], f"{label}.early.degraded.os_confinement_available") is False, f"{label}: degraded Grok confinement unavailable")
        need(boolean(grok["launched"], f"{label}.early.degraded.grok_launched") is False, f"{label}: degraded Grok not launched")
        need(integer(grok["grok_process_count"], f"{label}.early.degraded.grok_process_count", 0) == 0, f"{label}: degraded Grok process count")
        need(grok["reason"] == "os_confinement_unavailable", f"{label}: degraded Grok reason")
        grok_decided = timestamp(grok["decided_at"], f"{label}.early.degraded.grok.decided_at")
        need(launch_completed < grok_decided, f"{label}: degraded Grok chronology")

        early_started = preflight_started
        early_completed = grok_decided
        early_ready_at = grok_decided
        need(early_artifact["started_at"] == preflight["started_at"] and early_artifact["completed_at"] == grok["decided_at"], f"{label}: degraded artifact chronology")
        need(early_artifact["codex_preflight_receipt"] == preflight_ref and early_artifact["codex_launch_receipt"] == launch_ref and early_artifact["grok_fallback_decision"] == grok_ref, f"{label}: degraded artifact producer binding")
        derived_early = {
            "artifact": early_artifact_ref,
            "binding_prover": False,
            "capability": "can_probe",
            "completed_at": grok["decided_at"],
        ...
        refs = [early_artifact_ref, preflight_ref, source_preflight_ref, policy_ref, config_ref, launch_ref, source_launch_ref, grok_ref]
        digests = [early_artifact_digest, preflight_digest, source_preflight_digest, policy_digest, config_digest, launch_digest, source_launch_digest, grok_digest]
        need(len({reference["path"] for reference in refs}) == len(refs), f"{label}: degraded producer artifact alias")
        need(len(set(digests)) == len(digests), f"{label}: degraded producer digest alias")
        early_required_entries = [preflight_ref, source_preflight_ref, policy_ref, config_ref, launch_ref, source_launch_ref, grok_ref]
        for value, digest, name in (
            (preflight, preflight_digest, "preflight"),
            (source_preflight, source_preflight_digest, "source_preflight"),
            (policy, policy_digest, "policy"),
            (config, config_digest, "effective_config"),
            (launch, launch_digest, "launch"),
            (source_launch, source_launch_digest, "source_launch"),
            (grok, grok_digest, "grok"),
        ):
            reject_forbidden(value, forbidden | {digest}, f"{label}.early.degraded.{name}")
    ...
    early_required_entries.extend([early["receipt"], early["artifact"]])
    ...
    need(early_ready_at < staged_at, f"{label}: early producers before staging")
    ...
    required_bundle_entries = [receipt["base"]["final_audit"], receipt["base"]["final_evidence"], *early_required_entries]
    for required in required_bundle_entries:
        need(required in entries, f"{label}: bundle required entry")
    if degraded_early:
        need(entries == sorted(required_bundle_entries, key=lambda reference: reference["path"]), f"{label}: degraded bundle exact entries")
    ...
    for index, (seat, expected) in enumerate(zip(seats, roster)):
        ...
        for finding_index, finding in enumerate(artifact["material_findings"]):
            ...
            finding_time, _, _ = validate_finding(ref, root, forbidden, f"{seat_label}.finding[{finding_index}]")
            need(started <= finding_time <= finished, f"{seat_label}.finding[{finding_index}]: producer chronology")
    ...
    effective_policy = validate_policy(receipt["effective_policy"], validated_seats, root, forbidden, final, repo, expected_base, entries, f"{label}.policy")
    if final:
        ...
        if effective_policy["required_prover"]:
    ...
    for index, item in enumerate(chain, start=1):
        ...
        metadata = validate_receipt(prior_receipt, expected_base, root, forbidden, prior_digest, repo, False, f"{label}.attempt[{index}]")
        ...
        finding_time, contradiction, material = validate_finding(finding_ref, root, forbidden, f"{label}.restart[{index}].finding")
        finding_from_prior_wave = finding_ref in metadata["finding_refs"]
        need(finding_from_prior_wave or contradiction, f"{label}.restart[{index}]: unbound finding")
        if finding_from_prior_wave:
            need(material, f"{label}.restart[{index}]: prior-wave finding is not material")
        else:
            need(metadata["completed_at"] < finding_time, f"{label}.restart[{index}]: external finding not after prior completion")
        ...
        prior_metadata.append((item, metadata, finding_time, finding_from_prior_wave))
    for index, (item, metadata, finding_time, finding_from_prior_wave) in enumerate(prior_metadata):
        ...
        if not finding_from_prior_wave:
            need(finding_time < next_metadata["early_started"], f"{label}.restart[{index + 1}]: external finding not before replacement prover")
...
def validate_completion(repo, base, phase, supplied_f, root):
    ...
    metadata = validate_receipt(wrapper["receipt"], expected_base, root, {supplied_f, control, event_digest}, receipt_digest, repo, True, f"{phase}.receipt")
...
The PC-REVIEWTRUTH-11 native-first TDD transition keeps one immutable six-node RED set, then closes it at the lanes that own each surface:

- `env PHASE_LOOP_TDD_EXPECT_REVIEWTRUTH=1 PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_each_host_vendor_family_without_external_spawn phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_all_duplicate_matching_seats phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_keeps_non_native_adapters_and_standalone_topology phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_request_report_exact_once_identity_matrix phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_host_integrations_reject_missing_explicit_context phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_skill_contracts_use_typed_rendezvous -q` (SL1-T1 and SL2-T1 literal PC-REVIEWTRUTH-11 RED before any SL-2 production edit; retain the exact six-node output and require failures only at the six distinct `REVIEWTRUTH_RED::native_first_*` anchors)
- `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_each_host_vendor_family_without_external_spawn phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_all_duplicate_matching_seats phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_keeps_non_native_adapters_and_standalone_topology phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_request_report_exact_once_identity_matrix -q` (SL2-T3 literal PC-REVIEWTRUTH-11 runtime GREEN for the four SL-2-owned selector/routing/request-report nodes after only the SL-2 native-first runtime implementation)
- `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_each_host_vendor_family_without_external_spawn phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_all_duplicate_matching_seats phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_keeps_non_native_adapters_and_standalone_topology phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_request_report_exact_once_identity_matrix phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_host_integrations_reject_missing_explicit_context phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_skill_contracts_use_typed_rendezvous -q` (SL5-T3 literal PC-REVIEWTRUTH-11 full GREEN for the immutable six-node set after the host integrations and source/bundled skill mirrors land)

The EC-REVIEWTRUTH-18 and EC-REVIEWTRUTH-19 TDD transitions use these literal commands:

- `env PHASE_LOOP_TDD_EXPECT_REVIEWTRUTH=1 PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_board_uses_opus_prover_then_separate_fable_president phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_unavailable_uses_codex_prover_fallback phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_critic_dissent_never_skips_opus_or_fable phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_attests_can_probe_never_binding_prover phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_preflight_rejects_extra_authority_before_launch phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_receipts_bind_model_tools_roots_environment_and_artifact_digest -q` (SL1-T1 literal EC-REVIEWTRUTH-18 RED; retain the exact six-node output and anchor set before implementation)
- `env PHASE_LOOP_TDD_EXPECT_REVIEWTRUTH=1 PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_fable_president_binds_per_finding_dispositions_to_digest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_deferred_finding_requires_verbatim_issue_before_dispatch phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_nondeferrable_or_president_owned_finding_blocks phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_president_forces_ruling_at_three_round_cap -q` (SL1-T1 literal EC-REVIEWTRUTH-19 RED; retain the exact four-node output and anchor set before implementation)
- `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_board_uses_opus_prover_then_separate_fable_president phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_unavailable_uses_codex_prover_fallback phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_critic_dissent_never_skips_opus_or_fable phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_attests_can_probe_never_binding_prover phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_preflight_rejects_extra_authority_before_launch phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_receipts_bind_model_tools_roots_environment_and_artifact_digest -q` (SL2-T3 literal EC-REVIEWTRUTH-18 GREEN after only the topology owner implementation)
- `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_fable_president_binds_per_finding_dispositions_to_digest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_deferred_finding_requires_verbatim_issue_before_dispatch phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_nondeferrable_or_president_owned_finding_blocks phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_president_forces_ruling_at_three_round_cap -q` (SL5-T3 literal EC-REVIEWTRUTH-19 GREEN after only the president-policy and dispatch-consumer implementation)
...
The phrase `no-direct-claude -p four-usable-AGREE procedural root-panel record` retained inside
PC-REVIEWTRUTH-7 describes historical pre-ratification evidence only. It is not a live roster or
unanimity requirement; current execution is governed solely by EC-REVIEWTRUTH-16 through
EC-REVIEWTRUTH-19.
...
Every criterion also inherits PC-REVIEWTRUTH-9 and PC-REVIEWTRUTH-10's literal ten-node TDD
sequence. SL-1 authors the six topology/capability/safety and four authority falsifiers in
`phase-loop-runtime/tests/test_reviewtruth_phase.py` before any production edit, proves only their
ten mapped RED anchors, and lands them with the rest of the immutable tests-only boundary. SL-2
implements the board-role, model-selection, sequential-Claude, and terminal-outcome records;
SL-4 implements typed per-finding policy facts; and SL-5 wires president rounds, issue-before-
dispatch enforcement, and durable dispatch authorization. The identical ten nodes must be GREEN
before closeout. PC-REVIEWTRUTH-9 also consumes the immutable EC-REVIEWTRUTH-16 Fable-only
`binding_prover` assertion unchanged; EC-REVIEWTRUTH-16 and EC-REVIEWTRUTH-17 are consumed by reference and are not
redefined by this additive sequence.
...
- [ ] Plan-local PC-REVIEWTRUTH-11 universal native-first routing — proven by `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_each_host_vendor_family_without_external_spawn phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_all_duplicate_matching_seats phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_keeps_non_native_adapters_and_standalone_topology phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_request_report_exact_once_identity_matrix phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_host_integrations_reject_missing_explicit_context phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_skill_contracts_use_typed_rendezvous -q`; exactly those six nodeids run. Their retained pre-implementation executions fail only at six distinct `REVIEWTRUTH_RED::native_first_*` anchors; the four SL-2-owned runtime nodes turn GREEN at SL2-T3, and the identical complete six-node set turns GREEN at SL5-T3 after host integrations and source/bundled skill mirrors land. The matrix proves Codex/OpenAI, Claude/Anthropic, Gemini/Google, and Grok/xAI matching seats use native host subagents without an external matching-seat spawn; every duplicate matching seat is selected; Claude-family seats use native Anthropic subagents under Claude Code and the authenticated subscription-TUI adapter only under non-Claude hosts/standalone; nonmatching seats keep existing adapters; bare CLI topology is explicitly standalone/all-adapter; false or missing native capability cannot silently fall back; request/report identities and digests bind exactly once; and source plus bundled harness skills use the typed rendezvous instead of recursively launching a complete external board.
...
- [ ] Plan-local EC-REVIEWTRUTH-18 board topology — proven by `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_board_uses_opus_prover_then_separate_fable_president phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_unavailable_uses_codex_prover_fallback phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_critic_dissent_never_skips_opus_or_fable phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_attests_can_probe_never_binding_prover phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_preflight_rejects_extra_authority_before_launch phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_opus_prover_receipts_bind_model_tools_roots_environment_and_artifact_digest -q`; exactly those six full repo-relative PC-REVIEWTRUTH-9 nodeids run, with node-set SHA-256 `161d57ceca75fc0131822802282ff52985e9ee081e536c0db78a5f3e9f37292b`, and their retained pre-implementation run fails only at anchor-set SHA-256 `583efd5c10ee547677b045775a22a6116232ee61d4ac4fc04d1b40011395838f` before the identical set turns GREEN. The capability node consumes the unchanged PC-REVIEWTRUTH-7 Fable-only `binding_prover` assertion; the safety nodes independently make the Opus preflight, authority closure, and exact receipt fields load-bearing.
- [ ] Plan-local EC-REVIEWTRUTH-19 president authority — proven by `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_fable_president_binds_per_finding_dispositions_to_digest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_deferred_finding_requires_verbatim_issue_before_dispatch phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_nondeferrable_or_president_owned_finding_blocks phase-loop-runtime/tests/test_reviewtruth_phase.py::test_review_wave_president_forces_ruling_at_three_round_cap -q`; exactly those four PC-REVIEWTRUTH-10 nodes run, with node-set SHA-256 `01857917639d696d6163e219c16bf4f3ee1470300c6e24a68e2efd565f6d8eb9`, and their retained pre-implementation run fails only at anchor-set SHA-256 `8dc5f02d01ef37da5f5bed243c73ce416c69703df72fff62193cff00085ed2e2` before the identical set turns GREEN. The first node independently substitutes the exact finding text, artifact digest, disposition, and rationale and requires every substitution to fail, so a mutable or unbound rationale cannot satisfy the gate.
