...
This narrow two-file repair leaves the roadmap bytes unchanged and reseals only the REVIEWTRUTH manifest row. Historical review and predecessor digests remain labeled as such; this branch binds only the repaired Stage-A plan contract and its unchanged roadmap input, and neither authorizes lane dispatch before the required Stage-B freeze and ordered review.
...
- [ ] PC-REVIEWTRUTH-8 — EC-REVIEWTRUTH-17 is an exact 13-node literal TDD contract: eight nodes in `phase-loop-runtime/tests/test_reviewtruth_phase.py` and five in the already-owned `phase-loop-runtime/tests/test_phase_worktree_executor.py`, count `13`, exact sorted-LF SHA-256 `98c98c914479bc95eda6977a1e9cee6574b2cc6c28ab9460ad249eadebe54ca0`. The exact nodeids are `test_review_wave_plan_design_allows_critics_first_but_requires_early_probe_before_fable`, `test_review_wave_premerge_release_stages_early_prover_evidence_before_critics`, `test_review_wave_capabilities_distinguish_can_probe_from_binding_prover`, `test_review_wave_bundle_binds_input_head_bundle_and_evidence_digests`, `test_review_wave_contradicting_prover_invalidates_agree_and_requires_updated_bundle_rereview`, `test_review_wave_unconfined_grok_records_degraded_evidence_without_launch`, `test_review_wave_gemini_is_never_selected_as_write_capable_prover`, and `test_review_wave_predecessor_critic_artifacts_do_not_count_before_early_codex_evidence` under the first file; and `test_early_prover_codex_uses_cli_workspace_write_in_per_leg_ephemeral_worktree`, `test_early_prover_serial_holder_gets_distinct_data_scope_and_exact_digest_binding`, `test_early_prover_external_reaper_cleans_crash_residue_without_leg_authority`, `test_early_prover_codex_external_tool_policy_and_effective_preflight_fail_closed`, and `test_early_prover_external_tool_policy_preflight_and_receipt_are_digest_bound` under the second. Their exact corresponding anchors are `REVIEWTRUTH_RED::plan_design_gate_order`, `REVIEWTRUTH_RED::premerge_release_prover_first`, `REVIEWTRUTH_RED::capability_role_separation`, `REVIEWTRUTH_RED::artifact_bound_evidence`, `REVIEWTRUTH_RED::contradiction_re_review`, `REVIEWTRUTH_RED::degraded_unconfined_grok`, `REVIEWTRUTH_RED::gemini_ineligible`, `REVIEWTRUTH_RED::predecessor_critic_evidence`, `REVIEWTRUTH_RED::codex_workspace_write`, `REVIEWTRUTH_RED::per_leg_worktree_data_isolation`, `REVIEWTRUTH_RED::external_reaper`, `REVIEWTRUTH_RED::codex_external_tool_preflight`, and `REVIEWTRUTH_RED::external_tool_receipt_digest_binding`; exact sorted-LF anchor digest `f54bcf01ccfd2c8c92d790c2b34aa829b927412ae81182f67a8fcdb3b5dea991`.
  ...
  The coordinator creates one per-leg ephemeral Git worktree beneath `/mnt/workspace/worktrees/` when that root exists, otherwise the existing runtime worktree root; allocates a distinct per-leg data-scope identity with reset/create and teardown receipts; places launch `TMPDIR` canonically inside that worktree/data scope without adding it to writable roots; grants only one serial write-capable lease at a time; and leaves cleanup to an external coordinator reaper that can recover crash residue. The exact tests include positive controls proving a write inside the worktree and its nested `TMPDIR` succeeds through the worktree boundary, while writes to `/tmp`, to an unrelated external path supplied through `TMPDIR`, to the live worktree, and to shared data all fail or prevent launch. They also mutation-test either exclusion false/missing in policy, argv, effective probe, config receipt, preflight receipt, or launch receipt, plus a falsely claimed contained `TMPDIR`. The digest chain is deliberately acyclic: first compute the raw exact input, HEAD, tree, bundle, worktree, and data-scope receipt digests; next bind those raw digests and the external-tool policy and effective-config-receipt digests into the effective preflight receipt; then bind the policy/config/preflight/launch-or-no-launch receipt digests back into the final input/head, worktree, and data evidence envelopes; finally bind every raw and envelope digest plus output into the evidence-bundle digest. A substitution, omission, reordering, mismatch, or attempted self-referential digest invalidates the evidence. The leg cannot reap itself, access the live worktree/shared data, gain `/tmp` or `TMPDIR` authority, or claim `binding_prover`.
...
exclusion or any receipt substitution. No aggregate count/digest is accepted until Stage B
