...
- **Scope**: After SL-0 independently completes, use the exact clean ratified base that already contains the separately committed planning lock prerequisite to execute the exact-digest external bootstrap observer in one controlled pytest process before mutation; then land the complete REVIEWTRUTH falsifier and compatibility migration suite, deterministic marker activation, CI-safe dual-mode post-parser collection, the strict typed degraded-seat append/read/rewrite/reread contract, the exact-node PyPI and source-layout console-script availability dispositions and RED falsifiers, non-colliding native request/attempt identity tests, structured JUnit contract, retrospective SL-0 chronology verification, and panel-reviewed RED record before any production file changes.
