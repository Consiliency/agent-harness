...
sealed candidate profile under a new
private external runner root. The candidate digest binds both the preflight's write-once exact-base
record and the paired observation profile. The candidate must be internally self-consistent and
bind the exact base commit/tree, both dependency landing SHAs, runtime/tests tree, observer bytes, plan/roadmap bytes, provisioning identity,
source observation, Gate-A observation, and every raw set/tuple/count/digest; it is not compared
with or authorized by the historical `cee6cdc...` constants.
...
immutable observer payloads and the Stage-A plan/protocol digest; bind the sealed candidate-profile
digest; freeze the canonical installed Codex executable realpath/digest, version/help digests,
...
tests-only SL-1 mutation. Immediately before mutation, the coordinator re-verifies that canonical
base/tree and the runtime/tests tree are byte-identical to the Stage-B freeze. No future count or
...
- [ ] PC-REVIEWTRUTH-2 — Broad compatibility accounting is independent from phase-node accounting, separates marker deselections from skips, and uses the two-stage exact-base reanchor protocol above. Stage A runs only after CONFORM and HARDEN are both present in one exact fetched canonical base. Before any SL-0 dispatch or SL-1 mutation, the coordinator supplies their full immutable landing SHAs, `REVIEWTRUTH_RUNNER_ROOT` as a new canonical absolute path outside the exact git toplevel, and `REVIEWTRUTH_UV` as a canonical absolute executable path. It first runs the exact-base preflight below, which fetches `origin/main`, requires both supplied landings to be ancestors of the fetched commit, requires the clean full worktree HEAD to equal that commit, and writes a private write-once sidecar recording the exact base commit/tree and dependency SHAs. It then executes the five bootstrap commands: provision, materialize both immutable observers, freeze provisioning, observe source layout, and observe the installed-wheel Gate-A copied tree. The fifth bootstrap command seals an internally self-consistent candidate `paired-profile.json`; its terminal candidate digest binds both that profile and the exact-base sidecar and must not compare against the historical planning-author constants. No tracked test, parser, record, or production byte changes during Stage A.

  Stage B then freezes that observed profile into a revised exact-base plan, recomputes all aggregate suite/node/anchor counts and digests, and passes the ordered exact-digest Codex/critics/Fable review before SL-0 or SL-1 can begin. Only Stage B may replace “candidate” checks with literal equality against its reviewed exact values. After its SL-0 coordinator proof passes and before any SL-1-owned file is edited, the coordinator verifies the base/tree/runtime/tests bytes still equal Stage B; SL-1 may then author the literal RED tests, followed by the parser/conftest implementations and the sixth operational parse. The parser cannot be required before it exists and cannot authorize SL-0. The runner root and every non-venv bootstrap boundary must be newly created, owned by the effective UID, private (`0700` directories and `0600`/`0700` files), canonical, non-symlinked, and disjoint from the worktree; the only permitted internal symlink is the venv's interpreter link whose real target must remain beneath the runner root's uv-managed Python directory. No bootstrap path may be covered by a Git status allowlist.
...
Every criterion first inherits the Stage-A/Stage-B lifecycle. This Stage-A plan authorizes only dependency-complete reanchor observation. The reviewed Stage-B plan must freeze the exact base/profile and supply the independently executable SL0-T3 proof for all three exact `agent-harness#398` directives, the non-authorizing `agent-harness#405` tracker, an effective `pull_request` rule and canonical response digest, and separate two-parent record landing. No historical constant, predecessor panel, wrapper, future parser, or manual inspection can substitute.

Every criterion also inherits PC-REVIEWTRUTH-2's paired observations. Stage A seals the candidate
profile and digest with internal relations only. Stage B freezes those exact observed values,
reviews them in directive order, and requires a fresh exact match before SL-0/SL-1. The historical
...
directories before implementation mutation.
