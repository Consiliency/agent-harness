...
- Pytest/pluggy distribution versions, absolute module or distribution paths, module/file digests, and version-bearing approved core-plugin metadata remain mandatory in every bootstrap, source/CI, and Gate-A provenance record and mismatch diagnostic, but are never semantic equality keys by themselves. Cross-environment parity compares the repo-owned test/conftest/parser bytes and digests, declared selectors, exact application-plugin entry points and repo-owned origins, repository/root maps, normalized approved core-plugin roles/origins, canonical full and selected nodeids, skip/deselect categories, hook behavior, command profile, collection/import success, exit status, and JUnit/result accounting. The parser and conftest controls must accept a fixture that changes only those pytest/pluggy diagnostic values while preserving the semantic record, and must reject a paired toolchain fixture that changes plugin origin, collection, nodeids, skip/deselect accounting, hook behavior, exit status, or results. Missing or internally inconsistent provenance also fails; provenance-only release drift does not.
