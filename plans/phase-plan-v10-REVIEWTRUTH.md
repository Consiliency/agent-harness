...
- **Tasks**:
  ...
  - impl: SL2-T2 topology arm — Implement IF-0-REVIEWTRUTH-4 in the existing board/schema/composition seams: Opus primary through the canonical Claude subscription-TUI/native-fill adapter, typed Codex fallback after zero-launch Opus failure, all-four-terminal barrier, and the sequential Fable-president transition. Preflight and bind Opus's exact model/auth, scrubbed subscription environment, `Read,Write`-only tools, zero Bash/shell/web/MCP authority, limited add-directory roots, per-leg worktree/data scope, explicit mount, and serial lease without adding a launcher branch. Run the Opus/Codex board role as the sole current write holder, and derive binding-prover usability only from the later grounded artifact-bound Fable result.
...
| SL2-T2 | impl | SL2-T1 | all six SL-2 owned files | none | the literal EC-REVIEWTRUTH-17, EC-REVIEWTRUTH-18, and PC-REVIEWTRUTH-11 GREEN commands followed by existing focused and golden commands | Implement the exact file-by-file and topology contracts above. Reuse existing worktree lifecycle seams; the coordinator owns mount selection/preflight, parent/worktree creation, launch authorization, serial leases, digest binding, and the external reaper. Compose Opus primary or typed Codex fallback, wait for all four terminal outcomes, then invoke Fable sequentially and derive binding-prover usability from that president result. Classify all host-vendor matching seats through the typed native rendezvous and preserve only nonmatching or explicit standalone adapter legs. |
| SL2-T3 | verify | SL2-T2 | all six SL-2 owned files | none | the EC-REVIEWTRUTH-17, EC-REVIEWTRUTH-18, and PC-REVIEWTRUTH-11 GREEN commands plus existing marker-driven SL2 commands | Require all focused commands GREEN. Prove the external-tool and mount closure, isolated serial holder and reaper, policy/config/preflight/receipt-bound evidence, gate ordering, Opus-primary/typed-Codex-fallback selection, all-four-terminal barrier, sequential Fable president, Fable-only binding, contradiction re-review, universal native-first routing, explicit standalone behavior, and legacy non-native path/golden compatibility. |
...
- **Tasks**:
  ...
  - verify: SL5-T3 — Prove production reachability, gate-dependent ordering, role separation, durable early-evidence/degraded/reaper/contradiction records, full-board completion despite critic dissent, separate Fable-president per-finding rulings, issue-before-dispatch enforcement, non-deferrable blocking, and the three-round cap. Each change-panel attempt uses EC-REVIEWTRUTH-17 through EC-REVIEWTRUTH-19 and the canonical subscription-TUI route; no direct `claude -p`.
...
The PC-REVIEWTRUTH-11 native-first TDD transition uses the identical six-node set for both literal commands:
...
- `PYTHONPATH=phase-loop-runtime/src python3 -m pytest phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_each_host_vendor_family_without_external_spawn phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_routes_all_duplicate_matching_seats phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_keeps_non_native_adapters_and_standalone_topology phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_request_report_exact_once_identity_matrix phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_host_integrations_reject_missing_explicit_context phase-loop-runtime/tests/test_reviewtruth_phase.py::test_reviewtruth_native_first_skill_contracts_use_typed_rendezvous -q` (SL2-T3 literal PC-REVIEWTRUTH-11 GREEN for the identical immutable six-node set after only the SL-2 native-first runtime implementation)
