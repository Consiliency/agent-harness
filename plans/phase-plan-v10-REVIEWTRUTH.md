...
The native citation-marker family is detected after leading Markdown/whitespace normalization and case-insensitive key/colon-spacing recognition, but only the exact canonical `EVIDENCE_REF:` spelling may parse. A case-changed or colon-spaced look-alike line fails instead of remaining analytical prose.

Protected rendered-prompt marker families are detected after leading Markdown/whitespace normalization and case-insensitive key/colon-spacing recognition, then compared to the exact canonical case-sensitive lines. A bullet-prefixed, quoted, differently cased, or colon-spaced conflicting marker therefore fails instead of hiding beside the expected marker.
