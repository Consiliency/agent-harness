...
All prompt and native-report text uses one strict framing rule: UTF-8, NUL-free, terminal LF required, and CR, VT, FF, FS, GS, RS, NEL, U+2028, and U+2029 forbidden anywhere. CRLF and unterminated text therefore fail before marker or vote parsing. `AGREE`, `PARTIALLY AGREE`, `DISAGREE`, and the optional `VERDICT:` label are case-sensitive ASCII; `PARTIALLY AGREE` contains one literal ASCII space and no whitespace normalization. Substantive review/probe bodies require at least 40 raw UTF-8 bytes, at least 40 visible UTF-8 bytes after Unicode control/format/separator/whitespace characters are excluded, and at least eight alphanumeric code points; invisible padding cannot satisfy the floor. The early-prover terminal is the exact raw final nonempty line `PROBE_STATUS: CLEAR`; bare, case-changed, whitespace-padded, quoted, Markdown-prefixed, control-separated, or Unicode-separated probe-status terminals fail. A canonical `PROBE_STATUS: BLOCKED` may be retained only as standalone non-authorizing early-prover evidence; it makes the prover unusable and cannot appear in a completed review-wave receipt or restart chain, whose prior entries are complete post-`CLEAR` waves invalidated by a later material finding or contradiction.

Restart chronology distinguishes two trust paths. A material seat finding is bound to its native producer interval inside the prior wave. A separate contradiction not present in that wave must be reported strictly after the prior receipt completes and strictly before the replacement CLEAR prover starts; backdated or post-start contradiction records fail.
...
- **Tasks**:
  ...
  - test: SL1-T1 restart-finding chronology arm — Retain one material seat finding inside its native start/completion interval and one external contradiction strictly between prior receipt completion and replacement CLEAR-prover start as positive controls. Backdate the external contradiction before/equal to prior completion, move it to/after replacement prover start, move an in-wave material finding outside its producer interval, and relabel a non-contradicting external finding; every mutation must fail at its distinct chronology/binding anchor.
...
The strict dependency verifier is the following exact `60679`-byte UTF-8/LF Python source at SHA-256 `b5b4710f1c2245457fe794207ff403ed40d036ab07cc6bf6ebc4a28439cc7add`. The second Stage-A command extracts only these marked bytes from this plan into the private external `$REVIEWTRUTH_RUNNER_ROOT.stage-a-review-wave-verifier.py`, verifies both constants before execution, and never imports REVIEWTRUTH production code.
...
def validate_receipt(receipt, expected_base, root, forbidden, receipt_digest, repo, final, label):
    ...
    early_valid = early_native["status"] == "OK" and probe_status is not None and probe_substantive and native_early_grounding == required_early_evidence
    early_usable = early_valid and probe_status == "CLEAR"
    derived_early_outcome = probe_status if early_valid else "BLOCKED"
