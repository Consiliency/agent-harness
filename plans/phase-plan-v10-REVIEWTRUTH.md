...
- **Tasks**:
  ...
  - test: SL1-T1 parameterized-policy arm — In the same immutable review-wave node, retain the shipped-default fixture proving all four gates resolve to 3/3/true, then apply typed overrides independently. A `required_prover=false` fixture with all unrelated values byte-equal must remove only the Fable-prover shortfall and pass where the same fixture under `true` fails. Vendor/lens overrides and `on_shortfall=proceed_degraded` may waive only their availability floors; consensus still fails, and `required_prover=true` still fails, independently of `on_shortfall`. No verifier may re-impose the shipped tuple after resolution.
