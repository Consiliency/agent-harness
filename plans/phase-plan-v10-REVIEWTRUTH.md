...
The native citation-marker family is detected after leading Markdown/whitespace normalization, including `+` list bullets, and case-insensitive key/colon-spacing recognition, but only an unprefixed raw line with the exact canonical `EVIDENCE_REF:` spelling may parse. A Markdown-prefixed, case-changed, or colon-spaced look-alike line fails instead of remaining analytical prose.
...
def evidence_refs_from_text(value, label):
    ...
    for index, line in enumerate(value.splitlines()):
        ...
        encoded = line[len("EVIDENCE_REF:"):].strip()
