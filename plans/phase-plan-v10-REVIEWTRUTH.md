...
REVIEWTRUTH is explicitly selected for this run. Canonical `.phase-loop/state.json` and `.phase-loop/tui-handoff.md` now agree that REVIEWTRUTH is the current `planned` phase, and the newest canonical ledger event records the GPT-5.6 Sol planning run at maximum effort. The cumulative root-plan candidate owns the EC-REVIEWTRUTH-17 roadmap reseal, semantic repairs to REVIEWTRUTH plus its CONFORM/HARDEN producer contracts, the corresponding three manifest rows, and the already sealed tracked `phase-loop-runtime/uv.lock` bootstrap prerequisite. The current degraded-evidence integration repair changes only those three plans and manifest bindings; it neither regenerates nor changes the roadmap or lock bytes. Every other phase plan, manifest row, and unrelated coordinator-owned digest remains untouched. Legacy `.codex/phase-loop/` state is compatibility-only and is not authoritative.
...
This cumulative coordinated repair updates only the roadmap's EC-REVIEWTRUTH-17 contract from the superseded 13-node set to the exact 16-node set already required here, then reseals all six root plans' current roadmap bindings; only REVIEWTRUTH, CONFORM, and HARDEN receive semantic plan changes. The current delta adds the exact degraded no-launch producer/consumer union required by `agent-harness#398` comment `5139955591` and completes the launched path's independently verifiable policy/config/preflight/worktree/data-scope/reaper chain; it does not change the already sealed roadmap or lock bytes. Historical review and predecessor digests remain labeled as such. This branch binds the repaired producer/consumer dependency-completion contract and corrected roadmap input, and neither authorizes lane dispatch before the required Stage-B freeze and ordered review.
...
`early_prover` has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `outcome`, `receipt`, `role`, `seat`, `started_at`, `usable`, and `vendor`; those facts are derived rather than accepted. The artifact schema selects exactly one of two non-overlapping trust paths.
