...
- prerequisite test correction: prover-discovered regression tests in `phase-loop-runtime/tests/test_legible_review_repairs.py` originally landed on the target branch before implementation base `B`, skip as one file while the capability module is absent, and execute after the marker is installed. They are not part of the frozen 84-nodeid inventory and do not change either frozen test blob. The C5-discovered `test_attester_uses_latest_pre_base_frozen_test_anchor` is the sole later addition to that file; this plan repair adds the file and plan to the exact phase-owned set `O` and binds their resulting `B..P` delta instead of misreporting them as pre-base bytes.
