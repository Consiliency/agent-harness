...
Before any lifecycle transition or SL-0 write, SCHED SL-3 must amend this plan and the unique
HARDEN manifest row with a `SCHED_HARDEN_HANDOFF` binding the actual SL-2 commit, tree, exact
four-path production diff, SCHED plan digest, and roadmap digest. That amended HARDEN package
must receive a fresh exact-digest native-first four-seat board review; dissent, timeout, a missing
seat, or digest drift blocks HARDEN. The coordinator then fetches that canonical base, reruns the
...
- HARDEN is not write-disjoint from the current `CONFORM`, `FABPUB`, `LEGIBLE`,
  ...
  `SCHED_HARDEN_HANDOFF` successor plan/manifest and its fresh review before `SL-0`, fetches
  the exact post-FABPUB and post-SCHED target, and regenerates/reverifies the tests-only
  baseline; REVIEWTRUTH cannot begin until HARDEN's two-parent landing exists.
