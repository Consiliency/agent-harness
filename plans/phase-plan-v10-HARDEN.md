...
That already-planned `F` now carries the same strict producer contract consumed for CONFORM. The one newly appended event is the only `completed` event in the unique `v10-HARDEN` row whose `metadata.phase_completion_landing.schema` is `v10.phase-completion-landing.v1`; its payload has exactly `schema`, `phase_alias`, `plan_sha256`, `roadmap_sha256`, `run_id`, `canonical_origin`, `canonical_ref`, `audited_implementation_landing`, `audited_implementation_tree`, `final_audit_at`, `final_audit_sha256`, `final_evidence_sha256`, `early_codex`, `evidence_bundle_sha256`, `ordered_panel`, and `required_facts`. `phase_alias=HARDEN`; the plan and roadmap digests name their exact blobs at audited canonical-main `M`; `canonical_origin=Consiliency/agent-harness`; `canonical_ref=refs/remotes/origin/main`; `audited_implementation_landing=M`; and `final_audit_at` is the retained canonical UTC timestamp of the fresh exact-`M` final audit. `early_codex` has exactly `artifact_sha256`, `outcome=CLEAR`, `capability=can_probe`, and `usable=true`. `ordered_panel` has exactly four entries at positions `1..4` with `(seat,role)` exactly `("grok","critic")`, `("gpt-5.6-sol","critic")`, `("gemini","critic")`, and `("fable","binding_prover")`; each entry has exact `position`, `seat`, `role`, `artifact_sha256`, `outcome`, `verdict`, and `usable`, and the evidence proves Fable ran last after all critics on the evidence-bearing bundle. `required_facts` is exactly `{"fable_usable_agree":true,"sol_usable_agree":true}` and is derived from matching `outcome=reviewed`, `usable=true`, `verdict=AGREE` Fable and Sol entries. All named evidence, audit, bundle, and seat digests are lowercase SHA-256 over immutable bytes.

The strict event uses the existing `update_lifecycle` four-key envelope `at`, `by`, `metadata`, and `transition`. It is absent at `M`, preserves the entire executing lifecycle as an exact prefix, uses the same executor writer as the unique executing event, carries the same nonempty `metadata.phase_completion_landing.run_id` as that executing event's `metadata.run_id`, has `at` strictly after its `final_audit_at`, and makes the row's `status=completed` and `updated_at=at`. It includes neither `FH` nor `F` because those identities are derived from topology and would be self-referential. The coordinator creates `FH` as a single-parent commit whose only parent is `M` and whose only changed path is `plans/manifest.json`, then lands `F` with exactly ordered parents `[M,FH]`. Both `M..FH` and `M..F` change only the manifest and preserve every non-manifest blob; the event is added in `FH` and remains byte-semantically unchanged at `F` and fetched canonical main. Missing, duplicate, wrong-phase, stale-plan/roadmap, evidence/panel digest or outcome drift, unusable/non-`AGREE` Fable or Sol, reordered/wrong topology, arbitrary ancestor, identical `M`/`F`, extra lifecycle mutation, or any non-manifest delta blocks completion.
...
10. After that exact-`M` audit passes, the executor performs its normal
    ...
    phase, exact plan/roadmap blobs, audited `M`/tree, audit/evidence/bundle and
    four ordered seat digests/outcomes, early-Codex CLEAR fact, and usable
    `AGREE` Fable/Sol facts all match the retained final artifacts; the event is
