...
`Consiliency/agent-harness#241`. Live source inspection of the reviewed LEGIBLE landing
confirms that review staging preserves
...
branch, lands it through a manifest-only control PR whose merge has exactly two ordered parents, records the
...
manifest-only closeout PR whose merge has exactly two ordered parents as `F`, proves `M..F` is exactly the
...
The producer retains beneath the immutable coordinator-supplied canonical evidence root for the matching `evidence_root_id` the exact final-audit and final-evidence bytes, every producer record required by the selected early path, every seat execution attestation, rendered prompt, and native result, every canonical reducer artifact embedding exact raw review text where a model launched, the canonical bundle, early receipt, every seat receipt, the complete restart-chain records, and the typed policy resolver input/output bytes. Every retained file reference has exactly `path`, `bytes`, and `sha256`; its path is a nonempty normalized immutable UTF-8 relative locator with no absolute form, backslash, empty segment, `.`/`..` segment, or symlink at any ancestor. Before append, the producer opens each reference without following symlinks, proves that it resolves beneath that evidence root to a coordinator-owned regular file that is not group/other-writable, reads and retains the bytes, and closes every descriptor. From those retained bytes—not wrapper booleans or reviewer summaries—it independently recomputes path containment, integer byte counts, lowercase SHA-256 values, strict canonical JSON, common base and bundle equality, producer identities, launched-path policy/config/PASS/recheck/launch/mount/worktree/data-scope/reaper/prompt/citation/status facts or degraded-path failed-preflight/zero-process/no-confined-Grok facts, parsed chronology, roles, complete restart semantics, and the effective typed parameterized policy. Composition/auth preflight writes each launched execution attestation and binds its distinct rendered prompt before launch; the worktree executor, invoker, native-fill binder, and external reaper each write only their producer-owned launched records; only after the complete operational chain exists may a separate reducer create an artifact. The degraded reducer likewise runs only after its independent policy/effective-config/preflight/launch/fallback records exist, and no reducer may create or rewrite any producer or prompt record.

`early_prover` has exactly `artifact`, `binding_prover`, `capability`, `completed_at`, `outcome`, `receipt`, `role`, `seat`, `started_at`, `usable`, and `vendor`; its artifact schema selects exactly one of the two REVIEWTRUTH-frozen trust paths. A launched `v10.review-early-prover-artifact.v1` has the exact REVIEWTRUTH key set including `codex_preflight_receipt` and `codex_launch_receipt`; it retains the producer-owned policy/effective-config, compact PASS, full PASS and stable prelaunch recheck, compact/full one-process launch, safe workspace-mount, exact detached worktree/base, isolated data-scope, external-reaper, execution-attestation, rendered-prompt, and post-termination native-result records. Only that complete distinct-file chain plus a substantive grounded exact `PROBE_STATUS: CLEAR` derives usable launched evidence; launched `BLOCKED`, invalid, failed, ungrounded, aliased, shared-database, non-serial, unbounded, or unreaped output cannot enter a completed receipt. The sole exception is strict canonical `v10.review-early-prover-degraded-artifact.v1`, whose exact keys and producer records are those frozen by REVIEWTRUTH: failed Codex external-tool preflight with nonempty sorted reasons and zero agent launches; referenced canonical policy and effective-config records; a bound Codex launch receipt with `launched=false` and `codex_process_count=0`; and a bound Grok fallback decision with `os_confinement_available=false`, `launched=false`, `grok_process_count=0`, and exact reason `os_confinement_unavailable`. Only that chain derives `outcome="DEGRADED_NO_LAUNCH"`, `usable=false`, and Codex `can_probe` identity. It never satisfies `binding_prover`, waives no effective policy fact, or admits a model-authored `BLOCKED` substitute.

Both paths use the strict canonical mirrored early receipt and the same `bundle_staging` shape. The canonical `v10.review-evidence-bundle.v1` includes final-audit, final-evidence, early receipt, early artifact, and every producer record required by the selected path: the full operational and model-result chain above for launched CLEAR, or policy/effective-config/failed-preflight/zero-launch/no-confined-Grok records for degraded no-launch. The degraded bundle contains no early prompt or native result. The selected early path's final producer action completes before bundle staging; every critic and Fable starts only afterward and consumes that exact digest. The degraded branch is the sole exception expressly authorized by `agent-harness#398` comment 5139955591 when Codex fails preflight and confined Grok is unavailable; Fable remains the only binding prover and is still required by an effective `required_prover=true`. The bundle contains no receipt or bundle self-digest, and reviewers receive only its read-only by-reference bytes with no arbitrary working-tree or shared-development-database execution.
...
The native citation-marker family is detected after leading Markdown/whitespace normalization and case-insensitive key/colon-spacing recognition, but only exact canonical `EVIDENCE_REF:` spelling parses; case-changed or colon-spaced look-alikes fail instead of remaining analytical prose.

Protected prompt-marker keys are detected after leading Markdown/whitespace normalization and case-insensitive key/colon-spacing recognition, then compared to exact canonical case-sensitive lines; bullet-prefixed, quoted, differently cased, or colon-spaced conflicting markers fail.
...
identities, the exact SL-2 merge with two ordered parents and first-parent path set, ancestry from
...
- **Interfaces consumed**: `HARDEN-roadmap-obligations` (pre-existing),
  `HARDEN-live-source-anchors` (pre-existing), and `SCHED_HARDEN_HANDOFF` from the actual
  reviewed SCHED SL-2 landing.
- **Parallel-safe**: no
- **Tasks**:
  - test: Freeze the guard, all `SL-0.1`–`SL-0.5` HARDEN regression/mutation selectors, the exact 126-nodeid inventory partitioned as 115 `SL-1` nodes + 7 `SL-2` nodes + 4 `SL-3` evidence nodes, the exact 21-nodeid default skip set, all 105 migrated assertion branches, the 134-caller execution/pure classification, and the fresh-process/manifest falsifiers. In the already-owned `phase-loop-runtime/tests/test_harden_evidence_verifier.py`, the future completion-control scope explicitly includes `test_harden_completion_event_rejects_missing_duplicate_wrong_phase_or_drift`, `test_harden_completion_event_rejects_plan_roadmap_evidence_panel_or_required_fact_mismatch`, and `test_harden_completion_merge_rejects_wrong_order_parent_count_or_non_manifest_delta`. Paired positive `M -> FH -> F` fixtures prove either the full launched policy/effective-config/PASS/recheck/one-process launch/safe-mount/exact-worktree/isolated-data-scope/external-reaper/preflight-attestation → rendered-prompt → native-result → reducer-artifact chain or the strict policy/effective-config/failed-preflight/zero-launch/no-confined-Grok degraded chain before critics and native Fable; producer-derived identity/capability/binding/status/chronology/bundle; launched operational isolation plus prompt/native grounding or degraded zero-process derivation; event absence at `M`; exact addition at `FH`; byte-semantic preservation at `F`; unchanged non-manifest blobs; and no self-referential `FH`/`F` field. The unchanged named falsifiers first retain literal RED fixtures for `[elided]` plus 41 padding bytes ending in `AGREE`, an all-AGREE synthetic four-seat artifact/receipt set with no producer records, a reducer-only grounded generic Fable `AGREE`, a citation-only native body ending in `AGREE`, a substantive native body citing both final artifacts plus a third valid bundled artifact, prompts omitting declared lenses, a prompt carrying the expected marker plus plain and Markdown/whitespace/case/colon-spaced cloaked conflicting markers, and every cross-class status/outcome relabel, then reject exact-verdict suffixes, missing/malformed/extra/wrong and case/spacing/Markdown-look-alike native citations, prompt marker/ref omission, duplication, substitution, aliasing, or protected-family cloaking, opaque early artifacts, absent/swapped/aliased producer records, producer/identity/text/status/chronology/bundle substitutions, duplicate native-result IDs, every launched-path missing/aliased/failed/drifted policy/config/preflight/recheck/launch/mount/worktree/data-scope/reaper record, enabled MCP/plugin surface, writable-root or temporary-exclusion drift, wrong process count/base/tree/path containment, shared database access, non-serial holder, leg-owned cleanup, or failed reaping, and every degraded-path passing/empty-reason preflight, nonzero/launched Codex or Grok process, available confinement, producer/ref/base mismatch, or absent policy/effective-config record. These tests land in SL-0 and are not written by this planning repair.
...
`C` has sole parent `T`, the fetched target head containing the dual-parent tests
...
5. Ordered dual-parent topology is exact, not merely reachability. Let `B` be the
...
0. **Land the normal lifecycle append before lane work.** After validating the
   ...
   dual-parent manifest-only PR as `L`, fetches the exact target, and proves a
...
8. The external coordinator merges only exact `I` with ordered parents `[P,I]`,
   ...
   tests-only, and implementation ordered dual-parent PR merges, actual
...
10. After that exact-`M` audit passes, the executor performs its normal
    ...
    manifest-only dual-parent closeout PR as `F`. It proves ordered parents
    ...
    launched-CLEAR-or-degraded-no-launch-before-critics chronology, all-critics-before-Fable
...
- After that exact-plan panel and before any test write, the installed executor
  ...
  lands it alone through the distinct dual-parent control PR `L`, fetches the
...
- `SL-0` lands literally as tests only. No production source, executable,
  ...
  dual-parent tests-only PR into the exact clean post-`L` target branch. On the fetched
...
- HARDEN is not write-disjoint from the current `CONFORM`, `FABPUB`, `LEGIBLE`,
  `PROOFGATE`, `SCHED`, and `REVIEWTRUTH` phase plans. The overlapping phase-owned paths
  are `CHANGELOG.md`,
  `phase-loop-runtime/src/phase_loop_runtime/advisor_board/composition.py`,
  `phase-loop-runtime/src/phase_loop_runtime/cli.py`,
  `phase-loop-runtime/src/phase_loop_runtime/goal_coverage.py`,
  ...
  LEGIBLE → (PROOFGATE ∥ CONFORM) → FABPUB → HARDEN → REVIEWTRUTH, with SCHED SL-2
  landing before HARDEN writes `runner.py` or `launcher.py`. HARDEN requires the exact
  `SCHED_HARDEN_HANDOFF` candidate `C` plus receipt head `R` before `SL-0`, fetches
  the exact post-FABPUB and post-SCHED target, and regenerates/reverifies the tests-only
  baseline; REVIEWTRUTH cannot begin until HARDEN's dual-parent landing exists.
...
local_includes = subprocess.run(
    [GIT, "config", "--no-includes", "--get-regexp", r"^include(if)?\."],
    check=False,
    env=GIT_ENV,
...
unsafe_config = subprocess.run(
    [GIT, "config", "--includes", "--get-regexp", r"^(url\..*\.insteadof|https?\..*|credential\..*|core\.(gitproxy|sshcommand)|protocol\..*|remote\..*\.(uploadpack|receivepack|proxy))$"],
    check=False,
    env=GIT_ENV,
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE,
...
origin = git("config", "--no-includes", "--get", "remote.origin.url").decode().strip()
