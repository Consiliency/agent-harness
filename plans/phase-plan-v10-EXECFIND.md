...
beside a new per-finding decomposition. Brief inlining uses the `artifact_ref` seam
...
- **Interfaces provided**: the falsifier-block parser, `FindingFalsifier`/`FindingFalsifierAttachment`, `attach_finding_falsifiers`; per-finding decomposition (`finding_bound`/`finding_unbound`/`finding_receipt`/`finding_prose`) beside whole-leg codes; the `falsifier_policy`; brief instruction + inline-fixture support.
...
- **Touch-shape falsifier (named seams)**: `panel_invoker.py`, `governed_review.py`, `backing.py`, `composition.py`, `runner.py` are shared with open HARDEN/REVIEWTRUTH/LEGLIFE/RESIDUAL phases. A landing PR whose diff deletes or rewrites an existing line of a shared owned file OUTSIDE its named seams fails the phase. Named seams (additive, no existing line rewritten): in `panel_invoker.py`, the new parser function beside `terminal_verdict`, the attachment dataclasses, and `attach_finding_falsifiers`; in `governed_review.py`, the per-finding branch in `_findings_from_panel` and the `falsifier_policy`; in `backing.py`, the additive identity; in `composition.py`, the brief instruction; in `runner.py`, the repair-context seam (SL-3, held). Every other touch is a new file or the additive `review_stage.py` seam.
...
**Plan budget** (GOVLEAN policy — explicit justification for exceeding 3000 words): the overage is SL-0's nine-node recording-integrity set plus the proofgate refusal sweep, added over two review rounds after seats found real vacuity gaps. Compressing further re-introduces the "mirror dropped a check" risk these name-each-falsifier requirements exist to prevent.
