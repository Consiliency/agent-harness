...
- **Tasks**:
  ...
  - impl: In `governed_review.py`, add a post-`invoke_board`, pre-reduction seam in `governed_board_gate`: parse and attach falsifiers from usable leg text, authorize and run them against the clean exact `reviewed_sha`, canonicalize each returned metadata record to its SHA-256 digest, and pass the `(seat_key, finding_id)` result mapping into `_gate_result_from_panel` and `_findings_from_panel`. Never run a seat's text as a provider command or treat a seat-authored outcome as evidence. At `_findings_from_panel`, validate every binding against its key, the expected reviewed SHA, canonical record digest, and duplicate top-level fields *before* reading its outcome; classify only the verified record. Extend the additive per-finding branch to emit `finding_bound` (`red_on_head`, block), `finding_unbound` (`green_on_head`, warn, to the president), `finding_receipt` (a resolved record digest for `apply_failed`, `node_missing`, `error`, or invalid binding, block pending the president's ruling), and `finding_prose` (no falsifier, warn), keeping whole-leg codes for degraded/non-conforming legs. Thread `falsifier_policy` (`optional | required`, default `optional`) through the same keyword-only reducer seam to gate a prose blocking finding. Do not rewrite an existing whole-leg branch.
