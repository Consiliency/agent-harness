...
| Task ID | Type | Depends on | Files in scope | Tests owned | Test command |
|---|---|---|---|---|---|
| `SL1-T0` | `test` | (none) | two new SL-1 tests only | issue omission/schema negatives, agent-harness#548 lossless extension-field roundtrip, and fleet-source parity | `uv run --project phase-loop-runtime pytest -q phase-loop-runtime/tests/test_govlean_{plan_manifest_issue_dispositions,skill_policy_parity}.py` |
| `SL1-I0` | `impl` | `TG-GOVLEAN-0`, `SL0-I0` | `plan_manifest.py`, its existing phase-completion test, and twelve exact skill sources | frozen new GOVLEAN tests; edits forbidden | same targeted command plus `test_skill_plan_manifest_write.py`, now GREEN |
| `SL1-I1` | `impl` | `SL1-I0` | generated neutral and packaged skill directories | frozen; edits forbidden | run `regenerate_skills_bundle.py`, then `sync_skills_bundle.py` |
