...
- [ ] IF-0-PANEL-1 — the names SL-0's frozen tests import. EC-PANEL-1/4/5 carry the behaviour.
  ...
  - `panel_invoker`:
    ...
    - `deliver_seat_prompt(seat_key: str, route: str, prompt: str, send: Callable[[str], tuple[str, str]]) -> tuple[str, str]`, a module attribute looked up at call time whose default returns `send(prompt)`. On the brokered and TUI routes production calls it exactly once per seat, with that seat's `Seat.seat_key`, `route` in `{"brokered", "tui"}`, and the exact prompt it then hands to that seat's transport, after assembly and before any transport; its return is the seat's `(status, text)`. Replacing it changes neither the route nor the assembly: it marks that leg's result as non-production evidence (as `_has_injected_review_execution_seam` does) without switching away from the broker route. It is the observation seam that binds each frame to its seat for EC-PANEL-6 (agent-harness#1094).
...
- **`ec6_*` and `ec6h_*` observe delivery through `deliver_seat_prompt`**: exactly one delivery per seat, on its lane's route, each seat's own lens inside its own digest-bound instructions frame. A swap of two seats' lenses fails them. SL-0.5 runs `verify --receipt-only`; bare `verify`, which also requires every node to pass, is the phase suite's gate.
