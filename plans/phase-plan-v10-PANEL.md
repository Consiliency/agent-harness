...
- **`ec6_*` and `ec6h_*` bind each delivery to its seat by an identity production supplies, not by frame content**, so a swap of two seats' lenses fails them:
  ...
  - native fill: through the native request's own `seat_key` and `instructions` fields (`NativeAgentLegRequest`, pre-existing) -- each seat's own lens in its request's instruction channel, not only in metadata; no frame is required there (EC-PANEL-6). SL-0.5 runs `verify --receipt-only`; bare `verify`, which also requires every node to pass, is the phase suite's gate.
