...
class _ReviewMonitor:
    ...
    def observe(self, age: float | None = None, terminal: str | None = None) -> None:
        ...
        self.record.update(last_genuine_progress_age_s=age,
                           observation_state="progress_observed" if age == 0 else "progress_unobserved",
    ...
    *,
    ...
    def _popen() -> subprocess.Popen[bytes]:
        return subprocess.Popen(
            list(cmd) if review_monitor is None else review_monitor.owned_command(cmd),
            cwd=str(cwd),
            env=dict(env),
    ...
    try:
        while True:
            if review_monitor is not None:
                review_monitor.observe(time.monotonic() - last_heartbeat)
    ...
    *,
    ...
    try:
        ...
        try:
            def _popen() -> subprocess.Popen[bytes]:
                return subprocess.Popen(
                    list(command) if review_monitor is None else review_monitor.owned_command(command),
                    cwd=str(cwd),
                    env=dict(env),
                    stdin=slave_fd,
                    stdout=slave_fd,
...
def _default_spawn(
    ...
    *,
    ...
    """
    if review_monitor is not None or review_authorization is not None:
        try:
    ...
    try:
        ...
        if review_monitor is not None:
            return _BrokeredSpawnResult("DEGRADED", "", str(exc)[:200],
    ...
    **Observability (ABDOBS).** When ``sink`` is given, the natively-launched
    ...
    try:
        _advisor_board_backing.resolve_review_monitoring_policy(
            ...
            gateway=omnigent is not None or gateway_available is not None,
        )
    except ValueError as exc:
