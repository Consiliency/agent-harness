...
belongs to the seat's OWN vendor harness; see Seat Routing below). Harness/agy, Grok, Omnigent, native-host,
...
| host ↓ / seat → | claude (Fable/Opus) | codex (GPT-6 Astra) | <harness> | grok |
|---|---|---|---|---|
...
| <harness> CLI | TUI adapter | `codex` CLI | native, where the CLI offers subagents | `grok` CLI |
...
detects no other host, so under codex, <harness> or opencode the host's own vendor seat is still
launched as a CLI subprocess; the remaining cells are tracked on agent-harness#924. On this host: the Claude seat is the TUI adapter's, never this host's Task/subagent primitive. The <harness> seat is this host's own vendor and belongs to a native subagent once the runtime emits a request for it; until then it runs as the `agy` CLI subprocess and must not be hand-filled.
...
7. Every Fable or Opus seat requires the homebrew Claude Code self-PTY adapter after a metadata-only probe proves first-party `claude.ai` subscription auth. Never substitute a gateway, API, SDK, direct HTTP call, or this host's Task/subagent primitive; `tui_backing_required`, `subscription_auth_unproven`, and `tui_adapter_required` fail closed.
