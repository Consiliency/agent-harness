...
| host ↓ / seat → | claude (Fable/Opus) | <harness> (GPT-6 Astra) | gemini | grok |
|---|---|---|---|---|
| Claude Code | native sub-agent (emit → fill → invoke) | `codex` CLI | `agy` CLI | `grok` CLI |
| <harness> | TUI adapter (self-PTY) | native <harness> subagent | `agy` CLI | `grok` CLI |
