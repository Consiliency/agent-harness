...
def _is_installed_bytecode_cache_artifact(relative: Path) -> bool:
    """Only ``.pyc`` files beneath a ``__pycache__`` directory are import residue."""
    return relative.suffix == ".pyc" and "__pycache__" in relative.parent.parts


def _expected_contract_members() -> tuple[dict[str, str], dict[str, Any]]:
    root = Path(__file__).with_name("_contract")
    members = {"phase_loop_runtime/conformance/_contract/" + path.relative_to(root).as_posix(): _sha256_bytes(path.read_bytes()) for path in sorted(root.rglob("*")) if path.is_file() and not _is_installed_bytecode_cache_artifact(path.relative_to(root))}
...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    required_commit_keys = [
        ...
        "post_review_chronology_plan_candidate",
    ...
    expected_support_identities = {
        ...
        "post_review_test_landing": (
            "ae2212337582e0b5f1fb158b0cf4f1c29329e91e",
            "2d99602035c22dc751bafa2864fd8f280589f0e1",
            "d4d08d30b4b8d4c0eed0572a04945cbbe499a67b",
        ),
        "post_review_chronology_landing": (
            "ea3ee614b79dab086ae2bb816538c9f13f5a5eae",
            "ae2212337582e0b5f1fb158b0cf4f1c29329e91e",
            "68ba35aacc2232603367305c5bc169336e036102",
        ),
        "post_review_wheel_normalization_landing": (
            "b0332feca37e7f37e503fa87336e779ed2161388",
            "ea3ee614b79dab086ae2bb816538c9f13f5a5eae",
            "c40745a11f08772f1662585821b953fd5718c6ad",
        ),
        "post_review_producer_landing": (
            "3ba6f34d363267e98ffa431c1d3508d1809ae40b",
            "b0332feca37e7f37e503fa87336e779ed2161388",
            "f962de9fbb208a2d53d67ef74b5c37920fac02e7",
        ),
    }
    ...
    expected_post_review_plan_candidates = {
        "post_review_plan_candidate": "6b2b609098ca479b0f00ad5149b9c9742d635e36",
        "post_review_chronology_plan_candidate": "0f0f7873b7015b5d551ac9aec8049be306349207",
        "post_review_wheel_normalization_plan_candidate": "792e10874eaa7e0f85fbbe5a50c24a09b1aa45ad",
        "post_review_producer_plan_candidate": "2d64a456b7d6abf578daf73600dc70a03186db34",
    }
    for plan_key, plan_candidate in expected_post_review_plan_candidates.items():
        if identities[plan_key] != plan_candidate:
            raise ValueError(f"{plan_key} identity mismatch")

    expected_reproducible_sdist_tdd = {
    ...
    reproducible_sdist_tdd = git_proof.get("reproducible_sdist_tdd")
    if not isinstance(reproducible_sdist_tdd, dict):
        raise ValueError("reproducible_sdist_tdd missing or invalid")
    if {
        key: value
        for key, value in reproducible_sdist_tdd.items()
        if key not in {"red_vector", "green_vector"}
    } != expected_reproducible_sdist_tdd:
        raise ValueError("reproducible_sdist_tdd evidence mismatch")
    for oid in (
        expected_reproducible_sdist_tdd["base"],
        expected_reproducible_sdist_tdd["red_commit"],
        expected_reproducible_sdist_tdd["green_commit"],
    ):
        _val_oid(oid, "commit")
    if reproducible_sdist_tdd.get("red_vector") != [
        expected_reproducible_sdist_tdd["red_commit"],
        expected_reproducible_sdist_tdd["base"],
    ]:
        raise ValueError("reproducible_sdist_tdd RED vector mismatch")
    if reproducible_sdist_tdd.get("green_vector") != [
        expected_reproducible_sdist_tdd["green_commit"],
        expected_reproducible_sdist_tdd["red_commit"],
    ]:
        raise ValueError("reproducible_sdist_tdd GREEN vector mismatch")
    ...
    for stem in (
        "post_review_test",
        "post_review_chronology",
        "post_review_wheel_normalization",
        "post_review_producer",
    ):
        plan_key = (
            "post_review_plan_candidate"
            if stem == "post_review_test"
            else f"{stem}_plan_candidate"
        )
        plan_pv = [
            p.lower()
            for p in _run_git(
                ["rev-list", "--parents", "-n", "1", identities[plan_key]]
            ).split()[1:]
        ]
        test_pv = [
            p.lower()
            for p in _run_git(
                ["rev-list", "--parents", "-n", "1", identities[f"{stem}_candidate"]]
            ).split()[1:]
        ]
        if plan_pv != [identities[f"{stem}_parent"]] or test_pv != [identities[plan_key]]:
            raise ValueError(f"{stem} candidate chain mismatch")
        chain = [
            commit.lower()
            for commit in _run_git(
                [
                    "rev-list",
                    "--reverse",
                    f'{identities[f"{stem}_parent"]}..{identities[f"{stem}_candidate"]}',
                ]
            ).splitlines()
        ]
        if chain != [identities[plan_key], identities[f"{stem}_candidate"]]:
            raise ValueError(f"{stem} candidate must contain exact plan/test commits")
        plan_paths = set(
            _run_git(
                ["diff", "--name-only", identities[f"{stem}_parent"], identities[plan_key]]
            ).splitlines()
        )
        if plan_paths != {"plans/phase-plan-v10-CONFORM.md"}:
            raise ValueError(f"{stem} plan commit path mismatch")

    if identities["post_review_chronology_parent"] != identities["post_review_test_landing"]:
        raise ValueError("post_review_chronology_landing must descend from post_review_test_landing")

    if identities["post_review_wheel_normalization_parent"] != identities["post_review_chronology_landing"]:
        raise ValueError("post_review_wheel_normalization_landing must descend from post_review_chronology_landing")

    if identities["post_review_producer_parent"] != identities["post_review_wheel_normalization_landing"]:
        raise ValueError("post_review_producer_landing must descend from post_review_wheel_normalization_landing")
    ...
    expected_parent_vectors = {
        ...
        "history_retention_landing", "post_review_test_landing",
        "post_review_chronology_landing",
        "post_review_wheel_normalization_landing",
        "post_review_producer_landing",
    }
    ...
    _check_ancestor(identities["history_retention_landing"], identities["post_review_test_parent"])
    _check_ancestor(identities["post_review_test_landing"], identities["post_review_chronology_parent"])
    _check_ancestor(identities["post_review_chronology_landing"], identities["post_review_wheel_normalization_parent"])
    _check_ancestor(identities["post_review_wheel_normalization_landing"], identities["post_review_producer_parent"])
    _check_ancestor(identities["post_review_producer_landing"], identities["candidate_commit"])
    ...
    expected_post_review_test_paths = {
        "plans/phase-plan-v10-CONFORM.md",
        "phase-loop-runtime/tests/test_outside_agent_redaction_separation.py",
        "phase-loop-runtime/tests/test_outside_agent_conform_evidence.py",
    }
    expected_post_review_chronology_paths = {
        "plans/phase-plan-v10-CONFORM.md",
        "phase-loop-runtime/tests/test_outside_agent_conform_evidence.py",
    }
    expected_post_review_wheel_normalization_paths = {
        "plans/phase-plan-v10-CONFORM.md",
        "phase-loop-runtime/tests/_outside_agent_canonical.py",
    }
    expected_post_review_producer_paths = {
        "plans/phase-plan-v10-CONFORM.md",
        "phase-loop-runtime/tests/test_outside_agent_conform_evidence.py",
    }
    ...
    if impl_slot is not None:
        if not isinstance(impl_slot, dict) or set(impl_slot.keys()) != {"paths", "patch", "patch_digest"}:
            raise ValueError("implementation_patch_slot shape mismatch")
        if impl_slot["paths"] != sorted(_VERIFIER_INSERTION_PATHS):
    ...
    if transition.get("base_commit") != identities["post_review_producer_landing"]:
    ...
    transition_base = identities["post_review_producer_landing"]
    support_base = identities["history_retention_landing"]
    ...
    support_chain = [c.lower() for c in _run_git(["rev-list", "--reverse", f"{support_base}..{transition_base}"]).splitlines()]

    support_history = (
        ("46e7dab0ce4b8d92db00986166db6225f5eba8b7", "b09f300cb1e9527f48cf7e757c48e4e48eaede48", support_base),
        ("f475d52930af6ac4c2b17f2448a42565cfb6cf3f", "cf6a54d1de1e0d3ba64ac1c0f3b8e80b3427d344", "b09f300cb1e9527f48cf7e757c48e4e48eaede48"),
        ("9a2d4efa1dad1445f68472e69aaf5edef2df3a73", "da9036c7fa845d1311372a806e5ed6bc84b0e554", "cf6a54d1de1e0d3ba64ac1c0f3b8e80b3427d344"),
        ("5ab9c5235706483fb2403c3a55f4cf747c2e7714", "8387d5ac72114bab06445e88c5b39bd7ffdc29d4", "da9036c7fa845d1311372a806e5ed6bc84b0e554"),
    )
    if support_chain[:8] != [item for candidate, landing, _ in support_history for item in (candidate, landing)]:
        raise ValueError("CONFORM test support history mismatch")
    for support_candidate, support_landing, support_parent in support_history:
        support_candidate_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", support_candidate]).split()[1:]]
        support_landing_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", support_landing]).split()[1:]]
        if support_candidate_parents != [support_parent] or support_landing_parents != [support_parent, support_candidate]:
            raise ValueError("CONFORM test support parent mismatch")
        support_paths = set(_run_git(["diff", "--name-only", support_parent, support_candidate]).splitlines())
        if support_paths != {"phase-loop-runtime/tests/test_outside_agent_conform_evidence.py"}:
            raise ValueError("CONFORM test support path mismatch")

    wheel_red = "950fa0ed97ca0e0a416114887f38f941d43903a3"
    wheel_green = "391c1a66a85788c7649eca9486952b4cd526dcc3"
    wheel_landing = "bc340e3773875c75f2798e7f89b1020a17074d3d"
    wheel_parent = "8387d5ac72114bab06445e88c5b39bd7ffdc29d4"
    if support_chain[8:11] != [wheel_red, wheel_green, wheel_landing]:
        raise ValueError("CONFORM reproducible-wheel support history mismatch")
    if [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", wheel_red]).split()[1:]] != [wheel_parent]:
        raise ValueError("CONFORM reproducible-wheel red parent mismatch")
    if [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", wheel_green]).split()[1:]] != [wheel_red]:
        raise ValueError("CONFORM reproducible-wheel green parent mismatch")
    if [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", wheel_landing]).split()[1:]] != [wheel_parent, wheel_green]:
        raise ValueError("CONFORM reproducible-wheel landing parent mismatch")
    wheel_paths = set(_run_git(["diff", "--name-only", wheel_parent, wheel_green]).splitlines())
    if wheel_paths != {
        "phase-loop-runtime/tests/_outside_agent_canonical.py",
        "phase-loop-runtime/tests/test_outside_agent_conform_evidence.py",
    }:
        raise ValueError("CONFORM reproducible-wheel support path mismatch")

    inventory_candidate = "7477c537cea1782515b2ba00e92830f95feabacc"
    inventory_landing = "0788b0351efd6bdaf011404efaa63a7bb7a265c9"
    if support_chain[11:13] != [inventory_candidate, inventory_landing]:
        raise ValueError("CONFORM wheel-inventory support history mismatch")
    inventory_candidate_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", inventory_candidate]).split()[1:]]
    inventory_landing_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", inventory_landing]).split()[1:]]
    if inventory_candidate_parents != [wheel_landing] or inventory_landing_parents != [wheel_landing, inventory_candidate]:
        raise ValueError("CONFORM wheel-inventory support parent mismatch")
    inventory_paths = set(_run_git(["diff", "--name-only", wheel_landing, inventory_candidate]).splitlines())
    if inventory_paths != {
        "phase-loop-runtime/tests/_outside_agent_canonical.py",
        "phase-loop-runtime/tests/test_outside_agent_conform_evidence.py",
    }:
        raise ValueError("CONFORM wheel-inventory support path mismatch")

    compression_red = "909055447f882ea9576afeca07af3acfb14aa06a"
    compression_green = "25414f67b19b145f5f7dd9de9a941237b004d4b1"
    compression_landing = "9e8c3097fbd38893fbb77614ea9a49a5cf704a73"
    if support_chain[13:16] != [compression_red, compression_green, compression_landing]:
        raise ValueError("CONFORM wheel-compression support history mismatch")
    if [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", compression_red]).split()[1:]] != [inventory_landing]:
        raise ValueError("CONFORM wheel-compression red parent mismatch")
    if [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", compression_green]).split()[1:]] != [compression_red]:
        raise ValueError("CONFORM wheel-compression green parent mismatch")
    if [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", compression_landing]).split()[1:]] != [inventory_landing, compression_green]:
        raise ValueError("CONFORM wheel-compression landing parent mismatch")

    rebase_count_candidate = "a922f92cb5f3eff6ac847e3f42b4bf8bd35a7f29"
    rebase_count_landing = "2d99602035c22dc751bafa2864fd8f280589f0e1"
    if support_chain[16:18] != [rebase_count_candidate, rebase_count_landing]:
        raise ValueError("CONFORM stale-rebase-count support history mismatch")
    rebase_count_candidate_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", rebase_count_candidate]).split()[1:]]
    rebase_count_landing_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", rebase_count_landing]).split()[1:]]
    if rebase_count_candidate_parents != [compression_landing] or rebase_count_landing_parents != [compression_landing, rebase_count_candidate]:
        raise ValueError("CONFORM stale-rebase-count support parent mismatch")
    rebase_count_paths = set(_run_git(["diff", "--name-only", compression_landing, rebase_count_candidate]).splitlines())
    if rebase_count_paths != {"phase-loop-runtime/tests/test_outside_agent_conform_evidence.py"}:
        raise ValueError("CONFORM stale-rebase-count support path mismatch")

    if support_chain[18:21] != [
        identities["post_review_plan_candidate"],
        identities["post_review_test_candidate"],
        identities["post_review_test_landing"],
    ]:
        raise ValueError("CONFORM post-review test support history mismatch")
    if support_chain[21:24] != [
        identities["post_review_chronology_plan_candidate"],
        identities["post_review_chronology_candidate"],
        identities["post_review_chronology_landing"],
    ]:
        raise ValueError("CONFORM post-review chronology support history mismatch")
    if support_chain[24:27] != [
        identities["post_review_wheel_normalization_plan_candidate"],
        identities["post_review_wheel_normalization_candidate"],
        identities["post_review_wheel_normalization_landing"],
    ]:
        raise ValueError("CONFORM post-review wheel-normalization support history mismatch")
    if support_chain[27:30] != [
        identities["post_review_producer_plan_candidate"],
        identities["post_review_producer_candidate"],
        identities["post_review_producer_landing"],
    ]:
        raise ValueError("CONFORM post-review producer support history mismatch")
    if len(support_chain) != 30:
        raise ValueError(f"unexpected support chain count: {len(support_chain)}")

    if len(stages) == 3:
        if len(actual_rebased) != 6:
            raise ValueError(f"unexpected rebased_commits count for final scope: {len(actual_rebased)}")
    else:
        if len(actual_rebased) != 5:
            raise ValueError(f"unexpected rebased_commits count for pre-doc scope: {len(actual_rebased)}")

    inserted_matches = [
        commit
        for commit in actual_rebased
        if _run_git(["show", "-s", "--format=%(trailers:key=Conformance-Verifier,valueonly)", commit]).splitlines() == ["true"]
    ]
    if len(inserted_matches) == 1:
        inserted_commit = inserted_matches[0]
        inserted_index = actual_rebased.index(inserted_commit)
        inserted_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", inserted_commit]).split()[1:]]
        if inserted_index == 0 or inserted_parents != [actual_rebased[inserted_index - 1]]:
            raise ValueError("inserted verifier commit parent mismatch")
        inserted_files = set(_run_git(["diff", "--name-only", inserted_parents[0], inserted_commit]).splitlines())
        if inserted_files != _VERIFIER_INSERTION_PATHS:
            raise ValueError("inserted verifier commit modified unexpected files")
        inserted_patch = _run_git(["diff", "--no-ext-diff", "--no-textconv", "--no-color", "-U0", inserted_parents[0], inserted_commit, "--", *sorted(_VERIFIER_INSERTION_PATHS)])
        ...
        raise ValueError(f"expected exactly one declared verifier commit, got: {inserted_matches}")
    ...
    for line in actual_range_diff.splitlines():
        ...
        if m_eq:
            ...
            if new_idx != old_idx or old_idx < 1 or old_idx > 4:
        ...
        if m_add:
            ...
            if not inserted_commit.startswith(new_sha):
    ...
    if len(actual_rebased) in (5, 6):
        if add_matches != {5}:
            raise ValueError("verifier commit must map with > in range-diff")
    elif add_matches:
