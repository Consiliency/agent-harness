...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for line in actual_range_diff.splitlines():
        ...
        if m_eq:
            ...
            if new_idx != old_idx + 4 or old_idx < 1 or old_idx > 4:
                raise ValueError(f"range-diff equality out of bounds: {line_str}")
            if not expected_original[old_idx - 1].startswith(old_sha) or not actual_rebased[new_idx + 3].startswith(new_sha):
        ...
        if m_diff:
            ...
            if len(stages) != 3 or old_idx != 5 or new_idx != 10:
                raise ValueError(f"unauthorized range-diff patch modification (!): {line_str}")
            if not expected_original[4].startswith(old_sha) or not actual_rebased[13].startswith(new_sha):
        ...
        if m_add:
            ...
            if new_idx not in {1, 2, 3, 4, 9} or len(actual_rebased) not in (13, 14):
                raise ValueError(f"unauthorized range-diff added commit (>): {line_str}")
            added_commit = {1: actual_rebased[0], 2: actual_rebased[2], 3: actual_rebased[4], 4: actual_rebased[6], 9: inserted_commit}[new_idx]
    ...
    if len(actual_rebased) in (13, 14):
        if add_matches != {1, 2, 3, 4, 9}:
