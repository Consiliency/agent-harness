...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if actual_rebased[:4] != [item for candidate, landing, _ in support_history for item in (candidate, landing)]:
    ...
    if len(actual_rebased) in (9, 10):
        ...
        inserted_patch = _run_git(["diff", "--no-ext-diff", "--no-textconv", "--no-color", "-U0", actual_rebased[7], inserted_commit, "--", "phase-loop-runtime/src/phase_loop_runtime/conformance/outside_agent_conform_evidence.py"])
    ...
    for line in actual_range_diff.splitlines():
        ...
        if m_eq:
            ...
            if new_idx != old_idx + 2 or old_idx < 1 or old_idx > 4:
        ...
        if m_diff:
            ...
            if len(stages) != 3 or old_idx != 5 or new_idx != 8:
                raise ValueError(f"unauthorized range-diff patch modification (!): {line_str}")
            if not expected_original[4].startswith(old_sha) or not actual_rebased[9].startswith(new_sha):
        ...
        if m_add:
            ...
            if new_idx not in {1, 2, 7} or len(actual_rebased) not in (9, 10):
                raise ValueError(f"unauthorized range-diff added commit (>): {line_str}")
            added_commit = {1: actual_rebased[0], 2: actual_rebased[2], 7: actual_rebased[8]}[new_idx]
