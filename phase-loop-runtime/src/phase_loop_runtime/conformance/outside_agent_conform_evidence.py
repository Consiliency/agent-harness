...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for line in actual_range_diff.splitlines():
        ...
        if m_eq:
            ...
            if new_idx != old_idx + 6 or old_idx < 1 or old_idx > 4:
                raise ValueError(f"range-diff equality out of bounds: {line_str}")
            if not expected_original[old_idx - 1].startswith(old_sha) or not actual_rebased[new_idx + 4].startswith(new_sha):
        ...
        if m_diff:
            ...
            if len(stages) != 3 or old_idx != 5 or new_idx != 12:
                raise ValueError(f"unauthorized range-diff patch modification (!): {line_str}")
            if not expected_original[4].startswith(old_sha) or not actual_rebased[16].startswith(new_sha):
        ...
        if m_add:
            ...
            if new_idx not in {1, 2, 3, 4, 5, 6, 11} or len(actual_rebased) not in (16, 17):
    ...
    if len(actual_rebased) in (16, 17):
        if add_matches != {1, 2, 3, 4, 5, 6, 11}:
