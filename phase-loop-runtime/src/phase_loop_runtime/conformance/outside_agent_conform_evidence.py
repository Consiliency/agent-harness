...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    support_candidate = "46e7dab0ce4b8d92db00986166db6225f5eba8b7"
    support_landing = "b09f300cb1e9527f48cf7e757c48e4e48eaede48"
    if actual_rebased[:2] != [support_candidate, support_landing]:
        raise ValueError("structural raw-log test support history mismatch")
    ...
    if support_candidate_parents != [transition_base] or support_landing_parents != [transition_base, support_candidate]:
        raise ValueError("structural raw-log test support parent mismatch")
    support_paths = set(_run_git(["diff", "--name-only", transition_base, support_candidate]).splitlines())
    if support_paths != {"phase-loop-runtime/tests/test_outside_agent_conform_evidence.py"}:
        raise ValueError("structural raw-log test support path mismatch")

    if len(stages) == 3:
        if len(actual_rebased) != 8:
            raise ValueError(f"unexpected rebased_commits count for final scope: {len(actual_rebased)}")
    else:
        if len(actual_rebased) != 7:
            raise ValueError(f"unexpected rebased_commits count for pre-doc scope: {len(actual_rebased)}")

    if len(actual_rebased) in (7, 8):
        inserted_commit = actual_rebased[6]
        inserted_parents = [p.lower() for p in _run_git(["rev-list", "--parents", "-n", "1", inserted_commit]).split()[1:]]
        if inserted_parents != [actual_rebased[5]]:
            raise ValueError("inserted verifier commit parent mismatch")
        inserted_files = set(_run_git(["diff", "--name-only", actual_rebased[5], inserted_commit]).splitlines())
        ...
        inserted_patch = _run_git(["diff", "--no-ext-diff", "--no-textconv", "--no-color", "-U0", actual_rebased[5], inserted_commit, "--", "phase-loop-runtime/src/phase_loop_runtime/conformance/outside_agent_conform_evidence.py"])
    ...
    for line in actual_range_diff.splitlines():
        ...
        if m_eq:
            ...
            if new_idx != old_idx + 1 or old_idx < 1 or old_idx > 4:
                raise ValueError(f"range-diff equality out of bounds: {line_str}")
            if not expected_original[old_idx - 1].startswith(old_sha) or not actual_rebased[new_idx].startswith(new_sha):
        ...
        if m_diff:
            ...
            if len(stages) != 3 or old_idx != 5 or new_idx != 7:
                raise ValueError(f"unauthorized range-diff patch modification (!): {line_str}")
            if not expected_original[4].startswith(old_sha) or not actual_rebased[7].startswith(new_sha):
        ...
        if m_add:
            ...
            if new_idx not in {1, 6} or len(actual_rebased) not in (7, 8):
                raise ValueError(f"unauthorized range-diff added commit (>): {line_str}")
            added_commit = actual_rebased[0] if new_idx == 1 else actual_rebased[6]
