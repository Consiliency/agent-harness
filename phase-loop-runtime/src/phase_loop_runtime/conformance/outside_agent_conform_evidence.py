...
_RECORD_KEYS = {
    "record_id", "ordinal", "artifact_path", "artifact_sha256",
    "raw_log_path", "raw_log_sha256", "evidence",
}
_EXCLUSIVE_INPUTS = {
...
def _expected_contract_members() -> tuple[dict[str, str], dict[str, Any]]:
    ...
    members = {
        "phase_loop_runtime/conformance/_contract/" + path.relative_to(root).as_posix(): _sha256_bytes(path.read_bytes())
...
def _validate_records(mode: str, records: list[dict[str, object]]) -> tuple[dict[str, Any], bytes]:
    ...
    for record in records:
        ...
        if not isinstance(evidence, dict) or evidence != {
            ...
            "candidate_tree": facts.get("candidate_tree"),
        }:
...
def _parse_junit(facts: dict[str, Any], runner_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for case in cases:
        ...
        if classname:
            ...
            if failure is not None:
                ...
                if anchors.get(nodeid) not in failure_text:
    ...
    if f"{summary['failures']} failed, {passed} passed, {summary['skipped']} skipped," not in raw_log:
        raise ValueError("runner log summary mismatch")
    if any(f"FAILED {nodeid}" not in raw_log for nodeid in failed_ids):
        raise ValueError("runner log lacks failed node evidence")
...
def _validate_mutations(facts: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for mutation in mutations:
        ...
        if _read_verified(captured.get("result_path"), captured.get("result_sha256")) is None:
            raise ValueError("mutation result missing")
        observable = captured.get("observable")
        if not isinstance(observable, dict) or observable.get("classification") != "killed" or observable.get("candidate_clean") is not True:
            raise ValueError("mutation was not killed against a clean candidate")
        if observable.get("nodeid_matched") is not True or observable.get("anchor_matched") is not True:
            raise ValueError("mutation failure was not bound to its oracle")
...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if implementation.get("commit") != bindings["candidate_commit"] or implementation.get("tree") != bindings["candidate_tree"] or implementation.get("exit_code") != 0:
        raise ValueError("implementation candidate binding mismatch")
    if implementation.get("topology", {}).get("candidate_descends_from_test_candidate") is not True or implementation.get("topology", {}).get("test_paths_unchanged") is not True:
        raise ValueError("implementation topology invalid")
    outcomes = {mutation["id"]: "killed" for mutation in mutations}
    if implementation.get("mutation_outcomes") != outcomes:
...
def _validate_corpus(facts: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(corpus, dict) or set(corpus) != {"rows", "partitions"}:
        raise ValueError("corpus evidence missing")
    rows, partitions = corpus["rows"], corpus["partitions"]
    if not isinstance(rows, list) or not isinstance(partitions, dict):
        raise ValueError("corpus evidence malformed")
    expected = {
        "valid_submissions": sorted(row["case_id"] for row in rows if row["expected_valid"] and row["schema_target"] == "outside_agent_submission.v0.1"),
        "invalid_submissions": sorted(row["case_id"] for row in rows if not row["expected_valid"] and row["schema_target"] == "outside_agent_submission.v0.1"),
        "invalid_route_verdicts": sorted(row["case_id"] for row in rows if not row["expected_valid"] and row["schema_target"] == "outside_agent_route_verdict.v0.1"),
    }
...
def _validate_packages(facts: dict[str, Any], bindings: dict[str, Any], corpus: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, dict[str, str]]]:
    ...
    for name in ("direct-wheel", "sdist-derived-wheel"):
        if any(not (member.startswith("phase_loop_runtime/") or ".dist-info/" in member) for member in archive_members[name]):
            raise ValueError("wheel contains an unexpected top-level member")
    installed = _read_json_fact(facts.get("installed_package"))
    if set(installed) != {"package", "module_path", "variants", "contract_members", "corpus_partitions", "executions"}:
        raise ValueError("installed-package evidence shape mismatch")
    if installed["package"] != "phase-loop-runtime" or installed["module_path"] != bindings["module_path"] or installed["variants"] != list(_PACKAGE_VARIANTS) or installed["contract_members"] != expected_contract or installed["corpus_partitions"] != corpus["partitions"]:
...
def _validate_mode_specific(mode: str, facts: dict[str, Any], bindings: dict[str, Any], runner_manifest: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if mode == "package":
        label_map = {"direct_wheel": "direct-wheel", "direct_sdist": "direct-sdist", "sdist_derived_wheel": "sdist-derived-wheel"}
        for field, label in label_map.items():
            reference = facts[field]
            if reference.get("path") != facts["archives"][label]["path"] or reference.get("sha256") != facts["archives"][label]["sha256"]:
                raise ValueError("mode-specific archive binding mismatch")
            if reference.get("contract_members") != facts["package"]["contract_members"] or reference.get("provenance", {}).get("candidate_commit") != bindings["candidate_commit"] or reference.get("provenance", {}).get("candidate_tree") != bindings["candidate_tree"]:
                raise ValueError("mode-specific archive provenance mismatch")
...
def verify_conform_evidence_records(mode: str, records: list[dict[str, object]]) -> dict[str, object]:
    ...
    if facts.get("vendor") != vendor or runner_manifest.get("provenance") != vendor:
        raise ValueError("vendor provenance differs from packaged contract")
    if runner_manifest.get("contract_member_digests") != expected_contract:
        raise ValueError("runner contract inventory differs from packaged contract")
    ...
    archive_bytes = {
        name: _sha256_bytes(Path(details["path"]).read_bytes())
        for name, details in facts["archives"].items()
    }
    input_facts = {name: facts[name] for name in EVIDENCE_VERIFIER_INTERFACE[mode]["inputs"]}
    input_facts["manifest"] = _sha256_bytes(manifest_bytes)
    input_facts["archive_bytes"] = archive_bytes
    outcome_facts: dict[str, object] = {
        "junit": junit,
        "archive_members": archive_members,
        "mutations": mutations,
        "package_executions": installed["executions"],
    }
    ...
    evidence: dict[str, object] = {
        ...
        "mode_specific": {
            "mode": mode,
            "required_inputs": list(_EXCLUSIVE_INPUTS[mode]),
        },
    }
    if ec_matrix is not None:
    ...
    return {
        ...
        "recomputed_input_digest": _canonical_digest(input_facts),
        "recomputed_evidence_digest": _canonical_digest(outcome_facts),
        "evidence": evidence,
    }
