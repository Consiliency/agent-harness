...
def _validate_chronology(facts: dict[str, Any], bindings: dict[str, Any], mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(actual_rebased) in (5, 6):
        ...
        inserted_test_patch = _run_git(["diff", "--no-ext-diff", "--no-textconv", "--no-color", "-U0", actual_rebased[3], inserted_commit, "--", "phase-loop-runtime/tests/test_outside_agent_conform_evidence.py"])
        if hashlib.sha256(inserted_test_patch.encode("utf-8")).hexdigest() != "d5114ade824f58efab2dc2f3a1c3e89293678252f80079c73b59bb87b9df6f4d":
            raise ValueError("inserted verifier test repair patch mismatch")
