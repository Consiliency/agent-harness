...
**Diagnostic offset-integrity (threat model).** Each failing stage's `raw_tail` is sliced from its recorded `[log_offset, log_end_offset)` region, validated fail-closed against its execution-order neighbours' recorded offsets (`prev.log_end_offset <= log_offset <= log_end_offset <= next.log_offset`, bounded by `[0, len(log)]`); any violation yields an empty tail, never a slice into an adjacent stage. This makes a **single-field** offset tamper (which produces an inconsistent chain) fail closed.

**Whole-artifact integrity (agent-harness#243).** Beyond the per-stage offset table (which `log_sha256` does not cover), the ENTIRE `verification.json` is sealed. At write time a canonical digest of the artifact payload (all fields except the derived `log_sha256`) is embedded as the `verification-artifact-sha256:<hex>` trailer line in `verification.log`, and then `log_sha256` is computed over the whole log — so the log's SHA also seals the artifact digest. At validate time, on the would-be-PASS path (after the `nonzero_exit` / integrity branches), the digest is recomputed from `verification.json` and compared to the sealed trailer; a mismatch yields `artifact_seal_mismatch` and fails closed. This catches a **multi-field / structural** edit the per-stage checks miss — most importantly a verdict-flipping edit such as deleting a failed `commands[]` entry to forge a pass. The seal is keyless (an editor who can rewrite the artifact, the log trailer, AND recompute `log_sha256` can still re-seal — the same trust level #209 assumed), so this raises the bar to a coordinated multi-file edit rather than being cryptographically unforgeable; it is defense-in-depth. **Back-compat:** an artifact whose log carries no seal trailer (a v1/older run, or an externally-built log) skips the seal check and still validates. Separately, an artifact larger than `MAX_ARTIFACT_BYTES` is rejected `oversized_artifact` before it is parsed.
