...
class VerificationEvidenceTest(unittest.TestCase):
    ...
    def test_proofgate_v3_rejects_missing_duplicate_extra_substituted_or_surviving_parameter(self):
        ...
        def _contract():
            ...
            if hasattr(verification_evidence, "_validate_v3_extensions"):
                try:
                    verification_evidence._validate_v3_extensions({})
                except ValueError as exc:
                    if str(exc) == "mutated":
                        emit_mutation_observable("ec-proofgate-5.parameter-coverage", getattr(self, "record_property", None))
                        raise AssertionError("EC-5 mutation active in _validate_v3_extensions") from exc
                    raise
