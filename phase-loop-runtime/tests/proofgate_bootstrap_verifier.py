...
def _verifier_validate_candidate_binding(
    repo_path: Path | str | None = None,
    candidate_binding_path: Path | str | None = None,
) -> dict[str, Any]:
    ...
    if candidate_binding_path is not None:
        if repo_path is None:
            raise ProofgateBootstrapVerifierError(
                "Explicit candidate binding validation requires repo_path"
            )
        expect = "1"
        binding_str = str(candidate_binding_path)
    ...
    target_repo_path = Path(repo_path or Path.cwd()).resolve()
...
def verify_coordinator_evidence_capture(
    ...
    *,
    ...
    if mode not in evidence_map:
        raise ProofgateBootstrapVerifierError(f"unknown coordinator evidence mode: {mode}")
    junit_filename, report_filename = evidence_map[mode]
    ...
    cand_binding_before = {}
    if mode == "bootstrap_candidate" and repo_path is not None:
        cand_binding_path = root / "bootstrap-candidate-binding.json"
        cand_binding_before = _verifier_validate_candidate_binding(
            repo_path=repo_path,
            candidate_binding_path=cand_binding_path,
        )
        source_digest = cand_binding_before.get("source_digest", "")
        binding_digest = cand_binding_before.get("binding_digest", "")
        if (
            not source_digest
            or not binding_digest
            or not HEX_64_RE.fullmatch(source_digest)
            or not HEX_64_RE.fullmatch(binding_digest)
            or cand_binding_before.get("status") != "verified"
        ):
            raise ProofgateBootstrapVerifierError(
                "Armed candidate mode requires valid candidate binding validation yielding non-empty digests"
            )

    accounting = verify_junit_accounting(
        junit_bytes.decode("utf-8"),
        mode,
        phase_reports=root / report_filename,
    ...
    if mode == "bootstrap_candidate" and repo_path is not None:
        cand_binding_path = root / "bootstrap-candidate-binding.json"
        cand_binding_after = _verifier_validate_candidate_binding(
            repo_path=repo_path,
            candidate_binding_path=cand_binding_path,
        )
        if cand_binding_after != cand_binding_before:
            raise ProofgateBootstrapVerifierError("Candidate binding or Git facts drifted during verification")
        accounting["source_digest"] = cand_binding_before["source_digest"]
        accounting["binding_digest"] = cand_binding_before["binding_digest"]
    ...
    if mode == "bootstrap_candidate":
        ...
        if expect == "1" or binding_str:
            ...
            if (
                ...
                or not HEX_64_RE.fullmatch(source_digest)
                or not HEX_64_RE.fullmatch(binding_digest)
                or candidate_binding_after.get("status") != "verified"
            ):
                raise ProofgateBootstrapVerifierError(
                    "Armed candidate mode requires valid candidate binding validation yielding non-empty digests"
                )
...
def _validate_run_dir_output(output_str: str) -> Path:
    ...
    unresolved_run_dir = Path(run_dir_str.strip())
    if not unresolved_run_dir.is_absolute():
        raise ProofgateBootstrapVerifierError(f"PHASE_LOOP_RUN_DIR path '{unresolved_run_dir}' must be an absolute path")
    if not unresolved_run_dir.exists() or not unresolved_run_dir.is_dir():
        raise ProofgateBootstrapVerifierError(f"PHASE_LOOP_RUN_DIR path '{unresolved_run_dir}' does not exist or is not a directory")
    if unresolved_run_dir.is_symlink() or any(parent.is_symlink() for parent in unresolved_run_dir.parents):
        raise ProofgateBootstrapVerifierError(f"PHASE_LOOP_RUN_DIR path '{unresolved_run_dir}' or a parent is a symlink")

    run_dir = unresolved_run_dir.resolve()
    if not run_dir.is_dir():
        raise ProofgateBootstrapVerifierError(f"Resolved PHASE_LOOP_RUN_DIR path '{run_dir}' is not a directory")
    if run_dir.is_symlink() or any(parent.is_symlink() for parent in run_dir.parents):
        raise ProofgateBootstrapVerifierError(f"Resolved PHASE_LOOP_RUN_DIR path '{run_dir}' or a parent is a symlink")

    unresolved_out = Path(output_str) if Path(output_str).is_absolute() else (unresolved_run_dir / Path(output_str))
    if unresolved_out.is_symlink() or any(parent.is_symlink() for parent in unresolved_out.parents):
        raise ProofgateBootstrapVerifierError(f"Output path '{unresolved_out}' or a parent is a symlink")
    if unresolved_out.exists():
        raise ProofgateBootstrapVerifierError(f"Output path '{unresolved_out}' already exists")

    out_path = unresolved_out.resolve()
    try:
