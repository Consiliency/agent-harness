...
def coordinator_evidence_capture_pytest_args(
    ...
    *,
    ...
    if nodeids is not None:
        ...
        nodeid_args = tuple(nodeid.replace("phase-loop-runtime/", "") for nodeid in EXPECTED_PHASE_NODEIDS)
...
def coordinator_evidence_capture_argv(
    ...
    *,
    ...
    return (*prefix, sys.executable, "-m", "pytest", *coordinator_evidence_capture_pytest_args(mode, evidence_by_mode=evidence_by_mode, nodeids=nodeids))
...
def verify_observed_premerge_bootstrap_review_gate(
    ...
    *,
    ...
    if landing_kind == "PR-B":
        ...
        actual_cand_digests = {
            filename: hashlib.sha256(_read_coordinator_artifact(coordinator_root, filename)).hexdigest()
            for filename in required_cand_artifacts
        }
        ...
        if (
            len(obs_cand_digests_raw) != len(actual_cand_digests)
            or set(obs_cand_digests) != set(actual_cand_digests)
            or obs_cand_digests != actual_cand_digests
        ):
            raise ProofgateBootstrapVerifierError("coordinator observation candidate artifact digest mismatch")
...
class ProofgateAdminControlPlaneBoundary:
    ...
    def observe(self, request: Any = None) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not isinstance(installations_payload, dict):
                raise ProofgateObservationUnavailable("installations payload is not a JSON object")
            installations_list = installations_payload.get("installations")
            if not isinstance(installations_list, list):
                raise ProofgateObservationUnavailable("installations list is not a JSON list")
            ...
            if not isinstance(ruleset_rows, list):
                raise ProofgateObservationUnavailable("ruleset rows is not a JSON list")
