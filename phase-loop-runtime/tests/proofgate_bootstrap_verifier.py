...
def verify_proofgate_admin_authority(
    ...
    stored_identity.pop("candidate_oid")
...
def verify_junit_accounting(junit_xml_str: str, mode: str, *, phase_reports: list[dict[str, Any]] | Path | str | None = None, runner_envelope: dict[str, Any] | None = None, expected_attended_head_identity: str | None = None, expected_attended_stage: str | None = None, expected_attended_module_identity: str | None = None, strict_nodeids: bool = True) -> dict[str, Any]:
    ...
    """
    if mode == "bootstrap_candidate" and not (
        isinstance(junit_xml_str, (str, Path)) and os.path.isfile(str(junit_xml_str))
    ):
        raise ProofgateBootstrapVerifierError("Candidate mode requires a JUnit artifact file")
    if mode == "bootstrap_candidate" and not (
        isinstance(phase_reports, (str, Path)) and os.path.isfile(str(phase_reports))
    ):
        raise ProofgateBootstrapVerifierError("Candidate mode requires a phase-report artifact file")
...
class ProofgateAdminControlPlaneBoundary:
    ...
    def observe(self, request: Any = None) -> dict[str, Any] | None:
        ...
        try:
            ...
            installations_payload = self._gh_api(f"orgs/{self._ORGANIZATION}/installations")
            installations = [
                row
                for row in installations_payload.get("installations", [])
            ...
            member_proc = subprocess.run(
                ["gh", "api", f"orgs/{self._ORGANIZATION}/members/{reviewer.get('login')}"],
                capture_output=True,
                timeout=10,
            )
            if member_proc.returncode != 0:
                raise ProofgateObservationUnavailable("environment reviewer is not an active organization member")

            ruleset_rows = self._gh_api(f"repos/{self._REPOSITORY}/rulesets")
            matching_rulesets = [
                row
            ...
            ref_includes = ruleset.get("conditions", {}).get("ref_name", {}).get("include", [])
            ...
            permissions = tuple(sorted((str(key), str(value)) for key, value in installation.get("permissions", {}).items()))
            normalized_selected = [
            ...
            normalized_bypass = [
                ...
                for row in ruleset.get("bypass_actors", [])
            ]
            return {
                "repository": {
                    "id": str(repository.get("id")),
                    "name": repository.get("full_name"),
                    "owner_id": str(repository.get("owner", {}).get("id")),
                },
                "app": {
                    "id": str(app.get("id")),
                    "slug": app.get("slug"),
                    "owner_id": str(app.get("owner", {}).get("id")),
                },
                "installation": {
                    "id": installation_id,
                ...
                "ruleset": {
                    ...
                    "target_ref": self._EXTERNAL_REF,
                    "bypass_actors": normalized_bypass,
                },
                "broker": {key: value for key, value in broker.items() if key != "schema"},
            }
        except (ProofgateObservationUnavailable, OSError, ValueError, TypeError, json.JSONDecodeError):
            return None
