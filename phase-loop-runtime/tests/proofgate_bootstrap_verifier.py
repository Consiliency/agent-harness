...
def verify_observed_premerge_bootstrap_review_gate(
    ...
    *,
    ...
    if landing_kind == "PR-B":
        ...
        if (coordinator_root / "selector-repair-review-binding.json").is_file():
            verify_selector_repair_review_binding(
                ...
                expected_selector_repair_landing=base_oid,
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "admin-preflight":
        if (
            os.environ.get("PHASE_LOOP_PROOFGATE_ATTENDED_LIVE") != "1"
            and os.environ.get("PHASE_LOOP_PROOFGATE_OPERATOR_OPT_IN") != "1"
        ):
            sys.stderr.write("Verifier error: admin-preflight requires explicit attended operator opt-in\n")
            return 1
        if (
            repo_arg != expected.repository_name
