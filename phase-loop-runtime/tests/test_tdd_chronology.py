...
def _pr_b_write_decisive_bootstrap_artifacts(
    ...
    *,
    ...
    cand_pytest_args = coordinator_evidence_capture_pytest_args(
        "bootstrap_candidate",
        run_dir=run_dir,
    )
    ...
    def contract():
        ...
        def strict_binding_already_verified(*, binding_path, **_kwargs):
            ...
            return {
                ...
                "binding_digest": hashlib.sha256(binding_bytes).hexdigest(),
                "source_digest": binding_data["diff_sha256"],
            }
...
def test_pr_r_blocker_fable_f007_producer_output_satisfies_decisive_pr_b_verifier_contract(monkeypatch):
    ...
    cand_args = coordinator_evidence_capture_pytest_args("bootstrap_candidate")
...
def test_pr_r_blocker_fable_f008_real_producer_subprocess_execution_satisfies_verifier_contract(monkeypatch):
    f008_nodeid = "phase-loop-runtime/tests/test_tdd_chronology.py::test_pr_r_blocker_fable_f008_real_producer_subprocess_execution_satisfies_verifier_contract"
    if f008_nodeid not in sys.argv:
        has_capabilities = False
        try:
            from phase_loop_runtime import proofgate_receipts, launcher
            has_receipts = hasattr(proofgate_receipts, "ProofgateAttestationService") or hasattr(proofgate_receipts, "ProofgateReceiptSupervisor")
            has_launcher = hasattr(launcher, "build_proofgate_executable_launch_spec") and hasattr(launcher, "execute_proofgate_launch_spec")
            if has_receipts and has_launcher:
                has_capabilities = True
        except ImportError:
            pass
        if not has_capabilities:
            pytest.skip("this exact F008 is retained cross-landing RED and coordinator PR-R evidence deselects it")
    ...
    cand_binding_path = coordinator_root / "bootstrap-candidate-binding.json"
    monkeypatch.setenv("PHASE_LOOP_RUN_DIR", str(coordinator_root))
    monkeypatch.setenv("PHASE_LOOP_RUN_ID", "coordinator-candidate")
    monkeypatch.setenv("PHASE_LOOP_CANDIDATE_OID", candidate_oid)
    ...
    assert proc.returncode == 0, (
        f"FABLE-F008::real_producer_must_succeed: Producer subprocess expected returncode 0, got {proc.returncode}:\n"
        f"{combined_out}"
    )
    ...
    with _coordinator_run_dir(coordinator_root):
        verdict = verify_coordinator_evidence_capture(
            coordinator_root,
            "bootstrap_candidate",
            expected_candidate_oid=candidate_oid,
            evidence_by_mode={
    ...
    assert verdict.get("schema") == "proofgate_bootstrap_candidate_verdict.v1"
    assert verdict.get("status") == "verified"
    assert verdict.get("authorized_scope") == "sl0_b1_bootstrap_candidate_only"
    assert verdict.get("collected") == 11
    assert verdict.get("passed") == 11
    assert verdict.get("skipped") == 0
    assert verdict.get("failed") == 0
    assert verdict.get("errors") == 0
    assert verdict.get("authorizes_implementation") is False
    assert verdict.get("authorizes_final_completion") is False
    assert verdict.get("runner_envelope") is None
    assert verdict.get("mode") == "bootstrap_candidate"

    reports = verdict.get("reports", [])
    report_nodeids = tuple(
        r["nodeid"] for r in reports if isinstance(r, dict) and "nodeid" in r
    )
    assert report_nodeids == PR_B_BOOTSTRAP_CANDIDATE_NODEIDS
    assert all(report.get("candidate") == candidate_oid for report in reports)
    assert all(report.get("run_identity") == "coordinator-candidate" for report in reports)
    ...
    actual_junit_sha256 = hashlib.sha256(cand_junit.read_bytes()).hexdigest()
    assert verdict.get("junit_digest") == actual_junit_sha256

    actual_reports_sha256 = hashlib.sha256(cand_reports.read_bytes()).hexdigest()
    assert verdict.get("phase_reports_digest") == actual_reports_sha256

    actual_binding_sha256 = hashlib.sha256(cand_binding_path.read_bytes()).hexdigest()
    assert verdict.get("binding_digest") == actual_binding_sha256

    assert verdict.get("selector_digest") == PR_B_BOOTSTRAP_CANDIDATE_NODEIDS_SHA256

    binding_payload = json.loads(cand_binding_path.read_text(encoding="utf-8"))
    assert verdict.get("source_digest") == binding_payload.get("diff_sha256")

    tmp.cleanup()


def test_pr_r_blocker_fable_f008_verifier_requires_candidate_binding_and_whole_file_reports_digest(monkeypatch):
    try:
        from . import proofgate_bootstrap_verifier
        from .proofgate_bootstrap_verifier import (
            verify_coordinator_evidence_capture,
        ...
        raise ProofgateMissingCapabilityError(
            "FABLE-F008::verifier_contract_unimplemented"
        ) from exc
    ...
    cand_junit_xml, cand_reports_list = _candidate_phase_reports_and_junit()
    cand_junit.write_text(cand_junit_xml, encoding="utf-8")

    cand_pytest_args = coordinator_evidence_capture_pytest_args(
        "bootstrap_candidate",
        run_dir=coordinator_root,
    )
    cand_argv = ["pytest", *cand_pytest_args]
    cand_cmd_digest = hashlib.sha256(json.dumps(cand_argv).encode("utf-8")).hexdigest()
    for report in cand_reports_list:
        report["argv"] = list(cand_argv)
        report["command_digest"] = cand_cmd_digest
        report["candidate"] = candidate_oid
        report["run_identity"] = "coordinator-candidate"
    ...
    cand_phase_payload = {
        "schema": "proofgate_phase_reports.v1",
        "exitstatus": 0,
        "runs": [{"run_identity": "coordinator-candidate", "exitstatus": 0, "reports": cand_reports_list}],
        "reports": cand_reports_list,
        "capture": {
            "schema": "proofgate_coordinator_evidence_capture.v1",
            "plugin": "tests.proofgate_tdd_guard",
            "mode": "bootstrap_candidate",
            ...
            "run_identity": "coordinator-candidate",
            "junit_family": "legacy",
            "junit_filename": "compat-candidate.junit.xml",
            "junit_sha256": hashlib.sha256(cand_junit.read_bytes()).hexdigest(),
            "pytest_args_sha256": hashlib.sha256(
                json.dumps(list(cand_pytest_args)).encode("utf-8")
            ).hexdigest(),
        },
    }
    cand_reports.write_bytes(json.dumps(cand_phase_payload, sort_keys=True, separators=(",", ":")).encode("utf-8"))

    # A non-authoritative parser call remains unbound for compatibility fixtures.
    monkeypatch.setenv("PHASE_LOOP_RUN_DIR", str(coordinator_root))
    monkeypatch.setenv("PHASE_LOOP_RUN_ID", "coordinator-candidate")
    monkeypatch.setenv("PHASE_LOOP_CANDIDATE_OID", candidate_oid)
    monkeypatch.delenv("PHASE_LOOP_TDD_EXPECT_PROOFGATE_BOOTSTRAP_CANDIDATE", raising=False)
    monkeypatch.delenv("PHASE_LOOP_TDD_PROOFGATE_BOOTSTRAP_BINDING", raising=False)

    unbound_verdict = verify_coordinator_evidence_capture(
        coordinator_root,
        "bootstrap_candidate",
        expected_candidate_oid=candidate_oid,
        evidence_by_mode={"bootstrap_candidate": ("compat-candidate.junit.xml", "phase_reports_candidate.json")},
    ...
    assert unbound_verdict.get("source_digest") == ""
    assert unbound_verdict.get("binding_digest") == ""

    # Supplying the candidate repository makes the coordinator verifier authoritative.
    verdict = verify_coordinator_evidence_capture(
        coordinator_root,
        "bootstrap_candidate",
        expected_candidate_oid=candidate_oid,
    ...
    assert verdict.get("status") == "verified"
    assert verdict.get("phase_reports_digest") == hashlib.sha256(cand_reports.read_bytes()).hexdigest()
    assert verdict.get("binding_digest") == hashlib.sha256(cand_binding_path.read_bytes()).hexdigest()
    assert isinstance(verdict.get("source_digest"), str) and len(verdict.get("source_digest")) == 64
    assert verdict.get("source_digest") != "0" * 64

    # Explicit authoritative binding with a missing artifact fails closed.
    cand_binding_path.unlink()
    with pytest.raises(ProofgateBootstrapVerifierError):
        verify_coordinator_evidence_capture(
            coordinator_root,
            "bootstrap_candidate",
...
def test_pgr_blk_1_verify_junit_accounting_signature_frozen():
    verifier_path = Path(__file__).parent / "proofgate_bootstrap_verifier.py"
    source = verifier_path.read_text(encoding="utf-8")
    matching_lines = [
        line for line in source.splitlines() if line.startswith("def verify_junit_accounting(")
    ]
    assert matching_lines, "PGR-BLK-1::verify_junit_accounting_signature_drift: signature not found"
    actual_sig = matching_lines[0]
    expected_sig = (
        "def verify_junit_accounting(junit_xml_str: str, mode: str, *, "
        "phase_reports: list[dict[str, Any]] | Path | str | None = None, "
        "runner_envelope: dict[str, Any] | None = None, "
        "expected_attended_head_identity: str | None = None, "
        "expected_attended_stage: str | None = None, "
        "expected_attended_module_identity: str | None = None, "
        "strict_nodeids: bool = True) -> dict[str, Any]:"
    )
    assert "repo_path" not in actual_sig, (
        f"PGR-BLK-1::verify_junit_accounting_signature_drift: repo_path present in signature: {actual_sig!r}"
    )
    assert "candidate_binding_path" not in actual_sig, (
        f"PGR-BLK-1::verify_junit_accounting_signature_drift: candidate_binding_path present in signature: {actual_sig!r}"
    )
    assert actual_sig == expected_sig, (
        f"PGR-BLK-1::verify_junit_accounting_signature_drift: expected {expected_sig!r}, got {actual_sig!r}"
    )


def test_pgr_blk_2_runner_owned_evidence_root_required(monkeypatch, tmp_path):
    from .proofgate_bootstrap_verifier import (
        ProofgateBootstrapVerifierError,
        _validate_run_dir_output,
    )

    nonexistent_root = tmp_path / "nonexistent_run_dir"
    assert not nonexistent_root.exists()
    monkeypatch.setenv("PHASE_LOOP_RUN_DIR", str(nonexistent_root))

    try:
        _validate_run_dir_output("evidence_output.json")
        pytest.fail(
            "PGR-BLK-2::runner_owned_evidence_root_required: missing PHASE_LOOP_RUN_DIR was not rejected"
        )
    except ProofgateBootstrapVerifierError:
        pass

    assert not nonexistent_root.exists(), (
        "PGR-BLK-2::runner_owned_evidence_root_required: root directory was created for missing PHASE_LOOP_RUN_DIR"
    )
    assert not (nonexistent_root / "evidence_output.json").exists(), (
        "PGR-BLK-2::runner_owned_evidence_root_required: output file was created for missing PHASE_LOOP_RUN_DIR"
    )

    real_target_dir = tmp_path / "real_target_dir"
    real_target_dir.mkdir()
    symlink_root = tmp_path / "symlink_run_dir"
    symlink_root.symlink_to(real_target_dir, target_is_directory=True)
    monkeypatch.setenv("PHASE_LOOP_RUN_DIR", str(symlink_root))

    try:
        _validate_run_dir_output("evidence_output.json")
        pytest.fail(
            "PGR-BLK-2::runner_owned_evidence_root_required: symlink PHASE_LOOP_RUN_DIR was not rejected before resolution"
        )
    except ProofgateBootstrapVerifierError:
        pass

    assert symlink_root.is_symlink(), (
        "PGR-BLK-2::runner_owned_evidence_root_required: symlink PHASE_LOOP_RUN_DIR was modified or unlinked"
    )
    assert not (real_target_dir / "evidence_output.json").exists(), (
        "PGR-BLK-2::runner_owned_evidence_root_required: output file was created inside symlink target directory"
    )
    assert not (symlink_root / "evidence_output.json").exists(), (
        "PGR-BLK-2::runner_owned_evidence_root_required: output file was created under symlink directory"
    )
