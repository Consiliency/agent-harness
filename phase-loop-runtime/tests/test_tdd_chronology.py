...
def test_pr_r_blocker_terra_prr_001_binds_exact_reviewed_selector_repair(monkeypatch):
    try:
        from . import proofgate_bootstrap_verifier
        from .proofgate_bootstrap_verifier import (
            ProofgateAdminControlPlaneBoundary,
        ...
        raise ProofgateMissingCapabilityError(
            "TERRA-PRR-001::selector_repair_review_binding_unimplemented"
        ) from exc
    ...
    monkeypatch.setattr(
        ...
        "verify_selector_repair_review_binding",
        lambda **_kwargs: (_ for _ in ()).throw(
            ProofgateBootstrapVerifierError("TERRA-PRR-001::review_binding_gate_reached")
        ),
    )
    with _coordinator_run_dir(root_a), pytest.raises(
        ProofgateBootstrapVerifierError,
        match="TERRA-PRR-001::review_binding_gate_reached",
    ):
        verify_observed_premerge_bootstrap_review_gate(
            repo_a,
            landing_a,
...
def test_pr_r_blocker_prr_evidence_provenance_003_candidate_identity(
    ...
    _sync_pr_b_candidate_artifact_digests(
        proofgate_bootstrap_verifier, coordinator_root, PR_B_CANDIDATE_ARTIFACTS
    )
    _freeze_coordinator_root(coordinator_root)
...
def test_pr_r_blocker_prr_admin_005_binds_environment_and_ruleset_ids(monkeypatch):
    from . import proofgate_bootstrap_verifier
    from .proofgate_bootstrap_verifier import verify_proofgate_admin_identity_binding
...
def test_pr_r_blocker_panel_r3_admin_preflight_requires_operator_opt_in(
    ...
    output = tmp_path / "admin-preflight.json"
    argv = [
        "admin-preflight",
        "--repo",
    ...
    assert proofgate_bootstrap_verifier.main(argv) == 1
    assert calls == []
    assert not output.exists()
    assert "explicit attended operator opt-in" in capsys.readouterr().err

    monkeypatch.setenv("PHASE_LOOP_PROOFGATE_ATTENDED_LIVE", "1")
