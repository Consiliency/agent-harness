...
def test_provider_projection_allows_only_selected_vendor_subscription_material(record_property):
    ...
    def _contract():
        ...
        if not proofgate_attended_live():
            ...
            if not isinstance(turn_results, dict) or not turn_results.get("verified"):
                raise RuntimeError("Attended live mode provider supervisor verification failed")

            for prov_case in ATTENDED_REAL_PROVIDER_CASES:
                record = turn_results["records"][prov_case]
                assert record.get("request_count", 0) > 0
                assert "nonce" in record
                record_property(prov_case, record["property_value"])
