...
"""Run, locally and in a CI-like environment, the checks your change can affect (agent-harness#1029).
    ...
    python3 phase-loop-runtime/scripts/local_check.py [--base REF] [--full] [--host-env] [--dry-run]
...
def changed_files(repo: Path, base: str) -> list[str]:
    def git(*args: str) -> list[str]:
        out = subprocess.run(["git", *args], cwd=repo, check=True, capture_output=True, text=True).stdout
...
def imports_module(text: str, dotted: str) -> bool:
    """Does this test source import `dotted` (as `import a.b`, `from a.b import`, or `from a import b`)?"""
    parent, _, leaf = dotted.rpartition(".")
    if re.search(rf"(?m)^\s*(?:import|from)\s+{re.escape(dotted)}\b", text):
        return True
    if parent and re.search(rf"(?m)^\s*from\s+{re.escape(parent)}\s+import\s+\(?[^)\n]*\b{re.escape(leaf)}\b", text):
        return True
    return False
...
def check_python(repo: Path) -> str:
    """$PYTHON if set; otherwise a cached venv built with CI's install line (via uv).
    ...
    uv = shutil.which("uv")
    if uv is None:
        sys.exit("local_check: set PYTHON to an interpreter with the test deps, or install uv")
    print(f"local_check: provisioning {venv.relative_to(repo)} (CI's test deps; one-time)")
    subprocess.run([uv, "venv", "-q", "--allow-existing", "--python", "3.10", str(venv)], cwd=repo, check=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    tests, reasons = (None, ["--full"]) if args.full else select(pkg_root, files)
    print(f"local_check: {len(files)} changed file(s) vs {args.base}")
    for reason in reasons:
        print(f"  - {reason}")

    python = sys.executable if args.dry_run and not os.environ.get("PYTHON") else check_python(repo)
    ...
    print(f"  lint : {' '.join(lint_cmd) if lint_cmd else 'SKIPPED (uvx not found; CI pins ' + RUFF_PIN + ')'}")
    ...
    if tests != []:
        ...
        status |= subprocess.run(pytest_cmd, cwd=pkg_root, env=env).returncode
