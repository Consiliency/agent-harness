...
# LEGIBLE: discovery resolves the roadmap whose OWN banner declares ACTIVE — not a superseded one.
# REPLACES a `worktree-index --help` smoke that stayed green regardless (it proved only that a binary
# exists, never that any roadmap reports its own state). Genuine falsifiable gate, coupled to
# EC-LEGIBLE-7: RED while select_roadmap() returns the superseded convergence-v1 (the state today),
# GREEN the moment discovery is repointed to the ACTIVE-bannered roadmap.
PYTHONPATH=phase-loop-runtime/src python3 - <<'PY'
from pathlib import Path
from phase_loop_runtime.discovery import select_roadmap
resolved = select_roadmap(Path(".")).name
assert resolved == "phase-plans-v10.md", f"discovery resolves {resolved!r}, not the ACTIVE-bannered roadmap"
PY
