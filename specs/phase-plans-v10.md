...
- [ ] EC-REVIEWTRUTH-15 — **THE `agent-harness#398` LEG-CAPABILITY DESIGN IS DECIDED AND MAINTAINER-RATIFIED BEFORE ANY POSTURE-ASSUMING IMPLEMENTATION LANDS.** Lane D produces the capability design (e.g. probe execution against a THROWAWAY staged copy vs today's read-only bundle review) and a maintainer ratification RECORD exists (issue comment or disposition doc referenced from the implementing PR) before any Phase-1 change that encodes a seat-capability posture (typed-outcome semantics, grounding rules, leg launch flags in `panel_invoker.py`) merges; the ratified design may be "keep read-only" — the criterion binds the DECISION, not a direction. The ordering AND the content are bound by GIT ANCESTRY, not timestamps or references: the ratification record is a committed disposition document whose commit is an ANCESTOR of the posture-assuming merge commit (`git merge-base --is-ancestor <record> <merge>`), cited by full SHA in the MERGE COMMIT MESSAGE. Ancestry is structural — it cannot be backdated (committer timestamps are author-controlled and prove nothing), guarantees the record PRECEDED the merge by construction, and guarantees durable reachability (an ancestor of a main-merged commit cannot be GC-pruned). No other record form qualifies: a bare issue comment's content is editable after creation; decision text embedded in the posture merge's own message is CONTEMPORANEOUS, not prior, and does not satisfy the ordering. Completion is also POSITIVE: the cited record must actually STATE the ratified design. Falsified by a merged posture-assuming PR whose merge commit cites no record SHA, cites a SHA that is not its ancestor, or cites one that does not state a decision — each checkable forever by ancestry alone, immune to backdating, body edits, and branch pruning.
