...
Six phases are independent roots and may be planned and executed concurrently:
**LEGIBLE, REVIEWTRUTH, PROOFGATE, CONFORM, FABPUB, HARDEN**. RESIDUAL is NOT a root — it
`Depends on` FABPUB (its lane A rewrites the `verbs.py`/`train_runner.py` publish identity FABPUB
owns), so it must be planned and executed only after FABPUB's publish-identity migration lands.
...
3. **FABPUB** whenever capacity allows; it is the critical path and has no upstream dependency
   (RESIDUAL now hangs off it downstream, so start FABPUB before, never beside, RESIDUAL).
