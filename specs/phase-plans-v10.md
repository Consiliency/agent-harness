...
- [ ] EC-REVIEWTRUTH-17 — **ORDERED EARLY-PROVER EVIDENCE IS ROLE-SEPARATED, EXTERNAL-TOOL-CLOSED, ISOLATED, REAPED, AND CONTRADICTION-SAFE.** The additive review-wave path leaves `PANEL_LEGS` and non-review goldens byte-stable. At plan/design gates, critics-first remains permitted, but one serial early-prover holder is still present in the initial wave and its evidence is bundled before Fable. At pre-merge/release gates, Codex probes first and stages evidence before critics; critic verdicts over a bundle without that evidence do not count. Every subsequent wave repeats the early prover. The primary Codex command uses a coordinator-created per-leg ephemeral worktree under `/mnt/workspace/worktrees/` when present, never today's non-review `danger-full-access` branch or review read-only staged-copy branch, and contains `--sandbox workspace-write`, `--strict-config`, `--ephemeral`, `--ignore-user-config`, and `--ignore-rules` plus exact `approval_policy="never"`, `web_search="disabled"`, `sandbox_workspace_write.network_access=false`, and `sandbox_workspace_write.writable_roots=[]` overrides. It adds no directory, remote, search, browser, or hook-trust authority; explicitly empties the effective enabled MCP-server set by replacing every discovered layered server with a same-transport inert disabled definition; and explicitly disables every installed external or dynamic surface, including apps, local/remote plugins and plugin hooks, hooks, browser/CDP/computer use, MCP dependency installation/elicitation, web/image/remote-control tools, collaboration/multi-agent/fanout, skills or memories with external dependencies, and any current equivalent. Before launch, the coordinator runs an effective-state preflight against the same canonical Codex executable and exact override tuple; freezes the executable realpath/digest, version and help digests, complete feature-name/value inventory, effective workspace-write network/writable-root values, configured MCP name/transport inventory plus the post-override zero-enabled result, effective plugin inventory, and the canonical external-tool policy; and rejects a nonzero or unparseable probe, any unknown relevant surface, enabled surface, locked or ineffective override, changed executable/inventory/configuration, extra network/writable-root authority, or uninspectable state. Codex's shell sandbox is not evidence for MCP or other host-side tool confinement. A rejected preflight emits a digest-bound degraded no-launch receipt with a zero Codex process count; Codex may run only after a passing effective receipt. Each holder has a distinct data-scope identity and reset/cleanup receipt; only one write-capable holder may exist; a coordinator-side reaper removes crash residue; and the canonical external-tool policy, effective preflight receipt, and launch/no-launch receipt digests are bound with the exact input, base HEAD/tree, staged bundle, worktree, data-scope receipt, output, and evidence-bundle digests. If the CLI cannot prove the required state, Codex runs only under positive OS-level confinement that independently excludes live-tree/shared-data/external-tool authority; otherwise the degraded receipt records no Codex launch. Codex/Grok can attest `can_probe` but never `binding_prover`; only a usable grounded artifact-bound Fable review can attest `binding_prover` and satisfy effective `required_prover=true`. Grok is attempted only after that typed Codex failure and only under a positive real OS-confinement attestation; absent confinement emits degraded evidence and launches no Grok process. Gemini is never eligible. Any prover contradiction invalidates every affected earlier AGREE and forces those critics to re-review the updated bundle digest. Falsified by running a critic before evidence at pre-merge/release, counting an evidence-less critic, omitting the initial-wave early prover, launching two write holders, using the live worktree/shared data, relying on shell sandbox or user-config suppression as external-tool proof, permitting an enabled/unknown/locked/uninspectable external surface, extra network/writable-root authority, launching after failed preflight without OS confinement, failing to bind policy/preflight/receipt digests, accepting `danger-full-access` or read-only staging as the new path, leaving worktree/data residue after reaping, digest substitution, treating `can_probe` as binding, selecting Gemini, launching unconfined Grok, retaining an AGREE contradicted by evidence, or skipping re-review. Positive controls cover critics-first plan/design ordering, a fully inspectable zero-external-surface Codex launch, a confined Grok fallback after typed Codex preflight failure, and effective `required_prover=false` removing only the Fable hard block without altering the early-evidence ordering contract.
  ...
  `98c98c914479bc95eda6977a1e9cee6574b2cc6c28ab9460ad249eadebe54ca0`;
  ...
  `REVIEWTRUTH_RED::codex_external_tool_preflight`, and
  `REVIEWTRUTH_RED::external_tool_receipt_digest_binding`, with exact
  sorted-LF anchor SHA-256
  `f54bcf01ccfd2c8c92d790c2b34aa829b927412ae81182f67a8fcdb3b5dea991`.
