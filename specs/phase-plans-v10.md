...
| `phase-plans-v3.md` (model-routing-v2) | **SUPERSEDED — ABSORBED** | 2 of 4 phases DELIVERED (P2 real panel spawn, P3 planning gate). P1 (live pre-merge gate) AND P4 (`v3:188` end-to-end invariants) BOTH require the production block→fix→pass round, unwired (`runner.py:10140` `apply_fix=None`) — a fourth delivered-but-unmet; carried by EC-REVIEWTRUTH-8, which now carries both obligations |
| `phase-plans-v2.md` (model-routing-v1) | **SUPERSEDED — ABSORBED** | Same production fix-round gap as v3 → EC-REVIEWTRUTH-8 (P3 analog). v2's phase-level delivered-claims were taken from its banner, NOT re-derived from `#309`/`#310` commits — see the v2/v3 residual-risk caveat in the PR body. Shipped effort defaults (`execute/repair=high`, `review=max`) intentionally differ from the doc under #310 — a reconciliation, not a delivery gap |
| `phase-plans-v5.md` (Advisor Board) | **SUPERSEDED — ABSORBED** | Phases 1–4 and 7 DELIVERED. ABDOMNI + ABDOBS are built, wired and unit-tested but have ZERO production constructors → EC-LEGLIFE-6/7. The `<harness>-advisor-panel` bundle dirs are an INTENDED, test-enforced compat alias (`install_skills`/`SKILL_ALIASES`, byte-identical to `advisor-board` modulo `name:`, asserted by `test_advisor_board_alias_install.py`), NOT a stray — `v5:146` clause 3's pre-rename divergent duplicate is removed; nothing carried |
...
**EC-CONFORM-5, EC-CONFORM-6, and EC-CONFORM-7 are EXTERNALLY GATED on `Consiliency/spec` PR #102
(`spec#118`)** — lane B may prepare but must not close them while the contract is unmerged and
unpublished. **EC-CONFORM-8 is NOT externally gated** — it guards lane A's own dialect change
...
- [ ] EC-SCHED-7 — **WORKTREE-LOSS DISPOSITION (carried from v9 FAB lane (a), `phase-plans-v9.md:357`; v9's banner recorded it as "CANNOT-DETERMINE, not carried" (`v9:16`), and superseding v9 would otherwise make its acceptance condition silently unreachable — "cannot-determine" is not a disposition).** v9:357 required each of four divergent worktrees — `agent-harness-abdreg`, `ah-abdreg-pkg`, `ah-abdreg-rebase` (three copies on `feat/advisor-board-abdreg`) and `agent-harness-abdresolve` (`phase/abdresolve`) — to be landed, committed-and-parked, or explicitly discarded with a recorded decision, no silent loss. Disposition recorded (verified 2026-07-29, aligned to the honest record in `plans/design-fab-191-delta-review.md:514-521`, NOT smoothed): NONE of the four worktrees still exists under `git worktree list`; both branches `feat/advisor-board-abdreg` (`4c603c3`) and `phase/abdresolve` (`582037e`) survive on `origin` (confirmed by `git ls-remote`), so their COMMITTED state is parked-and-recoverable. The two uncommitted deltas differ and must NOT be conflated: (1) the `abdreg` copies (5 files) were INSPECTED — sibling copies revert committed safety fixes, an abandoned experiment, no value forgone (a genuine recorded discard); (2) `phase/abdresolve` (25 files) was **discarded UN-INSPECTED** — whether it was re-appearing already-committed work or genuine un-committed progress is UNKNOWABLE, and the FAB design record `design-fab-191-delta-review.md:514-521` explicitly DECLINES to claim "no silent loss" for it. So v9:357's "no silent loss" is fully held for the committed tips and for `abdreg`, but for `abdresolve` it is carried as a recorded POSSIBLE LOSS, stated plainly. This is NOT satisfiable by merely recording a decision — the un-inspected 25-file loss stands as stated. Satisfied by the two `origin` tips confirmed retained AND the `abdresolve` disposition carried as un-inspected-possible-loss matching the FAB design record. Falsified by either (a) `feat/advisor-board-abdreg` or `phase/abdresolve` being deleted from `origin` with no recorded decision — the observable is the missing ref, a silent loss of committed state; or (b) the disposition asserting `abdresolve` was cleanly reconciled or claiming "no silent loss" for it against `design-fab-191-delta-review.md:514-521`'s explicit refusal — the observable is a "clean"/"no silent loss" claim for `abdresolve` contradicting the recorded UNKNOWABLE
...
PROOFGATE ────────────┼─────────────→ SCHED
                      │
CONFORM ──────────────┤   [externally gated on spec#118 for EC-CONFORM-5/6/7]
                      │
FABPUB ───────────────┼─────────────→ FABREADMIT
                      │
HARDEN ───────────────┤
...
|---|---|---|
| `Consiliency/spec` PR #102 ratify-or-supersede | EC-CONFORM-5, EC-CONFORM-6, EC-CONFORM-7 | `spec#118` |
