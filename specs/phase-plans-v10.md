...
- [ ] EC-RESIDUAL-1 — **DEFERRED CARRY (obligation kept, NOT scheduled here) — `base` in the publish dedup key is the identity question that PARKED `agent-harness#368` and was ESCALATED to the maintainer (`plans/DISPOSITION-fab-288-p1-parked-20260729.md`, disposition `ba1b9fe`).** The obligation stands and is carried, not dropped: `publish_committed_branch` dedup keys should include `base` so a cross-base replay cannot skip the base scope-check (falsified by removing `base` from the dedup key and replaying the same branch across two bases, finding the second skips its scope-check — the observable is the dedup key tuple containing `base`). It CANNOT be scheduled as a concurrent root alongside `EC-FABPUB-4`: #368 round 9 overturned D-B3 to AC-17 (base folded into a 4-arg publish key because base is security-relevant at point of use), and rounds 10-11 then proved that change ORPHANS the append-only `EvidenceRecord`s (which carry no `base`) — the base-blind dual-read fallback replays BEFORE `execute`'s base check, the wrong-base replay AC-17 forbids. That self-defeating chain is why #368 is PARKED, not merged; base-in-publish-identity is an OPEN maintainer design call, unsettleable in this docs-consolidation PR. This criterion is therefore CARRIED as an open finding pointing at #368's identity resolution, and `EC-FABPUB-4` ships the status-quo `(repo, branch, head_sha)` publish key unchanged (it dedups crash-retries on committed identity and does NOT claim base-freedom), so the two are no longer contradictory concurrent roots. When #368's design call lands, this criterion re-derives against it.
