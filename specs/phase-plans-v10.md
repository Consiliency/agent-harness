...
- [ ] EC-REVIEWTRUTH-7 — A capped, empty, or TIMED-OUT leg is distinguishable from a leg that reviewed and found nothing, and is retried rather than counted; falsified by feeding an empty (capped) leg result OR a typed `leg_timeout` result and finding it counted as a reviewing seat rather than retried — the observable is the leg's typed status (capped/empty/timed-out) driving a retry, not a seat increment. The typed `leg_timeout` originates in LEGLIFE (EC-LEGLIFE-1: "a leg exceeding its bound is killed and reported as a typed timeout") and MUST reach this retry-not-count path — a timed-out leg that buckets "unusable" but is never retried is the cross-phase seam this closes, since `-7`'s prior "capped or empty" wording most likely excluded "timed out". Positive control: a leg that genuinely reviewed and returned no findings IS counted, not retried, proving the two are distinguished. Depends on LEGLIFE **lane A** — which owns timeout enforcement and the typed `leg_timeout` per LEGLIFE's Scope notes (the earlier "lane B" was WRONG; lane B owns the extensibility surface / per-repo seat declaration) — for THIS REVIEWTRUTH lane's validation: honest per-leg degradation is what makes a timeout distinguishable from a clean review. **KNOWN CROSS-PHASE ORDERING TENSION (flagged, NOT silently narrowed away):** this makes EC-7's validation CONSUME LEGLIFE lane A's `leg_timeout` — a LEGLIFE→REVIEWTRUTH content edge — while the phase DAG draws REVIEWTRUTH→LEGLIFE (because LEGLIFE lane B's custom-seat/lens work depends on REVIEWTRUTH making `Seat.lens` load-bearing). At LANE granularity it is ACYCLIC — LEGLIFE lane A (no REVIEWTRUTH dependency) → REVIEWTRUTH EC-7 → LEGLIFE lane B — but the coarse phase edge lumps both LEGLIFE lanes together, and the roadmap validator, which checks PHASE edges not CRITERION content, cannot see the criterion-level cycle. The obligation is NOT narrowed to dodge it; resolving it needs a plan restructure (sequence LEGLIFE lane A before REVIEWTRUTH, or originate `leg_timeout` in an earlier interface freeze) — a sequencing decision recorded for the maintainer, not forced by a wording change that would leave the seam weaker
