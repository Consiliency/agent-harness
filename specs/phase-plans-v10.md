...
- **IF-0-REVIEWTRUTH-4** — the ratification wave composes the existing early prober, a complete
  four-leg board whose Claude seat is `claude-opus-5` primary with typed Codex fallback on Opus
  preflight or availability failure, and a distinct sequential `claude-fable-5` president pass.
  The Opus/Codex board role is write-capable but attests only `can_probe`; only the later usable,
  grounded, artifact-bound Fable president attests `binding_prover`. Opus uses the canonical Claude
  subscription-TUI/native-fill adapter, never direct `claude -p`, inside IF-0-REVIEWTRUTH-3's
  isolated worktree, data scope, and serial lease. Its fail-closed preflight and digest-bound
  receipts prove the exact model, authenticated adapter, scrubbed environment, `Read,Write`-only
  tools, no Bash/shell/web/MCP authority, and roots limited to the staged review directory and
  per-leg worktree. All four board legs reach a terminal outcome before the president runs; critic
  verdicts are inputs and never dispatch gates.
- **IF-0-REVIEWTRUTH-5** — the Fable president records one exact-finding-text and artifact-digest
  bound `BLOCKING | DEFERRED` disposition plus rationale for every distinct board finding. Dispatch
  is blocked by any open `BLOCKING`; every `DEFERRED` requires a repository-qualified issue carrying
  the verbatim finding before dispatch. Findings that contradict staged prober evidence, an
  attestation claim, or a frozen-test invariant, plus findings originated by the president, are not
  deferrable. The president may request fix-and-re-review or unchanged-candidate cross-pollination,
  but must force a complete ruling by round three and may never start a fourth round.
...
- [ ] EC-REVIEWTRUTH-18 — **THE RATIFICATION BOARD IS EARLY-PROBER, THEN A COMPLETE FOUR-LEG BOARD WITH A READ-WRITE CLAUDE PROVER ROLE, THEN A SEPARATE FABLE PRESIDENT PASS.** The board's Claude seat uses `claude-opus-5` primary and typed Codex fallback only after Opus preflight or availability failure; this write-capable role attests `can_probe`, never EC-REVIEWTRUTH-16's `binding_prover`. Opus runs only through the canonical subscription-TUI/native-fill adapter under IF-0-REVIEWTRUTH-3 confinement, with fail-closed model/auth/tool/environment/root preflight and digest-bound launch/result receipts. After all four legs return or time out, a separate `claude-fable-5` president consumes their findings and alone may supply the usable grounded artifact-bound `binding_prover` fact. Critic dissent never skips the Opus/Codex role or the president. Falsified by omitting the write-capable Claude board role; selecting Fable into that board seat while Opus is available; granting Opus/Codex `binding_prover`; launching Opus with Bash, shell, web, MCP, unbounded roots, an unsanitized environment, or an ineffective preflight; accepting receipts not bound to the exact model, tools, roots, environment, and artifact digest; running the two Claude roles concurrently; or recording board findings without a later Fable president pass.

- [ ] EC-REVIEWTRUTH-19 — **THE FABLE PRESIDENT RULES LAST, PER FINDING, UNDER A HARD THREE-ROUND CAP.** For every distinct board finding, the president records `BLOCKING` or `DEFERRED` plus a rationale bound to the exact finding text and reviewed artifact digest. Between rounds it may request fix-and-re-review or unchanged-candidate cross-pollination. By round three it must classify every open finding; a fourth round is invalid. Dispatch cannot proceed with an open `BLOCKING`; every `DEFERRED` must already have a repository-qualified issue containing the verbatim finding; contradictions of staged prober evidence, attestation claims, or frozen-test invariants and president-originated findings are never deferrable. Falsified by a critic verdict preventing the prover or president from convening; dispatch with an open `BLOCKING`; an unbound or issue-less `DEFERRED`; a deferral of a non-deferrable or president-originated finding; a fourth president round; or findings without recorded per-finding dispositions.
...
**Spec closeout policy**
schema: `spec_delta_closeout.v1`; expected decision: `roadmap_amendment`; target surface:
`specs/phase-plans-v10.md`; `redaction_posture: metadata_only`; malformed evidence routes non-human
`blocker_class=contract_bug`.
