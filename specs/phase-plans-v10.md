...
- Granting any leg arbitrary execution against the live development tree or a shared development
  ...
  binding, one write-capable holder, and coordinator-side reaping. Grok is fallback-only after
  Codex preflight failure and only under real OS confinement; an unavailable confinement records
  degraded evidence and launches nothing. Gemini is ineligible. This does not make Codex or Grok a
  binding prover: only a usable grounded artifact-bound Fable review satisfies `required_prover`.
...
- **IF-0-REVIEWTRUTH-3** — REVIEWTRUTH owns an additive review-wave contract distinct from
  ...
  isolated per-leg data scope, one serial write-capable holder, and external reaping. Grok may run
  only after Codex preflight failure and only with a positive OS-confinement attestation; otherwise
  the coordinator records a degraded-evidence audit and does not launch it. Gemini is ineligible.
  Only usable grounded artifact-bound Fable attests `binding_prover`. Any prover finding that
  contradicts an earlier AGREE invalidates that AGREE and requires every affected critic to review
  the updated digest-bound bundle again.
...
- [ ] EC-REVIEWTRUTH-17 — **ORDERED EARLY-PROVER EVIDENCE IS ROLE-SEPARATED, ISOLATED, REAPED, AND CONTRADICTION-SAFE.** The additive review-wave path leaves `PANEL_LEGS` and non-review goldens byte-stable. At plan/design gates, critics-first remains permitted, but one serial early-prover holder is still present in the initial wave and its evidence is bundled before Fable. At pre-merge/release gates, Codex probes first and stages evidence before critics; critic verdicts over a bundle without that evidence do not count. Every subsequent wave repeats the early prover. The primary Codex command contains CLI-enforced `--sandbox workspace-write` and runs only in a coordinator-created per-leg ephemeral worktree under `/mnt/workspace/worktrees/` when present, never through today's non-review `danger-full-access` branch or today's review read-only staged-copy branch. Each holder has a distinct data-scope identity and reset/cleanup receipt; only one write-capable holder may exist; a coordinator-side reaper removes crash residue; and evidence binds the exact input, base HEAD/tree, staged bundle, worktree, data-scope receipt, output, and evidence-bundle digests. Codex/Grok can attest `can_probe` but never `binding_prover`; only a usable grounded artifact-bound Fable review can attest `binding_prover` and satisfy effective `required_prover=true`. Grok is attempted only after Codex preflight failure and only under a positive real OS-confinement attestation; absent confinement emits a degraded-evidence audit and launches no Grok process. Gemini is never eligible. Any prover contradiction invalidates every affected earlier AGREE and forces those critics to re-review the updated bundle digest. Falsified by running a critic before evidence at pre-merge/release, counting an evidence-less critic, omitting the initial-wave early prover, launching two write holders, using the live worktree/shared data, accepting `danger-full-access` or read-only staging as the new path, leaving worktree/data residue after reaping, digest substitution, treating `can_probe` as binding, selecting Gemini, launching unconfined Grok, retaining an AGREE contradicted by evidence, or skipping re-review. Positive controls cover critics-first plan/design ordering, a confined Grok fallback after Codex preflight failure, and effective `required_prover=false` removing only the Fable hard block without altering the early-evidence ordering contract.
...
review-wave order, Codex `workspace-write` launch branch, per-leg worktree/data isolation, and
coordinator reaping by extending the existing worktree lifecycle rather than adding a second
allocator. Lane C owns
...
that exact dependency-complete base without tracked mutation. Its executable preflight accepts the
future full CONFORM/HARDEN landing SHAs, fetches canonical `main`, proves both are ancestors of the
...
freeze those exact observed values, recomputes every suite/node/anchor aggregate, and passes an
exact-digest review ordered as early Codex evidence, critics including GPT-5.6 Sol, then usable
