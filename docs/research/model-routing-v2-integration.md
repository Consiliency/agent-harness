...
**PNLFEED prompt-feeding amendment (2026-06-30).** Codex and Gemini panel legs now receive the review artifact in the CLI prompt itself. The temp review directory remains useful for staged files and output separation, but no CLI leg depends on implicitly discovering `review-bundle.md` from the directory. Large artifacts use deterministic head/tail inline excerpts plus digest and size metadata rather than unbounded prompt growth.

**PNLCLAUDE Agent View amendment (2026-06-30).** The Claude panel leg now uses local Claude Code Agent View (`claude --bg`) with `claude-sonnet-5`, gated on Claude Code `2.1.197` or newer before launch. Missing, malformed, timed-out, or below-minimum Claude Code states return `UNAVAILABLE`, `TIMEOUT`, or `DEGRADED` leg evidence rather than silently falling back to an older Sonnet path. The implementation keeps provider API keys stripped from the child environment and does not use unsupported headless `claude -p` behavior.
...
- **codex / gemini legs** — replicate the script's per-leg subprocess approach (do not call the whole script):
  - codex: `timeout -k 15s <T>s codex exec --cd <dir> --skip-git-repo-check --sandbox read-only --model gpt-5.5 -c model_reasoning_effort=xhigh --output-last-message <out> "<PROMPT_WITH_INLINE_ARTIFACT>"` (clean review via `--output-last-message`; raw stdout is a noisy transcript; stdin is `DEVNULL`).
  - gemini: `cd <dir> && timeout -k 15s <T+60>s agy --model "Gemini 3.1 Pro (High)" --print-timeout <T>s -p "<PROMPT_WITH_INLINE_ARTIFACT>"` (stdout is the clean response; stdin is `DEVNULL`; `--add-dir` is not the artifact-feeding mechanism).
