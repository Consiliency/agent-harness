...
| STILL-LIVE | The defect is reachable on main; or it is safety-floor class with no reachability negative; or it contradicts a frozen requirement. |
...
<!-- row:R-008 -->
### R-008 — Track deferred CONFORM CI-prerequisite review notes
- **source:** Consiliency/agent-harness#466
- **origin:** CONFORM CI prerequisite, exact-head board for agent-harness#465
- **original ruling:** "These findings are preserved verbatim before the Fable president DEFERRED ruling, per Consiliency/agent-harness#442. They do not block the narrow prerequisite merge."
- **bound criteria:** none
- **current-main check:** c0e51591: no `build==1.5.1` under .github, ci or phase-loop-runtime/pyproject.toml; the tools declare `build==1.6.1`
- **safety floor:** not floor-class — evidence-narration findings about agent-harness#465's bundle and a yanked-pin durability note
- **disposition:** OBSOLETE — The yanked `build==1.5.1` pin these findings concern was removed by `fdd50704`.
- **promotion:** none
- **finding:**
...
````markdown
Follow-up from the exact-head board for Consiliency/agent-harness#465. These findings are preserved verbatim before the Fable president DEFERRED ruling, per Consiliency/agent-harness#442. They do not block the narrow prerequisite merge.

Artifact digest: `e873907acc6268617bc2061de19715253ecc3b3b7b65dfc1e0185417873954c1`
Board digest: `1de95e6ea29503245c29e159d1087d10774f5ada2f4cffe6ac67a4af70588115`

NON-BLOCKING FINDINGS

A. The evidence does not — and at this head cannot — demonstrate the pin's functional effect on
   this PR. Per finding 6, the load-bearing node is skipped at a4f36dc9. The GREEN control was run
   against a different commit (CONFORM candidate 403f0b63…). That is inherent to a
   prerequisite-first split and is not a defect, but it means this PR's own CI can only prove that
   the install step resolves, never that the pin fixes the test. The bundle should say so plainly
   rather than presenting RED/GREEN in a way that reads as validation of this head.

B. The GREEN control's environment is not literally the workflow dependency set. The bundle
   reports `build==1.5.1`, `setuptools`, and `pip`; the workflow installs
   `./phase-loop-runtime[visual]`, `pytest`, `build==1.5.1`, `setuptools>=68`. Verification step 2
   asks for "the exact workflow dependency set", so the reported evidence does not literally
   satisfy the bundle's own step. The deltas are unlikely to change the outcome, but the claim
   should be tightened or the run redone as specified.

C. Matrix-leg coverage is unstated. The change lands on python-version ['3.10','3.11','3.12'];
   the disposable environment's Python version is not recorded. The one new failure mode this PR
   can introduce is at install time — if `build==1.5.1` or the resolved `setuptools` carries a
   `requires-python` above 3.10, the 3.10 leg fails before any test runs. Exact-head CI does
   exercise the install step on all three legs even though the test is skipped, so the required-
   green gate in landing step 4 covers this; it must actually be enforced, and confirmed green on
   all three legs, before merge. The bundle records CI as "running", with no result in evidence.

D. Yanked-pin durability (acknowledged and tracked, restated for the record). Exact `==` is the
   right and only mechanism here: under PEP 592 a resolver skips yanked files unless the version
   is pinned exactly, so the bundle is correct that a range would fall back to 1.5.0. The residual
   cost is that every PR's CI on all three legs now depends on a retracted artifact remaining
   downloadable. #464 keeps the replacement path live, and the docs note correctly refuses to
   present the pin as a production requirement, which is the right containment.

E. Forward-looking maintenance note. The clean-room job is safe today only because
   `source_runtime_available()` is false there (finding 7). If a future change ever copies the
   `src/` tree into the clean room, or the test's standalone branch is removed, the Gate A venv
   would silently need the same pin. Worth a comment at test.yml:116 or in the docs note so the
   asymmetry between the two jobs is not read later as an inconsistency and "fixed" in the wrong
   direction.

NITS (non-blocking, style only)

- The docs note does not mention that the pin applies to all three matrix Python versions, nor
  name the test node it exists for. Naming
  `test_sdist_and_wheel_include_only_digest_enumerated_contract_mirror` would make the note
  self-grounding for whoever unwinds the pin under #464.
````

Comment by ViperJuice at 2026-08-05T23:42:30Z:

````markdown
Additional critic outputs preserved verbatim before the forced round-3 president ruling, so every possible DEFERRED classification has a pre-dispatch durable text record.

## grok-4.5 (verbatim)

I'll start by reading the authoritative review instructions, then the staged review bundle.Next I'll inspect the referenced source files, verification artifacts, and git state against the claimed binding and acceptance criteria.I'll independently verify the git binding, diff scope, frozen changelog, and CONFORM archive-test dependency claims.Verifying git binding, the exact pin's installability, and whether the archive-input claim holds.Running git binding checks and confirming whether the exact yanked pin is installable.# Integration review: `Consiliency/agent-harness#465`

**Scope:** CI prerequisite for CONFORM (`agent-harness#464`)  
**Base → head:** `5c211e76…` → `a4f36dc9…` (tree `63fd76a1…`)  
**Changed paths:** `.github/workflows/test.yml`, `docs/releases/ci-archive-build-tooling.md`

## Decision under review

Authorize a narrow prerequisite that only extends the ordinary pytest matrix install with exact `build==1.5.1` and `setuptools>=68`, plus a docs-freshness release note on an unfrozen path—without touching CONFORM tests, package metadata, runtime deps, release workflows, or the frozen changelog.

## Repo-grounded checks

| Check | Result |
|---|---|
| Exact binding / HEAD | Worktree HEAD is `a4f36dc9523f62a21bc727eb1359faad65b97371` |
| Two-path scope | Terra probe: only workflow + `docs/releases/ci-archive-build-tooling.md` |
| Frozen changelog | Terra probe: `CHANGELOG.md` blob `d57713c…` unchanged at base and head |
| Workflow install line | `pytest` job installs `"build==1.5.1" "setuptools>=68"` |
| Package deps untouched | `phase-loop-runtime/pyproject.toml` still has no runtime `build` dep |
| Release workflows untouched | `publish-*.yml` still use unpinned `build` for directory builds only |
| Docs-freshness | `docs-audit` against base: `docs_freshness: passed`, zero findings |
| Workflow syntax | `actionlint` clean after filtering pre-existing Blacksmith runner labels |

## Load-bearing correctness

1. **Why the pin is needed**  
   The frozen node `test_sdist_and_wheel_include_only_digest_enumerated_contract_mirror` (in `CONFORM_NEW_PRODUCTION_NODE_IDS`) builds from source and then runs:

   `python -m build --wheel --no-isolation … <sdist.tar.gz>`

   That archive-input form requires `build` ≥ 1.5.1. Without `build`/`setuptools` in the ordinary matrix, the test fails prerequisite discovery or the sdist-derived wheel step.

2. **Why exact `==1.5.1` is required**  
   PyPA yanked `1.5.1` (breaking-feature yank, not a known security defect). A range resolves to `1.5.0`, which lacks archive input. Exact `==` is the correct temporary pip override for a yanked version.

3. **Why this job, not package metadata**  
   The pin is only on the ordinary pytest matrix install. Clean-room uses a prebuilt wheel path when source runtime is absent and does not need archive-input support for this gate. Runtime/`pyproject` deps remain clean.

4. **Docs surface is correct**  
   Workflow changes require `CHANGELOG*` **or** `docs/release*` (`docs_surfaces.RELEASE_DOC_REQUIREMENTS`). The new `docs/releases/ci-archive-build-tooling.md` matches, documents CI-only intent, and points at `agent-harness#464` for unpinning—without colliding with CONFORM’s frozen changelog/handoff blobs.

5. **Prerequisite-head skip is expected, not a defect**  
   At this head the archive node is correctly skipped (`CONFORM SL-0 awaits the exact production capability marker`). Local `build 1.5.1` is present and accepts `.tar.gz` source input; TDD RED/GREEN on candidate `403f0b63…` remains the load-bearing archive proof for CONFORM itself.

## Blocking defects

**None.** No correctness, safety, or unmet-acceptance defect in the staged two-file change.

## Non-blocking notes

- Temporary yanked pin is verification infrastructure only; keep `agent-harness#464` open until a non-yanked archive-capable release or a ratified test repair lands.
- Exact-head CI green remains a landing gate (process), not a defect in these bytes.

AGREE


## gpt-5.6-sol (verbatim)

No blocking correctness, safety, or unmet-acceptance defects found.

The exact head/tree and two-path diff match the bundle; `CHANGELOG.md` retains its frozen blob. The pin is CI-only, covers all pytest matrix versions, and supplies PyPA’s documented `.tar.gz` input support introduced in `build 1.5.1`; exact pins remain installable under PyPI’s yanking rules. ([PyPA changelog](https://build.pypa.io/en/stable/changelog.html), [PyPI yanking policy](https://docs.pypi.org/project-management/yanking/))

The immutable archive-test blob matches the CONFORM candidate, the candidate patch applies cleanly atop this prerequisite, and [Consiliency/agent-harness#465](https://github.com/Consiliency/agent-harness/pull/465) is at the bound head with all reported exact-head checks successful. The prescribed ordered review, merge-commit landing, candidate rebase, and post-rebase verification remain required.

AGREE

## gemini-3.6-flash (verbatim)

### Executive Summary

The proposed change in PR **Consiliency/agent-harness#465** is a minimal, targeted CI prerequisite update designed to enable the CONFORM archive-input test suite in `agent-harness#464` to pass without leaking tooling dependencies into production package requirements or violating frozen changelog constraints. 

No blocking correctness, safety, or unmet-acceptance criteria defects were identified.

---

### Operating Constraint & Verification Caveats

In strict accordance with the headless execution constraints:
1. **No commands or scripts were executed**: Claims regarding test execution (e.g., RED state under `build 1.5.0` vs. GREEN state `1 passed in 30.75s` under `build==1.5.1`) and workflow syntax validation could not be directly executed or independently re-verified at runtime. They are evaluated based on the staged evidence provided in `review-bundle.md`.
2. **External paths were not opened**: The referenced context files outside the staged directory (`/mnt/HC_Volume_105438154/...`) could not be read or opened. The review was conducted exclusively against the staged `review-instructions.md` and `review-bundle.md` files located in `/tmp/pl-panel-3qsonw9h/review/`.

---

### Whole-Feature Integration & Safety Analysis

1. **Exact Pinning of Yanked Version (`build==1.5.1`)**:
   - PyPA / PyPI yanked `build 1.5.1`. Under standard PyPI resolution semantics, dependency range specifiers (e.g. `>=1.5.1`) ignore yanked versions and fall back to unyanked releases like `1.5.0` (which lacks sdist archive input support).
   - Specifying an exact pin (`build==1.5.1`) forces `pip` to install the yanked version specifically for the test matrix job, allowing the sdist archive-derived wheel build tests to run successfully.

2. **Isolation to CI Environment**:
   - The dependency additions (`"build==1.5.1"` and `"setuptools>=68"`) are restricted solely to the `pip install` step of `.github/workflows/test.yml`.
   - No runtime setup files (`setup.py`, `pyproject.toml`, etc.) are modified, preventing CI verification tooling from leaking into release artifacts.

3. **Changelog and Repository Integrity**:
   - `CHANGELOG.md` remains untouched at the frozen parent blob (`d57713c07468aba942c31744d0a272dd73771f2f`).
   - Docs-freshness requirements are cleanly satisfied via the new unfrozen path `docs/releases/ci-archive-build-tooling.md`, which properly documents the temporary nature of the pin and references `agent-harness#464` for future unpinning/tightening.

4. **Acceptance Criteria Alignment**:
   - The change strictly matches the authorized scope: updating dependency installation in `.github/workflows/test.yml` and adding `docs/releases/ci-archive-build-tooling.md`.
   - No implementation files, package dependencies, release workflows, or CONFORM test files are altered.

---

### Conclusion

The PR is correctly scoped, safe, and meets all acceptance criteria without introducing blocking defects.

AGREE
````

<!-- row:R-009 -->
### R-009 — CONFORM Gate A prerequisite: deferred exact-head review notes from agent-harness#468
- **source:** Consiliency/agent-harness#471
- **origin:** CONFORM Gate A prerequisite, exact-head board of agent-harness#468 at `b0ce04cd`
- **original ruling:** "This issue is the required pre-dispatch binding for findings forced to `DEFERRED` at the three-round Fable-president cap"
- **bound criteria:** none
- **current-main check:** c0e51591: Gate A sparse clone still present (phase-loop-runtime/scripts/gate_a_cleanroom.sh:236)
- **safety floor:** not floor-class — each finding is recorded as fail-closed, inert, or comment wording
- **disposition:** PARKED — President-DEFERRED hardening notes, each fail-closed or inert on CI.
- **promotion:** none
- **finding:**
...
Downstream hardening notes from the exact-head governed review of `Consiliency/agent-harness#468` at `b0ce04cd68db66aaf54ff909ec95553e3c99ef21`. This issue is the required pre-dispatch binding for findings forced to `DEFERRED` at the three-round Fable-president cap under `Consiliency/agent-harness#442`. None is a blocking correctness, safety, or unmet-acceptance defect in `Consiliency/agent-harness#468`.

## Verbatim stable findings

### OPUS-468-N001 — DEFERRED

> Dirty-tree divergence: the wheel is built from the working tree while evidence is cloned from HEAD; CI is clean and the plan imposes clean-tree checks, but a local dirty run could compare different bytes.

### OPUS-468-N002 — DEFERRED

> Path-resolution portability: resolved site-packages and source paths can differ under symlink-rewriting temp roots; this is fail-closed and does not affect ubuntu-2404 CI.

### OPUS-468-N003 — DEFERRED

> The candidate-source .pth remains present for the later profile-command seam probe; entry points still come from dist-info, so this does not weaken the seam.

### OPUS-468-N004 — DEFERRED

> The private sparse clone retains an origin pointing to the source checkout; it is detached at a fixed SHA and performs no later fetch, so this is inert but could be hardened by removing the remote.

### OPUS-468-N005 — DEFERRED

> The four-node deselection comment attributes the exclusion to unavailable documents, while the precise reason is the plan's pre-doc A2 partition; mechanism and selected node set are correct.

## President record

Fable returned usable `AGREE`, usable `AGREE`, and usable `PARTIALLY AGREE` across the three permitted rounds, each time finding no blocking defect and affirming that `Consiliency/agent-harness#468` may land before the CONFORM implementation is rebased. It did not emit the requested per-ID ledger and originated additional advisory observations, contrary to the ratified president grammar. At the hard cap, the coordinator therefore forces `PROCEED` and maps the five stable board findings above to `DEFERRED`.

President evidence SHA-256 values:

- round 1: `b367e5dfccfd196450581c3be25fcb3bcb83e03c50a1e2057086b01732ae8f0d`
- round 2: `c91796760eb8b658a7e9ce5abb83a1162787342eed2e1d9a8187c9b10928b1b0`
- round 3: `caf4bc9d0431ff1033f7c0430dee762d8dac7f752aef08dcdd0cabdd4a3d12f0`

Fable-originated observations are retained as advisory evidence for downstream triage but are not presidential findings or blockers, because `Consiliency/agent-harness#442` forbids the president from originating findings. Related already-owned fixture failures remain in `Consiliency/agent-harness#470`.
````
...
<!-- row:R-012 -->
### R-012 — Harden CONFORM final-parent attestation after agent-harness#495
- **source:** Consiliency/agent-harness#496
- **origin:** CONFORM, Fable president on exact-head agent-harness#495, Opus prover observations 5.1-5.5
- **original ruling:** "These are fail-closed hardening and wording follow-ups, not blockers for agent-harness#495."
- **bound criteria:** none
- **current-main check:** c0e51591: `_sealed_candidate_parent` still at phase-loop-runtime/tests/_outside_agent_canonical.py:1794
- **safety floor:** not floor-class — the ruling records the failure mode as fail-closed and unable to produce a false accept; the rest are nits
- **disposition:** PARKED — President-DEFERRED fail-closed hardening and wording.
- **promotion:** none
- **finding:**
...
````markdown
Follow-up ledger for observations deferred by the Fable president on exact-head agent-harness#495. The text below is copied verbatim from the Opus prover as required by the board policy.

## Verbatim deferred observations

5.1 #493's evidence set is narrower than #481's and #484's in one respect: it
requires no fresh full-suite zero-failure result and no B2-mode GREEN. Since
the rebind is inert in ordinary CI (§3) the regression surface is small, and
the source-release primary bytes cover the shape risk. Adding a B2-mode GREEN
at the candidate would make the proof symmetric with the earlier repairs at
near-zero cost.

5.2 The attestation covers CHANGELOG.md only, but
_sl2_compatibility_transitioned() asserts all-or-nothing across all four
SEALED_RELEASE_FINAL_PATHS (test_outside_agent_conform_evidence.py:486-499),
and _sealed_candidate_parent() matches on the eleven non-None
SEALED_RELEASE_CANDIDATE_PARENT_BLOBS entries
(_outside_agent_canonical.py:1726-1752). If any of those also drifted, the
authorized one-hunk repair would be insufficient and the attestation as
specified would not reveal it — it would surface later, fail-closed, in the B2
replay. Recommend extending the attestation to `git rev-parse
<final-parent-repair-parent>:<path>` for all four final paths and the eleven
non-null candidate-parent paths. Non-blocking because the failure mode is
fail-closed and cannot produce a false accept.

5.3 Residual drift window. The attestation binds the pin to the merge-base of
the #493 candidate with canonical main, not to canonical main's head, and not
to the eventual pre-doc implementation parent. Any later commit to main that
touches CHANGELOG.md before the implementation lands re-stales the pin — the
same failure #489/#491 caused — and would then require a further amendment,
since tests are frozen. This is inherent to pinning rather than deriving, is
fail-closed, and is out of scope for a docs-only amendment; recording it as a
watch item. The clone-state observation in §1 (origin/main at 19f6286…, two
fast-forwards on the review date) is the concrete reason to compute the
attestation against a freshly fetched canonical main immediately before merge,
which is what the amendment already says.

5.4 Style nit only: the assertion at _outside_agent_canonical.py:2112 carries
no CONFORM_RED anchor, so a stale final-parent pin surfaces as a bare
AssertionError from sealed_release_evidence() rather than a named anchor. Not
this change's concern; noted because it is part of why #493 needs out-of-band
evidence.

5.5 Wording nit: the second paragraph calls #493 "the separately landed …
repair" while the first calls it "pending". Read in context this means "landed
as its own merge", but "separately landing" would be unambiguous.

## President correction and disposition
Fable classified 5.1-5.5 DEFERRED and returned AGREE. In observation 5.2, `SEALED_RELEASE_CANDIDATE_PARENT_BLOBS` contains ten non-null entries, not eleven; the referenced non-null set remains unambiguous. These are fail-closed hardening and wording follow-ups, not blockers for agent-harness#495.
...
<!-- row:R-020 -->
### R-020 — test(FABREADMIT): tighten deferred epoch, inventory, and flag-off controls
- **source:** Consiliency/agent-harness#640
- **origin:** FABREADMIT SL-0 exact-digest board, candidate `e6f320e8`
- **original ruling:** "The separate Grok 4.6 president classified the following findings as downstream tightening rather than blockers to the tests-only freeze."
- **bound criteria:** none
- **current-main check:** c0e51591: `_assert_head_append_inventory` still at phase-loop-runtime/tests/test_fab_delta_consumer.py:1396
- **safety floor:** not floor-class — test-strength findings on frozen SL-0 controls; the ruling records no contradictory production behaviour
- **disposition:** PARKED — President-DEFERRED test tightening.
- **promotion:** none
- **finding:**
...
The exact-digest FABREADMIT SL-0 board reviewed candidate `e6f320e85fd226853b356103ee8451401ecdf9c1`. The separate Grok 4.6 president classified the following findings as downstream tightening rather than blockers to the tests-only freeze.

- President ruling SHA-256: `80f0cb6ef3e0f3c3656b132bc0371c0c20c500ec2efd26155248a8ab700d1b34`
- Consolidated findings SHA-256: `0e1700ff53541a731719652c595f559a7c98c24324753defd056d6050d341798`

## Verbatim deferred findings

FINDING FR-R5-08: DEFERRED — Defaulted `getattr` at broker:170, admission:166, consumer:1484-1485, premerge:194, and promotion:1575 can pass with the attribute absent, but direct epoch asserts exist on other arms (verbs:230, consumer:1246) and this does not force a contradictory production behavior. File before dispatch; not freeze-gating.

FINDING FR-R5-09: DEFERRED — `_assert_head_append_inventory` does exempt all of `_run_train_unfenced` (consumer:993), so an in-function third `pr_open` append would escape the module-scope probe. The inventory still requires the helper site and forbids `_fab_delta_readmit` appends, which catches the current unauthorized pair. Narrow the exemption or add an in-function mutation arm; file before dispatch.

FINDING FR-R5-10: DEFERRED — The node is a `_commit_broker_readmitted_head` symbol probe plus a replay of `test_flag_off_resume_does_not_run_torn_recovery`. Redundant, not a false-green that licenses a bad readmission implementation. File before dispatch.

## Disposition

Non-blocking for the current SL-0 freeze by president ruling. Address after FABREADMIT production lands, without editing the frozen tests-only tree unless the governed lifecycle explicitly authorizes a successor test contract.

````

Comment by ViperJuice at 2026-08-23T23:22:34Z:

````markdown
Scope update after inspecting current local head `df9ba8637e77c99ca1e57f98a467aba9c23587c7` for the FABREADMIT SL-0 candidate:

- FR-R5-08 is already resolved: the six defaulted `getattr(..., default)` assertion probes present at `e6f320e8` are absent from the current head.
- FR-R5-09 remains actionable: `_assert_head_append_inventory()` still exempts the entire `_run_train_unfenced` function, so an additional in-function `pr_open` append can evade the inventory assertion.
- FR-R5-10 remains a low-priority redundant/structural coverage concern, not a demonstrated false green.

Recommendation remains to keep this issue open and narrow its live scope to FR-R5-09/10. Preserve the frozen SL-0 tests now; repair the inventory assertion and consolidate the redundant flag-off probe only through an authorized separately reviewed successor test contract after FABREADMIT production lands and before dispatch.
````

<!-- row:R-021 -->
### R-021 — test(FABREADMIT): harden deferred chronology, authority receipt, and routing controls
- **source:** Consiliency/agent-harness#652
- **origin:** FABREADMIT SL-0 exact-digest board, candidate `9ff9807c`, plus later-round deferrals in comments
- **original ruling:** "The separate Grok 4.6 president classified the following findings as downstream tightening rather than blockers to the tests-only freeze."
- **bound criteria:** none
- **current-main check:** c0e51591: `test_fabreadmit_broker_authority_receipt_contract` still at phase-loop-runtime/tests/test_fabreadmit_broker.py:43
- **safety floor:** not floor-class — test-strength findings on frozen SL-0 controls, not production code paths
- **disposition:** PARKED — President-DEFERRED test tightening.
- **promotion:** none
- **finding:**
...
The exact-digest FABREADMIT SL-0 board reviewed candidate `9ff9807ceb4257eab9af78db058bcc83b4f5c6ab`. The separate Grok 4.6 president classified the following findings as downstream tightening rather than blockers to the tests-only freeze.

- Consolidated findings SHA-256: `1654b1bcd25054f32edf1de753fe0f70a9ce7f64cf91b2338b6d1674dbf9695c`
- President ruling SHA-256: `e5dd3e542dfa0b3a3b073b8c57955d3491b77fa1b36ad78b82699e9dd54295cb`
- Related deferred test-hardening issue: agent-harness#640

## Verbatim deferred findings

FR-R11-05: SL-0 requires the chronology `provenance fsync → gate PASS → broker grant → durable ledger append`, but the broker-backed commit-point tests only order the grant before the helper/append and never snapshot the admission store after a provenance-fsync or gate failure; an implementation granting admission before either prerequisite survives; required tests-only correction: add an ordered event spy and pre-grant crash/gate-failure arms proving zero durable admission.

FR-R11-06: `test_fabreadmit_broker_authority_receipt_contract` constructs literal `attempt_identity` and `authority_digest` values but never proves service-derived determinism or durable linkage, allowing constant identities/digests to pass; required tests-only correction: replay identical valid authority and vary one bound authority field, asserting deterministic equality/difference, single-allocation replay, and receipt identity/digest equality with the durable admission request/binding.

FR-R11-07: the rejection matrix lacks both the required valid-but-unrelated prior authority arm and a router-level canonical-repository mismatch before store selection—the existing mismatch only corrupts checkpoint JSON while authority and worktree still agree; required tests-only correction: exercise two real canonical repositories and a valid prior with incompatible train/FAB-run/roadmap authority, asserting rejection before selection and unchanged stores/evidence.

## President disposition

- FR-R11-05 widens chronology coverage but does not make the current helper-reachability test vacuous or false-RED.
- FR-R11-06 is a shape-contract hardening gap rather than a contradiction of a frozen invariant.
- FR-R11-07 extends the rejection matrix; router-before-selection remains an implementation obligation for SL-1.

These findings are non-blocking for the current SL-0 freeze. Address them after FABREADMIT production lands, without editing the frozen tests-only tree unless the governed lifecycle explicitly authorizes a successor test contract.

````

Comment by ViperJuice at 2026-08-24T23:47:53Z:

````markdown
Round-13 exact-head president disposition at candidate `1d6e428aa539a5fdb1d5d3c1f4b88d216e2bb9d2` adds the following verbatim deferred finding to this issue's routing-identity scope:

FINDING FABREADMIT-SL0-ROUTING-IDENTITY: DEFERRED — no SL-0 EC or SL-0 rejection-matrix task requires an authority.repository vs adapter-worktree routing-client denial; checkpoint-root repository mismatch and the positive `run_train` routing path are already present, so this SL-1 router check may be filed rather than blocking the tests-only freeze.

President ruling artifact: `/tmp/fabreadmit-sl0-review-1d6e428a-codex-fallback/04-president-ruling.json` (coordinator-local by-reference artifact). This is non-blocking for the current SL-0 freeze and remains a downstream successor-contract item.

````

Comment by ViperJuice at 2026-08-25T00:13:53Z:

````markdown
FABREADMIT SL-0 round-14 Grok 4.6 president deferral, candidate `cb0375073706a5f651da1cec5b305b520300b9ae`, exact finding text preserved before repair dispatch:

> FINDING FABREADMIT-SL0-NO-ADAPTER-COUNTER: DEFERRED — The frozen no-adapter control constructs `test_fabpub_shared_epoch._CountingAdapter` (`calls: list`), so `len(...)` is valid; the unused module-local integer adapter is not the asserted object.

The same ruling marked `FABREADMIT-SL0-ALLOCATOR-BEHAVIOR` and `FABREADMIT-SL0-EC5-ALIAS` BLOCKING and forced `RETURN FOR TESTS-ONLY REPAIR`. Full local ruling artifact: `/tmp/fabreadmit-sl0-review-cb037507-codex-fallback/04-president-ruling.json`.

````

Comment by ViperJuice at 2026-08-25T01:40:41Z:

````markdown
FABREADMIT SL-0 round-17 Grok 4.6 president deferrals, candidate `b9e2c80b09ccce184153c78f304fbb5439c7494f`, tree `7af245b0b7fbf11961874c783023ef0183bacfed`, exact finding text preserved before dispatch:

> FABREADMIT-SL0-EC4-SCAN-CONTROL: The hardcoded-epoch scanner has no clean synthetic-root control; an implementation returning `search_root is None` passes the dirty-root and default-root assertions without performing an AST scan. Add paired clean/dirty synthetic-tree controls through the same `search_root` path.

President disposition: **DEFERRED**. EC-4 already exercises the named dirty-root, enablement-false, and default-root controls; the paired synthetic control is downstream hardening.

> FABREADMIT-SL0-ROUTING-IDENTITY: No activated test submits an authority whose `repository` disagrees with its `adapter_worktree` canonical repository to the real routing client. Add a two-repository denial test asserting rejection before either store changes and with zero adapter calls.

President disposition: **DEFERRED**. Prior ruling stands; this is an SL-1 routing check and this digest did not change its premise.

> FABREADMIT-SL0-FABPUB-INTERLOCK: The readiness tests remove the FABREADMIT test environment before reload and never falsify active FABPUB capability. Add an environment-activated negative control with FABPUB inactive, asserting readiness and shortcut enablement remain false.

President disposition: **DEFERRED**. Active FABPUB is an SL-3 enablement conjunct, not an SL-0 interlock arm.

> FABREADMIT-SL0-ROUTING-AUTHORITY: The supposed routing spy observes `_commit_broker_readmitted_head`, never `routing_client.readmit_advanced_head`; direct service construction can pass, durable owned-scope derivation is not inspected, and no two-repository authority/worktree mismatch is rejected. Capture the authority entering the real router, assert its durable scope and canonical identity, and add mismatch denial before either store or adapter changes.

President disposition: **DEFERRED**. This is the same downstream SL-1 check as routing identity; the current SL-0 real-router and resolver-kill controls remain valid.

> FABREADMIT-SL0-CHRONOLOGY: No executable assertion orders provenance fsync, merged-gate PASS, broker grant, ledger append, in-memory advance, and exact-head merge; allocating before review/gate still passes. Add ordered instrumentation through `run_train` plus review-reject and gate-block controls proving the admission store remains unchanged.

President disposition: **DEFERRED**. Prior FR-R11-05 ruling stands; this extends chronology coverage without falsifying the frozen helper-reachability or end-to-end outcome controls.

> FABREADMIT-SL0-RECEIPT-INTEGRITY: The invalid-receipt matrix covers only absence and `proposed_head_sha`; a helper validating neither `prior_head_sha` nor `authority_digest` passes, and deterministic attempt identity is never checked. Add independent prior-head and authority-digest mutations with zero append, and assert replay returns the same attempt identity bound to the durable admission request.

President disposition: **DEFERRED**. Prior FR-R11-06 ruling stands; these mutations extend the existing receipt matrix.

> FABREADMIT-SL0-NO-PROVIDER-REQUEST: Zero adapter calls do not prove the frozen requirement that readmission never constructs `BrokerRequest`; an implementation may build and discard one while all tests pass. Add a construction-level exploding spy or equivalent AST/behavioral falsifier around direct and `run_train` readmission.

President disposition: **DEFERRED**. SL-0 requires zero provider-adapter calls, already asserted; never constructing `BrokerRequest` is an SL-1 implementation constraint.

Forcing decision: **proceed with SL-0 freeze of `b9e2c80b`**. Full local ruling artifact: `/tmp/fabreadmit-sl0-review-b9e2c80b-codex-fallback/04-president-ruling.json`.
...
<!-- row:R-023 -->
### R-023 — Manifest reconciliation follow-up: 4 surviving mutants on the ref-identity predicate, and three wording nits
- **source:** Consiliency/agent-harness#839
- **origin:** manifest reconciliation, final review round of agent-harness#832 (merged `c6b9924b`, 4/4 AGREE)
- **original ruling:** "All of this is **coverage and wording, not behaviour**"
- **bound criteria:** none
- **current-main check:** c0e51591: `_raw_ref_identity_is_readable` still at phase-loop-runtime/src/phase_loop_runtime/plan_manifest.py:1005
- **safety floor:** not floor-class — mutation coverage and wording on a manifest ref-identity predicate; the reviewer recorded correct behaviour on every shape built
- **disposition:** PARKED — Non-blocking coverage and wording follow-up.
- **promotion:** none
- **finding:**
...
````markdown
Follow-up from the final review round of Consiliency/agent-harness#832 (merged as `c6b9924b`, 4/4 cross-vendor AGREE at round 16). **Deliberately not folded onto that PR**: the 4/4 was bound to an exact head, and moving the head after a converged review would have invalidated it.

All of this is **coverage and wording, not behaviour** — the `fable` seat that found it was explicit that the merged code behaves correctly on every shape it built.

## 1. Four surviving mutants on `_raw_ref_identity_is_readable`

The seat mutated every operand of the predicate in one pass, so this is the complete set:

| mutant | change | behaviour the suite would not catch regressing |
|---|---|---|
| X7 | `return name is None or isinstance(name, str)` → `return isinstance(name, str)` | keep direction for a legacy "names nothing" ref |
| X8 | early-return `slug.strip() not in (…)` → `slug not in (…)` | destroy direction: whitespace slug + non-string `ref.file` |
| X9 | `("", "None")` → `("",)` | destroy direction: `"None"` slug + non-string `ref.file` |
| X14 | `("", "None")` → `("None",)` | destroy direction: `""` slug + non-string `ref.file` |

**X7 is the one with a documented shape behind it.** Measured through a real manifest and `render._manifest_disagreements` (snapshot `P: executing`, row `completed`, roadmap `v2`), every legacy "names nothing" ref — `{}`, `{"slug": null}`, `{type, status only}`, `{"slug": null, "file": null}`, `{"slug": "", "file": null}` — is kept and reported at head, and destroyed with only the INCOMPLETE note under X7.

It survives for the **same structural reason** the r15 finding did: `roadmap_ref: {}` is parametrised in `test_a_roadmap_ref_that_NAMES_NOTHING_is_not_read_as_a_FOREIGN_roadmap`, which reaches the detector through `read_manifest` — **not** through `parseable_plan_entries`, the loader this predicate guards. A test that names a shape but never traverses the guard cannot pin it.

X8/X9/X14 would only matter if the active roadmap's stem equalled something like `"123"` or `"True"`. **0 of 730 dict refs across 17 host manifests** have a non-resolving slug.

**Suggested fold:**
- add X7 to `scripts/mutate_phase_state_guards.py`, and add `{}` / `{"slug": null}` as parameters of `test_a_RESOLVING_ref_FILE_keeps_the_row_when_the_slug_does_not_resolve`, which already asserts through `parseable_plan_entries`;
- add one destroy-direction loader test parametrised over `slug ∈ {"", "   ", "None"}` × `"file": 123`, asserting `entries == 0` — covers X8, X9 and X14.

## 2. The INCOMPLETE note understates a structural failure

`render._INCOMPLETE_NOTE` says *"at least one manifest row could not be read"*. On a structural failure — an unsupported `schema_version`, a non-array `plans`, malformed JSON — **no** row was read. True but understated; the `state_reconciliation_incomplete` flag carries the meaning. A distinct message for the structural case would be more accurate.

## 3. Two record-keeping nits

- `_manifest_disagreements`' return annotation still reads `list[tuple[str, str, str]]`; it returns `(clashes, complete)`. (gemini, r16.)
- Recorded equivalent #4's proof in `scripts/mutate_phase_state_guards.py` has two sentences spliced together: *"Measured: the mutant measured at r11; re-verified by the seat every round since, where it was CAUGHT at r10."* The equivalence holds; only the record is garbled.

## Scope

Small, and entirely inside the files Consiliency/agent-harness#832 touched. It should go through its own review — and it would be a good candidate for the one-pass-over-every-operand method that found it, rather than the finding-per-round pattern that preceded it.

Related: Consiliency/agent-harness#837 (an unrecognised manifest status is inert and `validate_manifest` has no `src/` caller).

🤖 Generated with [Claude Code](https://claude.com/claude-code)

https://claude.ai/code/session_014isLC4AJBWT6yBm2qnUaQ5

````

Comment by ViperJuice at 2026-09-13T17:12:12Z:

````markdown
**One user-visible effect of the merge to know about: greenfield now prints the INCOMPLETE note.**

The r16 `fable` seat probed every repo under `/home/viperjuice/code` (17 manifests, plus synthetic shapes: 73 checks, no mismatches) to confirm the structural-failure fix only fires on real failures. It found exactly one real repo where `status` output changes: **greenfield**.

greenfield's `plans/manifest.json` uses the older `plan_manifest.v0.1` format. The loader rejects that on purpose, raising `unsupported manifest schema_version`. Before Consiliency/agent-harness#832, the render layer caught that error and reported a **complete** reconciliation with no output. That claimed a check that never ran. It now prints the INCOMPLETE note and sets `state_reconciliation_incomplete`.

**This is a true positive, not a regression.** In that repo the manifest-vs-runner check has never run, and `status` now says so. Two ways to resolve it, neither urgent:
- migrate greenfield's manifest to the current schema, so the reconciliation actually runs; or
- give the structural case its own message (item 2 above): "this manifest's schema is not supported by this runtime" is more useful to a greenfield operator than "at least one row could not be read".

The second option is why item 2 is worth doing even though it is cosmetic. greenfield is the concrete case where the current wording would mislead.
