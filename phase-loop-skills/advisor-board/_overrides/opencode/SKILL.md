...
| <harness> | TUI adapter | `codex` CLI | `agy` CLI | `grok` CLI |
...
detects no other host, so under codex, gemini or <harness> the host's own vendor seat is still
launched as a CLI subprocess; the remaining cells are tracked on agent-harness#924. On this host: the Claude seat is the TUI adapter's, never this host's Task/subagent primitive. No native cell is defined for <harness> yet; every seat runs as a CLI subprocess launched by the runtime.
