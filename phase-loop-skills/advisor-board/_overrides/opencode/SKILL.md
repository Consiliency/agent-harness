...
launched as a CLI subprocess; the remaining cells are tracked on agent-harness#924. On this host: the Anthropic seat is the TUI adapter's, never this host's Task/subagent primitive. No native cell is defined for `opencode` yet; every seat runs as a CLI subprocess launched by the runtime.
