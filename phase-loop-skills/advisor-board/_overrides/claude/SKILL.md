...
2. Spawn a native sub-agent (the Agent tool, `model: "opus"` or the seat's model) with `instructions.md` as its brief and `artifact.md` as the material; it must end with `AGREE` / `PARTIALLY AGREE` / `DISAGREE` on the LAST line. Write its review to `native-fill/<request_id>/<harness>.md`.
3. `phase-loop advisor-board <artifact> --native-leg claude=native-fill/<request_id> --json` — preflights the fill (duplicate, ineligible seat, artifact/brief digest, composition drift → typed refusal, zero launches), runs the other seats once, binds the fill before the president rules, and counts it as usable only when its last line is a conforming verdict.
